...
/-!
# Finite products of manifolds

A finite product of `C^r` manifolds is a `C^r` manifold over the product
model. Mathlib provides the product `ChartedSpace` instance but notes the
`IsManifold` instance for finite products as a gap; this module fills it:
the product of factorwise `C^r` atlases has `C^r` transition maps, checked
coordinatewise.

## Main definitions

* `instIsManifoldPi` — the canonical `IsManifold (ModelWithCorners.pi I) r (∀ i, M i)`
  instance from factorwise `IsManifold` instances.

## Main results

* `contDiffGroupoid_pi` — a finite product of `C^r` coordinate changes is `C^r`.
* `isManifold_pi` — specialization to `C^0` Euclidean-model factors
  (Lee, Example 1.8: product manifolds).

Reference: Lee, *Smooth Manifolds*, Ch. 1.

Ported from SmoothManifoldsLee SmoothManifoldsLee/Chap01/Sec01/Example_1_8.lean (a5f308c)
-/
...
factorwise transition maps. **Eng.** `OpenPartialHomeomorph.pi` commutes with
`symm.trans`; both sides are compared coordinatewise. -/
...
/-- **Math.** A point lies in the domain of a product transition map iff each
coordinate lies in the corresponding factorwise domain. **Eng.** Membership in
the `contDiffPregroupoid` source condition for `ModelWithCorners.pi I` separates
into the factorwise source conditions. -/
...
/-- **Math.** Each coordinate projection on a finite product of normed spaces is
`C^r`. **Eng.** The projection is continuous linear, hence `ContDiffOn` on any
set; stated for direct reuse against `contDiffOn_pi`. -/
...
/-- **Math.** A finite product of `C^r` coordinate changes is again a `C^r`
coordinate change. **Eng.** Membership in `contDiffGroupoid r (ModelWithCorners.pi I)`
is verified through `contDiffPregroupoid`, checking the forward and inverse
transition maps coordinatewise. -/
...
/-- **Math.** A finite product of `C^r` manifolds is a `C^r` manifold over the
product model. **Eng.** Canonical owner instance over `ModelWithCorners.pi`;
primitive data are the factorwise charted-space and manifold structures. -/
...
topological manifold structure modeled on the product of the factor Euclidean
spaces (Lee, Example 1.8). **Eng.** `C^0` Euclidean specialization of
`instIsManifoldPi`; holds by instance search. -/
