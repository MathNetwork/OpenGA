...
The Phase 3.4 (revised) `BoundedVariationFunction` primitive provides
...
finite-difference quotient) once a `BoundedVariationDistributionalDerivative`
...
self-build via Federer slicing once a `BoundedVariationDistributionalDerivative`
primitive is available (~80 LOC), or wait for Mathlib upstream BV
infrastructure.

**Ground truth**: Maggi 2012 Proposition 13.1. -/
theorem levelSet_finitePerimeter_exists (u : BoundedVariationFunction E) :
...
`Classical.choose`. -/
noncomputable def levelSet (u : BoundedVariationFunction E) : ℝ → FinitePerimeter E :=
...
theorem levelSet_carrier_iff (u : BoundedVariationFunction E) :
...
when a `BoundedVariationDistributionalDerivative` primitive matures (~250 LOC), or
...
theorem coarea_formula (u : BoundedVariationFunction E) :
    totalVariation u =
...
@[simp] theorem coarea_formula_finite (u : BoundedVariationFunction E) :
    ∫⁻ t, (levelSet u t).perimMeasure Set.univ ∂(MeasureTheory.volume : Measure ℝ) < ⊤ := by
  rw [← coarea_formula u]
...
example (u : BoundedVariationFunction E) :
    totalVariation u =
