...
/-!
# GeometricMeasureTheory.Isoperimetric.BoundedVariationFunction
...
The framework's `BoundedVariationFunction` bundles the underlying measurable function
...
Without `BoundedVariationFunction.totalVariation`, those statements would degenerate
to vacuous existence quantifiers; with it they carry the actual
mathematical content $\|u\|_{L^p}$ and $\|Du\|$.

## Bridge investment

Phase 3.4 (revised) treats `BoundedVariationFunction` as a **bridge primitive** in
...
/-! ## BoundedVariationFunction structure -/
...
structure BoundedVariationFunction (E : Type*) [MeasurableSpace E] where
...
namespace BoundedVariationFunction

variable {E : Type*} [MeasurableSpace E]

/-- BV functions coerce to underlying functions. -/
instance : CoeFun (BoundedVariationFunction E) (fun _ => E → ℝ) := ⟨BoundedVariationFunction.toFun⟩

end BoundedVariationFunction
...
noncomputable def totalVariation (u : BoundedVariationFunction E) : ENNReal := u.totalVariation
...
namespace). -/
@[simp] theorem totalVariation_lt_top' (u : BoundedVariationFunction E) :
    totalVariation u < ⊤ := u.totalVariation_lt_top

/-- Total variation is non-negative (trivial via `ENNReal`). -/
theorem totalVariation_nonneg (u : BoundedVariationFunction E) :
...
self-build of Maggi 13.1 when a `BoundedVariationDistributionalDerivative` primitive
...
function $\chi_\Omega$ as a `BoundedVariationFunction`. Real `noncomputable def`
via `Classical.choose` over the existence axiom
`characteristic_isBV`. -/
noncomputable def ofFinitePerimeter (Ω : FinitePerimeter E) : BoundedVariationFunction E :=
...
section BoundedVariationFunctionTest

variable {E : Type*} [MetricSpace E] [MeasurableSpace E] [BorelSpace E]

/-- Self-test: `ofFinitePerimeter` produces a `BoundedVariationFunction`. -/
noncomputable example (Ω : FinitePerimeter E) : BoundedVariationFunction E := ofFinitePerimeter Ω
...
example (u : BoundedVariationFunction E) : totalVariation u < ⊤ := by simp

end BoundedVariationFunctionTest
