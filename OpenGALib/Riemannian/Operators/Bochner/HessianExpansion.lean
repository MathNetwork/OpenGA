...
theorem mfderiv_gradientNormSq_apply
    ...
    (h_grad_y : TangentSmoothAt (manifoldGradient (I := I) HasMetric.metric f) y) :
    mfderiv I 𝓘(ℝ, ℝ) (‖grad_g[I] f‖²_g) y v
      = 2 * metricInner y
              (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y v)
              (manifoldGradient (I := I) HasMetric.metric f y) := by
  show mfderiv I 𝓘(ℝ, ℝ)
        (fun z : M => metricInner z (manifoldGradient (I := I) HasMetric.metric f z)
                                      (manifoldGradient (I := I) HasMetric.metric f z)) y v = _
  ...
  have h := leviCivitaConnection_metric_compatible HasMetric.metric
  ...
  -- Cast h to typeclass `metricInner` abbrev form for rw matching.
  change ((mfderiv% fun y' => metricInner y' (manifoldGradient (I := I) HasMetric.metric f y')
              (manifoldGradient (I := I) HasMetric.metric f y')) y) v
        = metricInner y
            ((leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
              (manifoldGradient (I := I) HasMetric.metric f) y v)
            (manifoldGradient (I := I) HasMetric.metric f y)
          + metricInner y (manifoldGradient (I := I) HasMetric.metric f y)
            ((leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
              (manifoldGradient (I := I) HasMetric.metric f) y v) at h
  rw [h, metricInner_comm y (manifoldGradient (I := I) HasMetric.metric f y)
       ((leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
          (manifoldGradient (I := I) HasMetric.metric f) y v)]
  -- `covDerivAt HasMetric.metric Y x = lcc.toFun Y x` def-eq; close `a + a = 2 * a` via ring after `show`.
  show metricInner y
        ((leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
            (manifoldGradient (I := I) HasMetric.metric f) y v)
        (manifoldGradient (I := I) HasMetric.metric f y)
...
theorem hessian_gradientNormSq_apply_section
    ...
    (h_grad : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
              (fun y => (⟨y, manifoldGradient (I := I) HasMetric.metric f y⟩ : TangentBundle I M))) :
    hessian (I := I) (M := M) HasMetric.metric (‖grad_g[I] f‖²_g) B.toFun B.toFun x
    = 2 * (metricInner x
            (secondCovDerivSection (I := I) (M := M) HasMetric.metric              (manifoldGradient (I := I) HasMetric.metric f) B.toFun B.toFun x)
            (manifoldGradient (I := I) HasMetric.metric f x)
         + metricInner x
            (covDeriv HasMetric.metric B.toFun (manifoldGradient (I := I) HasMetric.metric f) x)
            (covDeriv HasMetric.metric B.toFun (manifoldGradient (I := I) HasMetric.metric f) x)) := by
  classical
  let gradF : SmoothVectorField I M := ⟨manifoldGradient (I := I) HasMetric.metric f, h_grad⟩
  have h_grad_smoothAt : ∀ y, TangentSmoothAt (manifoldGradient (I := I) HasMetric.metric f) y :=
    fun y => gradF.smoothAt y
  have hg_smooth : ContMDiff I 𝓘(ℝ, ℝ) ∞ ((‖grad_g[I] f‖²_g) : M → ℝ) := by
    show ContMDiff I 𝓘(ℝ, ℝ) ∞
      (fun y => metricInner y (manifoldGradient (I := I) HasMetric.metric f y)
                                (manifoldGradient (I := I) HasMetric.metric f y))
    exact metricInner_contMDiff h_grad h_grad
  have h_grad_g_cmd : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
              (fun y => (⟨y, manifoldGradient (I := I) HasMetric.metric (‖grad_g[I] f‖²_g) y⟩
                        : TangentBundle I M)) :=
    Riemannian.manifoldGradient_smooth_of_smooth HasMetric.metric (‖grad_g[I] f‖²_g) hg_smooth
  have h_grad_g_smoothAt :
      TangentSmoothAt (manifoldGradient (I := I) HasMetric.metric (‖grad_g[I] f‖²_g)) x :=
    TangentSmoothAt.mk
      (h_grad_g_cmd.mdifferentiableAt (by simp : (∞ : ℕ∞ω) ≠ 0))
  have hBsm : TangentSmoothAt B.toFun x := B.smoothAt x
  -- C² gap on ∇f: smoothness of `y ↦ covDerivAt HasMetric.metric ∇f y (B y)` at x
  -- (smooth-direction case via `leviCivitaConnection_smoothAt_smoothVF_dir`).
  have hBnf_smooth : TangentSmoothAt
      (fun y : M =>
        (leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
          (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y)) x :=
  ...
  have h_bridge := Riemannian.Operators.hessian_eq_mDirDeriv_iterate_sub_chris HasMetric.metric
  ...
  have h_inner_eq :
      (fun y : M => mDirDeriv (I := I) (‖grad_g[I] f‖²_g) y (B.toFun y))
        = (fun y : M => 2 * metricInner y
                    (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
                    (manifoldGradient (I := I) HasMetric.metric f y)) := by
  ...
  have h_chris_term : mDirDeriv (I := I) (‖grad_g[I] f‖²_g) x
        ...
        = 2 * metricInner x
              (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x
                (covDeriv HasMetric.metric B.toFun B.toFun x))
              (manifoldGradient (I := I) HasMetric.metric f x) :=
    mfderiv_gradientNormSq_apply f x (covDeriv HasMetric.metric B.toFun B.toFun x)
      (h_grad_smoothAt x)
  ...
  have h_pull_two :
      mDirDeriv (I := I)
          (fun y : M => 2 * metricInner y
                  (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
                  (manifoldGradient (I := I) HasMetric.metric f y)) x (B.toFun x)
        = 2 * mDirDeriv (I := I)
              (fun y : M => metricInner y
                  (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
                  (manifoldGradient (I := I) HasMetric.metric f y)) x (B.toFun x) := by
    show mfderiv I 𝓘(ℝ, ℝ)
        (fun y : M => 2 * metricInner y
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
            (manifoldGradient (I := I) HasMetric.metric f y)) x (B.toFun x) = _
  ...
  -- Level-2 metric-compat on (B, ∇_B ∇f, ∇f) at x; converts to covDerivAt HasMetric.metric form.
  have h_compat2 := leviCivitaConnection_metric_compatible HasMetric.metric
    B.toFun
    (fun y : M =>
      (leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
        (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
    (manifoldGradient (I := I) HasMetric.metric f) x hBsm hBnf_smooth (h_grad_smoothAt x)
  change mDirDeriv (I := I)
      (fun y : M => metricInner y
        (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
        (manifoldGradient (I := I) HasMetric.metric f y)) x (B.toFun x)
      = metricInner x
          (covDerivAt HasMetric.metric
            (fun y : M =>
              covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (B.toFun y))
            x (B.toFun x))
          (manifoldGradient (I := I) HasMetric.metric f x)
        + metricInner x
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x (B.toFun x))
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x (B.toFun x))
        at h_compat2
  ...
  have h_secondCDS :
    ...
    rfl
  -- `covDeriv HasMetric.metric B.toFun ∇f x = covDerivAt HasMetric.metric ∇f x (B x)` (def).
  have h_covDeriv_eq :
      covDeriv HasMetric.metric B.toFun (manifoldGradient (I := I) HasMetric.metric f) x
        = covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x (B.toFun x) := rfl
...
theorem hessian_gradientNormSq_apply_chartFrame
    ...
    (h_grad : ContMDiff I (I.prod 𝓘(ℝ, E)) ∞
              (fun y => (⟨y, manifoldGradient (I := I) HasMetric.metric f y⟩ : TangentBundle I M))) :
    hessian (I := I) (M := M) HasMetric.metric (‖grad_g[I] f‖²_g)
        (fun _ : M => (v : TangentSpace I x))
        (fun _ : M => (v : TangentSpace I x)) x
    = 2 * (metricInner x
            (secondCovDerivAt (I := I) (M := M) HasMetric.metric              (manifoldGradient (I := I) HasMetric.metric f) x v v)
            (manifoldGradient (I := I) HasMetric.metric f x)
         + metricInner x
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x v)
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x v)) := by
  let gradSV : SmoothVectorField I M := ⟨manifoldGradient (I := I) HasMetric.metric f, h_grad⟩
  have h_grad_smoothAt : ∀ y, TangentSmoothAt (manifoldGradient (I := I) HasMetric.metric f) y :=
  ...
  -- C² gap on ∇f: smoothness of `y ↦ covDerivAt HasMetric.metric ∇f y v` at x.
  have hSvsm : TangentSmoothAt
      (fun y : M =>
        (leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
          (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x)) x :=
    leviCivitaConnection_smoothAt_smoothVF_dir HasMetric.metric
  ...
  have h_inner_eq :
      (fun y : M => mDirDeriv (I := I) (‖grad_g[I] f‖²_g) y (v : TangentSpace I x))
        = (fun y : M => 2 * metricInner y
                    (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
                    (manifoldGradient (I := I) HasMetric.metric f y)) := by
    funext y; exact mfderiv_gradientNormSq_apply f y v (h_grad_smoothAt y)
  rw [h_inner_eq] at h_bridge
  have h_chris_term : mDirDeriv (I := I) (‖grad_g[I] f‖²_g) x
              (covDeriv HasMetric.metric (fun _ : M => (v : TangentSpace I x))
                        (fun _ : M => (v : TangentSpace I x)) x)
        = 2 * metricInner x
              (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x
                (covDeriv HasMetric.metric (fun _ : M => (v : TangentSpace I x))
  ...
  have h_pull_two :
      mDirDeriv (I := I)
          (fun y : M => 2 * metricInner y
                  (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
                  (manifoldGradient (I := I) HasMetric.metric f y)) x v
        = 2 * mDirDeriv (I := I)
              (fun y : M => metricInner y
                  (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
                  (manifoldGradient (I := I) HasMetric.metric f y)) x v := by
    show mfderiv I 𝓘(ℝ, ℝ)
        (fun y : M => 2 * metricInner y
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
            (manifoldGradient (I := I) HasMetric.metric f y)) x v = _
    have h_smul : (fun y : M => 2 * metricInner y
              (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
              (manifoldGradient (I := I) HasMetric.metric f y))
            = (2 : ℝ) • (fun y : M => metricInner y
    ...
    rfl
  rw [h_pull_two] at h_bridge
  -- Level-2 metric-compat on (const v, S_v, ∇f); convert lcc.toFun to covDerivAt HasMetric.metric
  -- (def-eq) at h_compat2 to match h_bridge's pattern.
  have h_compat2 := leviCivitaConnection_metric_compatible HasMetric.metric
    (fun _ : M => (v : TangentSpace I x))
    (fun y : M =>
      (leviCivitaConnection (I := I) (M := M) HasMetric.metric).toFun
        (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
    (manifoldGradient (I := I) HasMetric.metric f) x hVsm hSvsm (h_grad_smoothAt x)
  change mDirDeriv (I := I)
      (fun y : M => metricInner y
        (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x))
        (manifoldGradient (I := I) HasMetric.metric f y)) x v
      = metricInner x
          (covDerivAt HasMetric.metric
            (fun y : M =>
              covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) y (v : TangentSpace I x)) x
            (v : TangentSpace I x))
          (manifoldGradient (I := I) HasMetric.metric f x)
        + metricInner x
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x (v : TangentSpace I x))
            (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x (v : TangentSpace I x))
        at h_compat2
  rw [h_compat2] at h_bridge
  -- Normalize `(∇[const v] const v) x` notation to `covDerivAt HasMetric.metric (const v) x v` (def-eq)
  -- to match the `secondCovDerivAt_def` unfolding.
  change hessian (I := I) (M := M) HasMetric.metric (‖grad_g[I] f‖²_g)
        (fun _ : M => (v : TangentSpace I x))
        (fun _ : M => (v : TangentSpace I x)) x =
      2 * (metricInner x
              (covDerivAt HasMetric.metric
        ...
        - 2 * metricInner x
              (covDerivAt HasMetric.metric (manifoldGradient (I := I) HasMetric.metric f) x
                (covDerivAt HasMetric.metric (fun _ : M => (v : TangentSpace I x)) x
                  (v : TangentSpace I x)))
              (manifoldGradient (I := I) HasMetric.metric f x) at h_bridge
  rw [h_bridge]
  have h_secondCD :
      metricInner x
          (secondCovDerivAt (I := I) (M := M) HasMetric.metric            (manifoldGradient (I := I) HasMetric.metric f) x v v)
