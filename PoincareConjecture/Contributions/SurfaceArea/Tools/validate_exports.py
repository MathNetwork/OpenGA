...
import json
import subprocess
from export_surface_area import DIRECTORY, REPOSITORY, digest
...
def main():
    ...
    if len(rows[0])!=8 or rows[0]!=rows[1]:
        raise RuntimeError('Export changed an elaborated type or definition body')
    if 'All 8 definitions and proofs passed the axiom audit.' not in results[2]:
    ...
    record['validation']={'status':'passed','checked_at':datetime.now(timezone.utc).isoformat(),
      'exact_types_and_definition_bodies':8,'solution_types':'both exact matches',
      'axioms':'All eight source/export declarations use only propext, Classical.choice and Quot.sound.',
      'staged_build':'passed, Mathlib and published metric definition only',
    ...
    print('Validated one definition bundle and both exact surface-area proofs.')
