...
theorem solution
    ...
    (hjumps : ∀ i, i + 1 < n →
      widths (i + 1) (times (i + 1)) ≤ widths i (times (i + 1))) :
    times n ≤ (C ^ ((1 : ℝ) / 4) + W₀ / (16 * Real.pi * C ^ ((3 : ℝ) / 4))) ^ (4 : ℕ) - C := by
  exact OpenGA.le_widthExtinctionTime_across_downward_jumps hC n hn times widths
    hzero htimes hcont hinitial hfinal hslope hjumps
