...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_MetricFamilyRegularAt_of_chartGram_timeDeriv
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.MetricFamilyRegularAt.of_chartGram_timeDeriv | throwError "Missing target"
