...
import Solutions.Sol_OpenGA_RicciFlow_traceTimeDeriv_inducedMetric
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `OpenGA.RicciFlow.traceTimeDeriv_inducedMetric | throwError "Missing target"
