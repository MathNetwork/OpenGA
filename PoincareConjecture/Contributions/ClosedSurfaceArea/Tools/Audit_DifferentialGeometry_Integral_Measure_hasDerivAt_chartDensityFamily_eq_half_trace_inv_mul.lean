...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_hasDerivAt_chartDensityFamily_eq_half_trace_inv_mul
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.hasDerivAt_chartDensityFamily_eq_half_trace_inv_mul | throwError "Missing target"
