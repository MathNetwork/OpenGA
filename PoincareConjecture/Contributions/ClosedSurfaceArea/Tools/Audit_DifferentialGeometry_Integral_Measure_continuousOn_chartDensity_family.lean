...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_continuousOn_chartDensity_family
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.continuousOn_chartDensity_family | throwError "Missing target"
