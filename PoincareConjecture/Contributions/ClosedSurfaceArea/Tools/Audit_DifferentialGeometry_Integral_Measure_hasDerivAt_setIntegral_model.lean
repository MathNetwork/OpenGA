...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_hasDerivAt_setIntegral_model
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.hasDerivAt_setIntegral_model | throwError "Missing target"
