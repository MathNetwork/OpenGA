...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_FunctionRegularAt_const
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.FunctionRegularAt_const | throwError "Missing target"
