...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_chartGramMatrix_pullback_eq_sum
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.chartGramMatrix_pullback_eq_sum | throwError "Missing target"
