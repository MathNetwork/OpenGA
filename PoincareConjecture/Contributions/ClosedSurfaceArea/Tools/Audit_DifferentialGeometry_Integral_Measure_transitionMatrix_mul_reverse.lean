...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_transitionMatrix_mul_reverse
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.transitionMatrix_mul_reverse | throwError "Missing target"
