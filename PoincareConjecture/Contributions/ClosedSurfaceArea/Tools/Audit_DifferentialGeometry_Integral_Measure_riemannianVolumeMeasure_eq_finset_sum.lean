...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_riemannianVolumeMeasure_eq_finset_sum
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.riemannianVolumeMeasure_eq_finset_sum | throwError "Missing target"
