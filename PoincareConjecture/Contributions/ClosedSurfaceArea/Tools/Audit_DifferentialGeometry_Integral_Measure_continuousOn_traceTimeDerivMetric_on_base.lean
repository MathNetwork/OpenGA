...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_continuousOn_traceTimeDerivMetric_on_base
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.continuousOn_traceTimeDerivMetric_on_base | throwError "Missing target"
