...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_chartBasisVec_contMDiffOn
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.chartBasisVec_contMDiffOn | throwError "Missing target"
