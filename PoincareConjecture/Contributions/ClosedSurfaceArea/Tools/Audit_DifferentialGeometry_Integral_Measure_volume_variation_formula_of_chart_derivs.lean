import Theorems.Thm_DifferentialGeometry_Integral_Measure_volume_variation_formula_of_chart_derivs
import Solutions.Sol_DifferentialGeometry_Integral_Measure_volume_variation_formula_of_chart_derivs
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.volume_variation_formula_of_chart_derivs | throwError "Missing target"
