...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_perm_sum_eq_trace_adjugate_mul
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.perm_sum_eq_trace_adjugate_mul | throwError "Missing target"
