...
import Solutions.Sol_OpenGA_RicciFlow_inducedMetric_regularOn
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `OpenGA.RicciFlow.inducedMetric_regularOn | throwError "Missing target"
