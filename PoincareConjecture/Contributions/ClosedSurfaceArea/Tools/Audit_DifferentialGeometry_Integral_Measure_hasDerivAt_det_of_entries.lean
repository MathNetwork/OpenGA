...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_hasDerivAt_det_of_entries
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.hasDerivAt_det_of_entries | throwError "Missing target"
