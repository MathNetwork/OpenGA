...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_per_chart_integrand_hasDerivAt
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.per_chart_integrand_hasDerivAt | throwError "Missing target"
