...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_integral_chart_ae
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.integral_chart_ae | throwError "Missing target"
