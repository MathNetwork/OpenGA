...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_chartLocalMeasure_lintegral
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.chartLocalMeasure_lintegral | throwError "Missing target"
