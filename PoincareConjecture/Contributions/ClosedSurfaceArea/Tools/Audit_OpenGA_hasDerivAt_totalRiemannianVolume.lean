...
import Solutions.Sol_OpenGA_hasDerivAt_totalRiemannianVolume
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `OpenGA.hasDerivAt_totalRiemannianVolume | throwError "Missing target"
