...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_chartBasisVecFiber_pullback
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.chartBasisVecFiber_pullback | throwError "Missing target"
