...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_chartGramMatrix_dotProduct_mulVec
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.chartGramMatrix_dotProduct_mulVec | throwError "Missing target"
