...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_aemeasurable_chartDensity_symm_pullback
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.aemeasurable_chartDensity_symm_pullback | throwError "Missing target"
