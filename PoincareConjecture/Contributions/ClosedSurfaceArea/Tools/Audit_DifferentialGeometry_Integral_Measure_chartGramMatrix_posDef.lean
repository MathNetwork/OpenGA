...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_chartGramMatrix_posDef
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.chartGramMatrix_posDef | throwError "Missing target"
