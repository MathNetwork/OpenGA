...
unsafe def main (args : List String) : IO UInt32 := do
  ...
  let cmdState := Command.mkState env messages (({} : Options).set `synthInstance.maxHeartbeats (400000 : Nat) |>.set `maxHeartbeats (8000000 : Nat) |>.set `maxRecDepth (4000 : Nat))
  ...
  if let some start := header.raw.getPos? then
    if let some finish := header.raw.getTailPos? then
      IO.println (Json.mkObj [("kind", Json.str "header"),
        ("start", posJson fm start), ("end", posJson fm finish)]).compress
  -- Preserve exact command ranges for namespace/section and header handling.
  for cmd in s.commands do
    let some start := cmd.getPos? | continue
    let some finish := cmd.getTailPos? | continue
  ...
  for cmd in s.commands do
    ...
    let fields := [("kind", Json.str "decl"),
                   ...
                   ("coreDecl", rangeJson fm declaration?),
