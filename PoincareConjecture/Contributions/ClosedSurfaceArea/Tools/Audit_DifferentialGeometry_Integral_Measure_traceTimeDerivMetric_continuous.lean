...
import Solutions.Sol_DifferentialGeometry_Integral_Measure_traceTimeDerivMetric_continuous
import Lean
open Lean in
run_meta do
  let env ← getEnv
  let some target := env.find? `DifferentialGeometry.Integral.Measure.traceTimeDerivMetric_continuous | throwError "Missing target"
