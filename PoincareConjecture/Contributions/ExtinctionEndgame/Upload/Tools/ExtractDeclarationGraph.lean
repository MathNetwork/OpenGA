...
run_meta do
  ...
  let projPrefix := `OpenGALib
  let isProj : Name → Bool := fun m => projPrefix.isPrefixOf m || m == `OpenProblems || m == `Reduction
  ...
  IO.FS.writeFile "Upload/Metadata/declaration_graph.jsonl" (String.intercalate "\n" lines.toList)
