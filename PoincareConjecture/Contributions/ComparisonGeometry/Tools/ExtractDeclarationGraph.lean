...
run_meta do
  ...
  let projPrefixes := [`OpenGALib, `DifferentialGeometry]
  let isProj : Name → Bool := fun m => projPrefixes.any (·.isPrefixOf m)
  ...
  IO.FS.writeFile "PoincareConjecture/Contributions/ComparisonGeometry/Metadata/declaration_graph.jsonl" (String.intercalate "\n" lines.toList)
