...
We use the notation from Chapter~\ref{ch:sweepouts}. Recall that a sweepout $\Phi$ of $M$ is a continuous family of hypersurfaces $\{\Phi(x) = \partial\Omega(x)\}_{x \in [0,1]}$ with $\Omega(0) = \emptyset$ and $\Omega(1) = M$ (Definition~\ref{def:sweepout}). We denote $\mathcal{F}(\Phi) := \sup_{x\in [0,1]} \mathbf{M}(\Phi(x))$ for the height of a sweepout (Definition~\ref{def:height}), and the width is $W = \inf_{\Phi \in \mathcal{S}} \mathcal{F}(\Phi)$.
...
$\Omega_i$ (see Section~\ref{sec:caccioppoli}). The
...
\[
    \mathbf{M}(\phi(t^*)) = W.
\]
Since the sweepout $\phi$ is continuous in the flat norm topology (see Chapter~\ref{ch:sweepouts}), and flat convergence implies $L^1$ convergence of characteristic functions, we have $\mathbb{1}_{\Omega_i} \to \mathbb{1}_{\Omega(t^*)}$ in $L^1(M)$. By uniqueness of BV limits, the Caccioppoli set $\Omega$ from Step~1 equals $\Omega(t^*)$.
