...
\begin{remark}[Role of the doubled manifold]\label{rem:double-nested}
The doubled manifold $\widetilde{M}$ is used in this chapter solely for the construction of Morse functions whose level sets give nested sweepouts: the doubling provides a closed ambient space in which standard Morse theory applies, and the Morse function can be chosen tangent to $\partial M$ in a collar neighbourhood. The min-max theory itself (pull-tight, almost minimizing, regularity) is carried out on $M$ using the relative cycle framework of Li--Zhou~\cite{LZ17}.
\end{remark}

\section{Step 1: Preliminary Modification}
Fix $\br{\Gamma_t} \in \mathcal{S}(U)$ with the corresponding open sets $\br{\Omega_t}$ with $\mathcal{F}(\br{\Gamma_t}) \leq A$. We apply the preliminary modification as in \cite[Lemma 6.2]{CL20} to $\br{\Gamma_t}$. The only modification is that (1.2) measures area intersecting $\mathrm{int}(M)$. 
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item[(B)] $\Omega_{t_{i + 1}}' \subset
             \Omega_{t_i}'$ and there exists a Morse function $g_i: cl(\Omega_{t_i}\backslash\Omega_{t_{i + 1}}) \to [t_i, t_{i + 1}]$, such that $\Gamma_t' = g_i^{-1}(t)$ and $\Omega_t' = \Omega_{t_i}'\cup g_i^{-1}((-\infty, t])$ for $t \in [t_i, t_{i + 1}]$. 
        \end{enumerate}
    \end{enumerate}
    \begin{proof}
...
\begin{enumerate}
    ...
    \begin{equation*}
        \haus^n(\Gamma_t^i\cap M) \leq \max\br{\haus^n(\Gamma_{t_i}\cap M), \haus^n(\Gamma_{t_{i+1}}\cap M)} + \eps.
    \end{equation*}
\end{enumerate}
