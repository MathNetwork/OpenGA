...
\begin{proof}
Choose $r>0$ so that $B_r(x)\Subset M$. \xinze{which one?} Local isoperimetric inequality provides a constant
\[
...
\]
Set $U:=B_R(x)\cap M$.  Its relative boundary equals $\partial B_R(x)\cap M$, so $\haus^{n}(\partial_{\mathrm{rel}}U)\le C_I/100$.
...
\begin{proof}
The argument is the same as in \cite[Lemma~8.8]{CL20}.  Admissible vector fields are taken tangent to $\partial M$ by averaging over the reflection in the doubled manifold.  The compactness of the sets
\[
...
Given $t_0-\eta<b<b'<a'<a<t_0+\eta$, choose $a''\in(b',a')$ and $b''\in(b',b'')$, and fix:
\begin{itemize}
...
Let $U\subset M$ be a good set.  For every $\delta>0$ there exists a smooth free boundary minimal hypersurface $\Sigma$ in $M$ such that
\begin{align*}
    \haus^n(\Sigma\cap M) &\le W_\partial(U)+\haus^n(\partial_{\mathrm{rel}}U),\\
...
Combining Lemma~\ref{lem:good-set-sublinear}, Proposition~\ref{prop:non-escape}, Proposition~\ref{prop:regular-limit}, and Theorem~\ref{thm:main-sup}, we obtain the free boundary minimal hypersurface asserted by Theorem~\ref{thm:}\xinze{where is the main theorem???}.  This completes the proof of the main existence result.
