...
The construction of nested sweepouts is crucial for our work. As discussed in \cite[§6]{CL20}, nested families allow us to glue local deformations while keeping precise control over the maximal area of the sweepout. A family $\{\Gamma_t\}_{t\in[0,1]}$ is \emph{nested} if it is given by the level sets of a Morse function, so that the associated regions $\Omega_t = f^{-1}((-\infty,t))$ vary by elementary topological moves (handle attachments or deletions) as $t$ increases.

The main result of this chapter, Proposition~\ref{prop:nested-sweepout}, shows that any sweepout of a bounded relative open set $U$ can be approximated by a nested sweepout with arbitrarily small area increase. The proof follows the strategy of \cite[Lemma~6.2]{CL20}, adapted to the free-boundary setting, and proceeds in three main steps:
...
functions whose level sets have controlled area.
...
Fix $\br{\Gamma_t} \in \mathcal{S}(U)$ with the corresponding open sets $\br{\Omega_t}$ with $\mathcal{F}(\br{\Gamma_t}) \leq A$. We apply the preliminary modification as in \cite[Lemma 6.2]{CL20} to $\br{\Gamma_t}$. The only thing we should be careful with is (1.2) of \ref{lem:preliminary-modification} as in our setting the function measures the area of the sweepout intersecting with $\text{int}(M)$. 
\begin{lemma}[Preliminary Modification]\label{lem:preliminary-modification}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item[(A)] $\Omega_{t_i}' \subset \Omega_{t_{i + 1}}'$ and there exists a Morse function $g_i: cl(\Omega_{t_{i + 1}}\backslash \Omega_{t_i}) \to [t_i, t_{i + 1}]$, such that $\Gamma_t' = g_i^{-1}(t)$ and $\Omega_t' = \Omega_{t_i}'\cup g_i^{-1}((-\infty, t))$ for $t \in [t_i, t_{i + 1}]$. 
            \item[(B)] $\Omega_{t_{i + 1}}' \subset \Omega_{t_i}'$ and there exists a Morse function $g_i: cl(\Omega_{t_i}\backslash\Omega_{t_{i + 1}}) \to [t_i, t_{i + 1}]$, such that $\Gamma_t' = g_i^{-1}(t)$ and $\Omega_t' = \Omega_{t_i}'\cup g_i^{-1}((-\infty, t])$ for $t \in [t_i, t_{i + 1}]$. 
        \end{enumerate}
    \end{enumerate}
    \begin{proof}
        Let $M'$ be the compact subset of $\Tilde{M}$ that contains the closure of $\Omega_t$ for all $t \in [0, 1]$.
        ...
        \begin{enumerate}
            \item[(i)] $B$ is $(1 + L_{\mathrm{ball}})$-bilipschitz diffeomorphic to the Euclidean ball of the same radius;
            \item[(ii)] $\haus^n(B\cap \Gamma_t) < \frac{\eps}{20}$ for $t \in [0, 1]$.
        \end{enumerate}
        Condition (ii) can be realized because of the no concentration of mass property (s4)
        ...
        in $\mathbb R^{n+1}_+$. All local deformations in such $B_i$ are generated by compactly 
        supported vector fields tangent to $\partial M$ along $\partial M$ 
        (or, equivalently, we perform the modification in the doubled ambient $\widetilde M$ and then 
        intersect back with $M$). This preserves the free-boundary condition and keeps the same 
        ...
        \begin{enumerate}
            ...
            &\haus^{n + 1}(B_i\cap(\Omega_{s_j}\backslash\Omega_{s_{j + 1}})) + \haus^{n + 1}(B_i\cap(\Omega_{s_{j + 1}}\backslash\Omega_{s_j})) \\
            &< \min\br{\frac{r\eps}{10k}, \left(\frac{\eps}{10}\right)^{\frac{n + 1}{n}}};
            \end{align*}
            \item[(iv)] $\abs{\haus^n(B_i\cap \Gamma_{s_j}) - \haus^n(B_i\cap \Gamma_{s_{j + 1}})} \leq \frac{\eps}{10k}$
        \end{enumerate}
        ...
        \end{equation*}
        By the \emph{relative} coarea formula~\ref{thm:coarea-formula} and (iii), we may assume $B_i'$ satisfies
        \begin{equation}\label{eq:coarea-small-boundary-ball}
            \haus^n(\partial B_i'\cap (U_j^1\cup U_j^2)) \leq \frac{\eps}{4k}.
        \end{equation}
        ...
        If $\Omega_{s_j^l}'$ is empty we set $\Gamma_{s_j^l}'$ to be a point inside $\Omega_{s_j^{l - 1}}'$. By properties (iii) and (iv) surface $\Gamma_{s_j^l}'$ will satisfy the desired upper bound on the area.
        ...
        \end{itemize} 
        In both cases we want the homotopies to satisfy the desired upper bound on the areas.

        Consider the set $\Omega_{s_j^{2i - 2}}'\backslash\Omega_{s_j^{2i - 1}}' = B_i'\cap U_j^1$. After smoothing the corner we call this set $U$. We map $B_i'$ to $\R^{n + 1}$ by a $(1 + L_{\mathrm{ball}})$-bilipschitz diffeomorphism. Existence of the desired nested families follows by Lemma ~\ref{lem:existence-nested-between-sets}. The upper bound for the area follows from the upper bound for the area at the endpoints and properties $(i)$ and $(ii)$. 
        \begin{equation*}
            \begin{split}
                \haus^n(\Gamma_t'\cap M) \leq& \haus^n(\partial\Omega_{s_j^{2i - 1}}'\cap M) + \haus^n(\partial(\Omega_{s_j^{2i - 2}}\backslash\Omega_{s_j^{2i - 1}})\cap M)\\
                &+ C(n)(1 + L_{\mathrm{ball}})^n\haus^{n + 1}(\Omega_{s_j^{2i - 2}}'\backslash\Omega_{s_j^{2i - 1}}') + \eps\\
                \leq& \haus^n(\Gamma_{s_j}\cap M) + \eps/2 + \eps/10 + \eps/4k\\
                &+ C(n)(1 + L_{\mathrm{ball}})^n\min\br{\frac{r\eps}{10k}, (\frac{\eps}{10})^{\frac{n + 1}{n}}}\\
                \leq& \haus^n(\Gamma_{s_j}\cap M) + \eps
            \end{split}
        \end{equation*}
    \end{proof}
\end{lemma}
...
Having subdivided the original sweepout into monotone intervals in Step 1, we now construct nested
families on each interval $[t_i, t_{i+1}]$ with controlled area. The key idea is to push the
boundaries $\Omega_{t_i}$ and $\Omega_{t_{i+1}}$ slightly inward or outward to ensure that the
resulting regions can be connected by a Morse function whose level sets have bounded area.
...
\begin{enumerate}
    ...
    \item[(2.2)] $\max\br{\haus^n(\partial_{rel}\Omega_0^i), \haus^n(\partial_{rel}\Omega_1^i))}$ \\
    $\leq \max\br{\haus^n(\Gamma_{t_i}\cap M), \haus^n(\Gamma_{t_{i + 1}}\cap M)} + \eps$;
    ...
    \begin{equation*}
        \haus^n(\Gamma_t^i\cap M) \leq \max\br{\haus^n(\Gamma_{t_i}\cap M), \haus^n(\Gamma_{t_i}\cap M)} + \eps.
    \end{equation*}
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{equation*}
        \Omega_0^i = \Omega_{t_{i + 1}}\backslash cl(N_\delta(\partial\Omega_{t_{i + 1}}))
    \end{equation*}
    ...
    \begin{equation*}
        \haus^n((\partial\Omega_{t_{i + 1}}\times \br{s})\cap M) \leq \haus^n(\partial_{rel}\Omega_{t_{i + 1}}) + \eps/2.
    \end{equation*}
...
\begin{proposition}\label{prop:gluing-two-nested}
    Suppose $\br{\Gamma_t^a}$ and $\br{\Gamma_t^b}$ are two nested families (with corresponding families of open sets $\br{\Omega_t^a}$ and $\br{\Omega_t^b}$ respectively) and $\haus^n(\Gamma_t^i\cap M) \leq W$. Suppose moreover that $\Omega_0^b \subset \Omega_1^a$. For any $\eps > 0$ there exists a nested family $\br{\Gamma_t}$ and a corresponding family of open sets $\br{\Omega_t}$ such that
    \begin{equation*}
        \haus^n(\Gamma_t\cap M) \leq W + \eps,
    \end{equation*}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            A = \inf_{\Omega'\in\mathcal{S}}\haus^n(\partial_{rel}\Omega')
        \end{equation*}
        and choose $\Omega\in \mathcal{S}$ with
        \begin{equation*}
            \haus^n(\partial_{rel}\Omega) < A + \frac{\eps}{4}.
        \end{equation*}
        We set $\alpha = \partial\Omega$. We claim that $\Omega$ and $\alpha$ satisfy properties (i) and (ii) from Lemma~\ref{lem:extension}I for $\Omega_t = \Omega_t^a$. Indeed if $\Omega'$ satisfies $\Omega \subset \Omega' \subset \Omega_1$, we have
        \begin{equation*}
        \begin{split}
            \haus^n(\alpha\cap M) <& A + \frac{\eps}{4}\\
            \leq& \haus^n(\partial_{rel}\Omega') + \frac{\eps}{4}
        \end{split}
        \end{equation*}
        ...
        \begin{equation*}
            \haus^n(\Tilde{\Gamma}_t^a) \leq W + \eps.
        \end{equation*}
        On the other hand, $\Omega$ and $\alpha$ also satisfies properties $(1)'$ and $(2)'$ from Lemma~\ref{lem:extension}II for $\Omega_t = \Omega_t^b$. Indeed, for any open set $\Omega'$ with $\Omega_0^b \subset \Omega' \subset \Omega$ then again we have $\Omega_0^b \subset \Omega$ and 
        \begin{equation*}
            \haus^n(\alpha\cap M) < \haus^n(\partial_{rel}\Omega') + \frac{\eps}{4}
        \end{equation*}
        ...
        \begin{equation*}
            \haus^n(\Tilde{\Gamma}_t^b) \leq W + \eps.
        \end{equation*}
        We define the desired nested family $\Gamma_t$ simply by concatenating these two nested families.
    \end{proof}
\end{proposition}
Now we are ready the complete the proof of Proposition~\ref{prop:nested-sweepout}. We apply local monotonization~\ref{sec:local-monotonization} to define families $\br{\Gamma_t^i}$ for $i = 1, \dots, N - 1$. 
...
After iteratively preforming this operation $N$ times we obtain the desired nested family. This finishes the proof of Proposition~\ref{prop:nested-sweepout}.
