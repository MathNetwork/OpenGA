...
An alternative approach to min--max theory, developed by Chodosh--Liokumovich--Spolaor~\cite{CLS22}, is based on the notion of \emph{non-excessive sweepouts}. This framework provides a conceptually simpler variational characterization of critical sweepouts without requiring the pull-tight procedure central to the classical Almgren--Pitts method.

\subsection{Sweepouts and Width}

\begin{definition}[Sweepout]\label{def:sweepout-cls}
A \emph{sweepout} of $M$ is a map $\Phi: [0,1] \to \mathbf{Z}_{n}(M; \mathbb{Z}_2)$ continuous in the flat topology such that $\Phi(x) = \partial \Omega(x)$, where $\{\Omega(x) : x\in [0,1]\}$ is a family of Caccioppoli sets with $\Omega(0) = \emptyset$ and $\Omega(1) = M$. We denote by $\mathcal{S}$ the collection of all such sweepouts.

The \emph{width} is defined by
\[
W = \inf_{\Phi \in \mathcal{S}} \sup_{x\in [0,1]} \mathbf{M}(\Phi(x)).
\]
\end{definition}

We will denote $\mathbf{w}(\Phi) := \sup_{x\in [0,1]} \mathbf{M}(\Phi(x))$ for the width of a sweepout $\Phi$, and switch freely between the notation $\mathbf{M}(\Phi(x))$ and $\per(\Omega(x))$.
...
\begin{itemize}
    \item \emph{optimal} if $\mathbf{w}(\Phi) = W$;
...
\begin{proposition}[Stationary varifolds from non-excessive sweepouts]\label{thm:CLS-stationary}
Let $(M^{n+1},g)$ be a closed Riemannian manifold with $2\leq n\leq 6$, and let $\Phi$ be a non-excessive (ONVP) sweepout with width $W = \mathbf{w}(\Phi)$. Then there exists a stationary $n$-varifold $V$ in $M$ with $\mathbf{M}(V) \leq W$.
\end{proposition}
...
\begin{itemize}
    ...
    \item In the non-excessive framework, one works with varifolds, and integrality must be established separately (see Section~\ref{sec:integrality-no-cancellation}).
    \item Both approaches use pull-tight to obtain stationarity and regularity, but pull-tight alone does not produce integrality.
\end{itemize}
\end{remark}
...
approximated by one-sided area minimizers in any neighborhood. Such points typically arise at branch
points or other geometric singularities where local deformations can reduce area.
...
associated to slices $T_i = \phi(t_i)$ with $\mathbf{M}(T_i) \to \mathbf{w}(\phi)$.

We identify each slice $T_i \in \mathbf{Z}_n(M;\mathbb{Z}_2)$ with its corresponding Caccioppoli set
$\Omega_i$ via Theorem~\ref{thm:caccioppoli-cycles} (see Section~\ref{subsec:caccioppoli-sets}). The
...
\textbf{Step 2: Perimeter convergence.} The key observation is that we have perimeter convergence:
\[
    \per(\Omega_i, M) = \mathbf{M}(V_i) \to \mathbf{M}(V) = \per(\Omega, M).
\]
This follows from the fact that $\mathbf{M}(T_i) = \per(\Omega_i, M)$ (by the identification of
currents and Caccioppoli sets), and $\mathbf{M}(T_i) \to \mathbf{w}(\phi) = \mathbf{M}(V)$ by
construction. Moreover, by lower semicontinuity of perimeter,
\[
    \per(\Omega, M) \leq \liminf_{i\to\infty} \per(\Omega_i, M) = \mathbf{M}(V).
\]
On the other hand, the varifold $V$ has mass $\mathbf{M}(V)$, and if $V$ is the varifold induced by
$\bdy\Omega$, then $\mathbf{M}(V) = \per(\Omega, M)$. Therefore, perimeter convergence holds:
\[
    \per(\Omega_i, M) \to \per(\Omega, M).
\]
...
\begin{remark}
\xinze{Modified: Replaced detailed red questions with reference to fundamental obstruction in Chapter 2.}
The fundamental obstruction to proving Conjecture~\ref{conj:mass-cancellation-integrality} is explained in Remark~\ref{rem:index-requires-width} (Chapter~\ref{ch:examples}): when $\mathbf{M}(V) < W$, we cannot apply the index bound $\mathrm{Index}(V) \leq 1$ from min-max, which prevents us from using Wickramasekera's regularity theory to establish integrality. This represents a genuine obstruction rather than merely a technical gap.
\end{remark}
...
\subsection{Wickramasekera's Regularity Theorem}
...
\begin{remark}[Why the case analysis is complete]
\xinze{New remark: Incorporated from previous long red annotation explaining case completeness and stability.}

Since $V = |\partial\Omega|$ in the no mass cancellation case, tangent cones at singular points are constrained by the Caccioppoli set structure. For stationary boundaries of Caccioppoli sets in $\mathbb{R}^{n+1}$ ($n \leq 6$), the only possible singular cones are:
\begin{itemize}
    \item $C = |H_1| + |H_2|$ (two half-hyperplanes meeting at angle $< \pi$) — a "wedge" or "crease"
    \item $C = 2|H|$ (single half-hyperplane with multiplicity 2) — a "fold"
\end{itemize}
Higher multiplicities ($q \geq 3$) or more complex configurations (three or more sheets meeting) would require mass cancellation and are impossible for Caccioppoli boundaries. This explains why Cases 1a and 1b below are exhaustive.

Moreover, stability follows from the index bound: by Theorem~\ref{thm:index-from-minmax} (Chapter~\ref{ch:examples}), the min-max construction gives $\mathrm{Index}(V) \leq 1$. Combined with the deformation arguments below showing at most one unstable direction, we conclude $\mathrm{Index}(V) = 0$ (stability) or $\mathrm{Index}(V) = 1$ (one-sided minimization), both of which lead to regularity via Wickramasekera~\cite{Wic14} or Liu~\cite{Liu19}.
\end{remark}
...
\begin{remark}[Dimension of the singular set]
\xinze{New remark: Condensed from previous red annotation about why sing V has multiple points.}

If the $\alpha$-structural hypothesis fails at $Z \in \sing V$, then the tangent cone has an $(n-1)$-dimensional common boundary $L$ (the "crease" in Case 1a or the "fold line" in Case 1b). Since $V$ approximates this cone structure near $Z$ in the Hausdorff sense, the singular set $\sing V$ must contain an $(n-1)$-dimensional set near $Z$. This ensures that there exist other singular points $X \in \sing V$ near $Z$ where similar deformation arguments apply. Thus, the geometric structure of the tangent cone automatically guarantees that $\sing V$ is not an isolated point but has positive Hausdorff dimension.
\end{remark}

Therefore, the $\alpha$-structural hypothesis must hold.
\end{proof}

\xinze{Modified: Simplified from previous long red annotation, now references Chapter 2 for details.}

By Theorem~\ref{thm:index-from-minmax} (Chapter~\ref{ch:examples}), the min-max construction gives $\text{index}(V) \leq 1$. As explained in Remark~\ref{rem:index-implies-stability}, this index bound holds \emph{a priori} before regularity is established, avoiding any circular reasoning.
...
\xinze{Modified: Converted critical red issue into remark clarifying this section is conditional.}

The representation $V = \sum_i\kappa_i|\Sigma_i|$ with \emph{integer} multiplicities assumes integrality, which is precisely what Conjecture~\ref{conj:mass-cancellation-integrality} seeks to establish. The analysis below should be understood as conditional on this conjecture. The fundamental obstruction to proving integrality in the mass cancellation case is explained in Remark~\ref{rem:index-requires-width}: without $\mathbf{M}(V) = W$, we cannot apply the index bound to deduce regularity.
\end{remark}
...
framework constructed in Chapters~\ref{ch:structure}--\ref{ch:nested-sweepout} provides a continuous
...
By Theorem~\ref{thm:CLS-stationary}, there exists a stationary varifold $V$ with $\mass(V) \leq \mathbf{w}(\Phi)$.
