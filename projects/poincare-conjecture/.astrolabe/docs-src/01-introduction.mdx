...
> diffeomorphic to $S^3$.** (Every topological $3$-manifold admits a
...
This conjecture was formulated by Henri Poincaré [Poincare]
...
in Perelman's three preprints [Perelman1, Perelman2, Perelman3].
...
the Poincaré Conjecture and more general topological
...
prove the Poincaré Conjecture (and \entryref{9999efa2be2c} below
which implies the Poincaré Conjecture). For
a detailed history of the Poincaré Conjecture, see Milnor's survey
article [Milnorsurvey].

i̱gskip
A class of examples closely related to the $3$-sphere are *the
$3$-dimensional spherical space-forms*, i.e.,
the quotients of $S^3$ by free, linear actions of finite subgroups of the
orthogonal group $O(4)$. There is a generalization of the Poincaré
Conjecture, called the **$3$-dimensional spherical space-form
...
$3$-dimensional spherical space-form conjecture is the Poincaré Conjecture.

 As indicated in Remark 1.4 of [Perelman3], the arguments we present
 here not only prove the Poincaré Conjecture, they prove the $3$-dimensional space-form conjecture.
 In fact, the purpose of this book is to prove the following
 more general theorem.

<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="0.1" label="Theorem1" -->
**Theorem.** Let $M$ be a closed, connected $3$-manifold and
...
This immediately implies an affirmative resolution of the Poincaré
Conjecture and of the $3$-dimensional spherical space-form
conjecture.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="0.2" -->
**Corollary.** (a) A closed, simply connected $3$-manifold is diffeomorphic to
...
Poincaré Conjecture and the more general $3$-dimensional spherical space-form
...
introduced by Richard Hamilton in [Hamilton3MPRC]:

$$
\frac{\partial g(t)}{\partial t}=-2\mathit{Ric}(g(t)),
$$

where $\mathit{Ric}(g(t))$ is the Ricci curvature of the metric $g(t)$.
...
paper [Hamiltonsurvey], the book by Chow-Knopf
[ChowKnopf], or the book by Chow, Lu, and Ni [ChowLuNi].
...
[HamiltonNSRF3M] Hamilton showed that if the Ricci flow exists
...
One is led to study a more general evolution process
 ...
 of four-manifolds, [Hamilton4MPIC].
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="0.3" label="surgery" -->
**Theorem.** Let $(M,g_0)$ be a closed Riemannian $3$-manifold.
...
While \entryref{847af3ecd4a8} is central for all applications of Ricci
flow to the topology of three-dimensional manifolds, the argument
for the $3$-manifolds described in \entryref{9999efa2be2c} is
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="0.4" label="finiteext" -->
**Theorem.** Let $M$ be a closed $3$-manifold whose fundamental group is a free
...
\entryref{7078952cb9e4}.). Let $g_0$ be any Riemannian metric on
...
$(M,g_0)$ as initial metric as described in \entryref{847af3ecd4a8}.
...
given by Perelman in [Perelman3], see also
[ColdingMinicozzi].

We immediately deduce \entryref{9999efa2be2c} from
Theorems \entryref{847af3ecd4a8} and \entryref{127db1dc9315} as follows: Let $M$ be a
$3$-manifold satisfying the hypothesis of \entryref{9999efa2be2c}.
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="0.5" label="corequiv" -->
**Corollary.** Let $(M_0,g_0)$ a connected Riemannian manifold with no locally
...
**Proof.** Repeated application of \entryref{847af3ecd4a8} shows that (1) implies
(2). The implication (2) implies (3) is immediate from van Kampen's
theorem. The fact that (3) implies (1) is \entryref{127db1dc9315}.
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="0.6" -->
**Remark.** (i) The use of the sphere theorem is unnecessary in the above
...
topological conclusion of \entryref{9999efa2be2c} for that component,
...
\entryref{7078952cb9e4} that the manifolds that satisfy the four equivalent
...
Thurston, see [Thurstongeom], known as Thurston's
...
which admits a decomposition along incompressible (I.e.,
...
essential way on \entryref{847af3ecd4a8}, namely the existence of
...
([Perelman2]), following earlier work of Hamilton
([HamiltonNSRF3M]) in a simpler case of bounded curvature. They
also involve a result (Theorem 7.4 from [Perelman2]) from the
theory of manifolds with curvature locally bounded below that are
collapsed, related to results of Shioya-Yamaguchi [SY]. The
Shioya-Yamaguchi results in turn rely on an earlier, unpublished
work of Perelman proving the so-called 'Stability Theorem.'
Recently, Kapovich, [Kap] has put a preprint on the archive
giving a proof of the stability result. We have been examining
another approach, one suggested by Perelman in [Perelman2],
avoiding the stability theorem, cf, [KleinerLott2] and
[MorganTian2]. It is our view that the collapsing results
...
same close scrutiny that the arguments proving the Poincaré
Conjecture have received. This process is underway.

In this book we do not attempt to explicate any of the results
beyond \entryref{847af3ecd4a8} described in the previous paragraph
that are needed for the Geometrization Conjecture. Rather, we
content ourselves with presenting a proof of \entryref{9999efa2be2c}
...
of Chapter 17 we have established \entryref{847af3ecd4a8}. Chapter 18
...
the manifold is *$\kappa$-non-collapsed* at $x$
provided that the following holds: For any $r$ such that the norm of
the Riemannian curvature tensor, $|\mathit{Rm}|$, is $\le r^{-2}$ at
all points of the metric ball, $B(x,r)$, of radius $r$ centered at
$x$, we have $\mathit{Vol} B(x,r)\ge \kappa r^n$. There is a
relationship between this notion and the injectivity
radius of $M$ at $x$. Namely, if $|\mathit{Rm}|\le r^{-2}$ on $B(x,r)$ and if $B(x,r)$ is
...
non-negative sectional curvature. A *soul* is a compact, totally
...
$\epsilon$-neck in $M$, and such that the *core*, ${\mathcal C}\setminus
...
call the cap an $(C,\epsilon)$-cap. See
Fig. 0.1.
 An
...
is proved in the appendix at the end of the book. **We shall fix
...
Hamilton [Hamilton3MPRC] introduced the Ricci flow
equation,
...
$$
\frac{\partial R}{\partial t}(x,t)=\triangle
...
were proved by Hamilton ([Hamilton3MPRC]) using the Nash-Moser inverse
function theorem,
 ([HamiltonIFTNM]). These results are standard for
...
$$
 ...
 In particular, all the metrics in this flow
differ by a constant factor depending on time and the metric is a
decreasing function of time. These are called *shrinking
...
### Controlling higher derivatives of curvature

Now let us discuss the smoothness results for geometric limits. The
general result along these lines is Shi's theorem, see [Shi1, Shi2]. Again, this is a standard type of
...
four-dimensional manifold ${\mathcal M}$ (space-time) with a time function $\mathbf{t}\colon {\mathcal M}\to \Ar$
and a smooth vector field $\chi$. These are required to satisfy:

1. Each $x\in {\mathcal M}$ has a neighborhood of the form
$U\times J$, where $U$ is an open subset in $\Ar^3$ and $J\subset
\Ar$ is an interval, in which $\mathbf{t}$ is the projection onto $J$
...
$\mathbf{t}$. We call $\mathbf{t}^{-1}(t)$ the $t$ time-slice. It is a
smooth $3$-manifold.
1. The image $\mathbf{t}({\mathcal M})$ is a connected interval $I$ in
$\Ar$, possibly infinite. The boundary of ${\mathcal M}$ is the
pre-image under $\mathbf{t}$ of the boundary of $I$.
1. The level sets $\mathbf{t}^{-1}(t)$ form a codimension-one foliation of
${\mathcal M}$, called the horizontal foliation, with the boundary
components of ${\mathcal M}$ being leaves.
1. There is a metric $G$ on the horizontal distribution, i.e., the distribution
tangent to the level sets of $\mathbf{t}$. This metric induces a
...
1. Because of the first property the integral curves of $\chi$
...
to Hamilton (see Section 4 of [Hamiltonsurvey]), and derives
many of its consequences, which are also due to Hamilton (cf.
[HamiltonNSRF3M]). This principle and its consequences are at
...
Equation (0.1) proves that the minimum of the scalar
...
$$
 ...
 and thus the equation must develop a singularity at or
before time $n/\left(2R_\mathit{min}(0)\right)$.
...
- **(1)** For $3$-dimensional flows, if the Ricci curvature is positive,
...
round; see [Hamilton3MPRC].
- **(2)** For $3$-dimensional flows, as the scalar curvature goes to $+\infty$
...
zero; see [HamiltonNSRF3M]. This condition is called *pinched toward positive curvature*.
- **(3)** Motivated by a Harnack inequality for the heat equation established
by Li-Yau [LiYau], Hamilton established a Harnack
inequality for the
curvature tensor under the Ricci flow for complete manifolds
$(M,g(t))$ with bounded, non-negative curvature operator; see
[Hamiltonharnack]. In the applications to three dimensions, we
...
- **(4)** If a complete $3$-dimensional Ricci flow
...
[Hamilton4MPCO].
- **(5)** In particular, there is no Ricci flow of non-negative curvature tensor
...
[Cheeger]. Here Cheeger obtained topological results. In
[Gromov] Gromov proposed that geometric limits should exist in
the Lipschitz topology and suggested a result along these lines,
which also was known to Cheeger.
 In
[GreeneWu], Greene-Wu gave a rigorous proof of the compactness
...
[SPeters]. Assuming that all the derivatives of curvature are
...
Hamilton in [Hamiltonlimits] extended these results to Ricci
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="0.7" -->
**Theorem.** Suppose we have a sequence of based Ricci flows
...
Hamilton in [Hamilton4MPIC] had introduced the notion of
...
functional, *the reduced length*, which is the tool by which he
...
### The reduced length function

In Chapter 6 we come to
...
and $\gamma(0)=x$. These paths are said to be *parameterized by
backward time*. See Fig. 0.2. The ${\mathcal
L}$-*length* of such a path is given by
...
There is also the closely related *reduced length*
...
theory for the usual energy function (Even though this
...
t\}}\bar\tau^{-3/2}e^{-\ell_{(x,t)}(q,\bar\tau)}d\mathit{vol}_{g(\bar\tau}(q),
$$

 where, as before $\bar\tau=t-\bar t$.
...
### Application to non-collapsing results
...
making a stronger curvature bound assumption: one considers
 points $(x,t)$ and constants $r$ with the property that $|\mathit{Rm}|\le r^{-2}$ on
$P(x,t,r,-r^2)=B(x,t,r)\times (t-r^2,t]$. The
$\kappa$-non-collapsing condition applies to these balls and says
that $\mathit{Vol}(B(x,t,r))\ge \kappa r^3.$ The basic idea in proving
...
### Application to ancient $\kappa$-non-collapsed solutions
...
soliton is called an *asymptotic
soliton* for the
original $\kappa$-solution, see Fig. 0.3.
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="0.8" -->
...
hypothesis is essential. See Hamilton's work [Hamiltonsurvey]
...
Hamilton's maximum principle result (5) in Chapter 0.3.5)
...
perform surgery. This we do in Chapter 12. We fix a
...
outside this region has positive sectional curvature, see Fig. 0.4. Any such metric will suffice for the gluing
process, and we fix one and call it the *standard
...
1. $(x,t)$ is contained in the core of a $(C_2,\epsilon)$-cap,
where $C_2<\infty$ is a given universal constant depending only on $\epsilon$.
1. $(x,t)$ is the center of a strong $\epsilon$-neck.
1. $(x,t)$ is the center of an evolving $\epsilon$-neck whose
initial slice is at time zero.

These form the second source of models for canonical neighborhoods in a Ricci
flow with surgery. Thus, we shall set $C=C(\epsilon)=\mathit{max}(C_1(\epsilon),C_2(\epsilon))$ and we shall find $(C,\epsilon)$-canonical
...
surgery operations. We define *space-time* to be a
$4$-dimensional Hausdorff singular space with a time function $\mathbf{t}$ with the property that each time-slice is a compact, smooth
...
there are *exposed regions* at a discrete
...
[0,1)$, see Fig. 0.5. There is a natural
...
space-time with the property that the restriction of $G$, $\mathbf{t}$
and $\chi$ to the open subset of smooth points forms a generalized
Ricci flow. We call this the *associated generalized Ricci
flow* for the Ricci flow with surgery.

### The inductive conditions necessary for doing surgery
...
Fix $\epsilon>0$ sufficiently small and let $C=\mathit{max}(C_1,C_2)<\infty$, where $C_1$ is the constant associated to
...
1. $x$ is contained in a connected component of a
time-slice that is a $C$-component.
1. $x$ is contained in a connected component of its time-slice
that is within $\epsilon$ of round in the
$C^{[1/\epsilon]}$-topology.
1. $x$ is contained in the core of a $(C,\epsilon)$-cap.
1. $x$ is the center of a strong $\epsilon$-neck.
...
1. The initial metric is normalized, meaning that for the
...
1. The curvature of the flow is pinched toward positive.
1. There is $\kappa>0$ so that the associated generalized Ricci flow is
$\kappa$-non-collapsed on scales at most $\epsilon$, in the sense
that we require only that balls of radius $r\le \epsilon$ be
$\kappa$-non-collapsed.
1. There is $r_0>0$ such that any point of space-time at which
...
such tubes *$\epsilon$-horns*. We are interested in
...
time $T$ evolve under the Ricci flow. See Fig. 0.6.
...
## Extending Ricci flows with surgery
...
This completes our outline of the proof of \entryref{847af3ecd4a8}.
...
find a connected open subset ${\mathcal X}$ of $\mathbf{t}^{-1}([T_0,\infty))$ with the property that for each $t\ge  T_0$
...
necessary for the topological application (\entryref{127db1dc9315}),
or indeed, for any other topological application. The reason is the
sphere theorem (see [Hempel]), which says that if $\pi_2(M)$ is
...
[Perelman3] and the other due to Colding-Minicozzi in
[ColdingMinicozzi]. In their approach Colding-Minicozzi
associate to a non-trivial element in $\pi_3(M)$ a non-trivial
element in $\pi_1(\mathit{Maps}(S^2,M))$. This element is represented
by a one-parameter family of $2$-spheres (starting and ending at
the constant map) representing a non-trivial element $\xi\in
\pi_3(M_0)$. They define the *width* of this homotopy class by
...
[Jost], they show that this function satisfies the same forward
...
daunting (Colding and Minicozzi tell us they plan to give an
...
in \entryref{127db1dc9315} the entire manifold must disappear at
finite time.
...
curve-shrinking flow (see [AG]). Unfortunately, this flow can
...
the Ricci flow does not. Following Perelman, or indeed [AG],
one uses the device of taking the product with a small circle and
using loops, called *ramps*, that go around that
...
Perelman's arguments [KleinerLott]. We have referred to those
...
Xiao-Dong Wang, see [STW], wrote notes for the seminars at MIT,
...
First and foremost are Perelman's three preprints, [Perelman1],
[Perelman2], and [Perelman3]. The first of these
...
surgery, and the last gives the short-cut to the Poincaré
...
Kleiner and John Lott, [KleinerLott], which greatly expand and
clarify Perelman's arguments from the first two preprints. There is
also a note on Perelman's second paper by Yu Ding [yuding].
...
[SY]. Lastly, after we had submitted a preliminary version of
this manuscript for refereeing, H.-D. Cao and X.-P. Zhu published an
article on the Poincaré Conjecture and Thurston's Geometrization
Conjecture; see [CaoZhu].
