...
# Chapter 17 -- Completion of the proof of Theorem \entryref{703e4cfeece7}
...
the proof of \entryref{703e4cfeece7} it remains for us to show the
existence of strong canonical neighborhoods. This is the result of
the next section.

## Proof of the strong canonical neighborhood assumption

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="17.1" label="extend" -->
**Proposition.** Suppose that for some $i\ge 0$ we have surgery parameter sequences
...
the constant in \entryref{4ce8b5da9c58} associated to these
...
also the five properties given in the hypothesis of \entryref{703e4cfeece7} with
...
**Proof.** Suppose that the result does not hold.
 ...
 $\delta_{a,b}\le \delta(r_a)$, where $\delta(r_a)\le \delta_i$ is the constant in \entryref{4ce8b5da9c58}
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="17.2" label="16.2" -->
**Lemma.** For each $a$, and given $a$, for all $b$ sufficiently large there is $t_{(a,b)}\in
...
M}_{(a,b)},g_{(a,b)})$ to $\mathbf{t}^{-1}([0,T_i))$ satisfies the
hypothesis of the proposition, we see that any counterexample point
$x$ has $\mathbf{t}(x)\ge T_i$. Take a sequence $x_n=x_{n,(a,b)}$ of
counterexample points with $\mathbf{t}(x_{n+1})\le \mathbf{t}(x_{n})$ for
all $n$ that minimizes $\mathbf{t}$ among all counterexample points in
the sense that for any $\xi>0$ and for any counterexample point
$x\in {\mathcal M}_{(a,b)}$ eventually $\mathbf{t}(x_{n})< \mathbf{t}(x)+\xi$. Let $t'=t_{(a,b)}'=\mathit{lim}_{n\rightarrow\infty}\mathbf{t}(x_{n})$. Clearly, $t'\in [T_i,T_{(a,b)})$, and by construction
the restriction of $({\mathcal M}_{(a,b)},G_{(a,b)})$ to $\mathbf{t}^{-1}([0,t'))$ satisfies the $(C,\epsilon)$-canonical neighborhood assumption
with parameter $r_a$. Since the surgery times are discrete, there is
$t''=t_{(a,b)}''$ with $t'<t''\le T_{(a,b)}$ and a diffeomorphism
$\psi=\psi_{(a,b)}\colon M_{ t'}\times[t',t'')\to \mathbf{t}^{-1}([t',t''))$ compatible with time and the vector field. We
view $\psi^*G_{(a,b)}$ as a one-parameter family of metrics
$g(t)=g_{(a,b)}(t)$ on $M_{t'}$ for $t\in [t',t'')$. By passing to a
subsequence we can arrange that $\mathbf{t}(x_{n})\in [t',t'')$ for all
...
$x_{(a,b)}$. According to \entryref{3aa5883ab629} there are
...
\entryref{14ff8384814f} and \entryref{50312568fd70} are strong
...
contains $x_{(a,b)}$. By \entryref{7c2758f0e2b3}, for all $n$
...
(f_{(a,b)}\times\mathit{Id})^*\psi_{U_{(a,b)}}^*(R({x_{(a,b)}})G_{(a,b)})$ is
...
- **(i)** $\psi_{U_{(a,b)}}$ extends backward past $
t_{(a,b)}'-R^{-1}(x_{(a,b)})$.
- **(ii)** There is a flow line through a point $y_{(a,b)}\in U_{(a,b)}$ that is defined
...
embedding $\psi_{n,(a,b)}\colon U_{(a,b)}\times (\mathbf{t}(x_{n,(a,b)})-R^{-1}(x_{n,(a,b)}),\mathbf{t}(x_{n,(a,b)})]\to
...
the Riemannian metrics $(f_{(a,b)}\times\mathit{Id})^*\psi_{n,(a,b)}^*(R(x_{n,(a,b)})G_{a,b})$ converge in the
$C^\infty$-topology to the pullback $(f_{(a,b)}\times\mathit{Id})^*\psi_{U_{(a,b)}}^*(R(x_{(a,b)})G_{a,b})$. Clearly then, for
...
S^2\times (-\epsilon^{-1},\epsilon^{-1})$ where $\mathit{lim}_{n\rightarrow\infty}s_{n,(a,b)}=0$. The last thing we do is
...
$$
S^2\times (-\epsilon^{-1},\epsilon^{-1})\stackrel{\mathit{Id}\times
...
t_{(a,b)}$ from above. (See Fig. 17.1.) We denote by
...
$\tilde t'_{(a,b)}=\mathbf{\tilde t}(x_{(a,b)})$. Since the initial time of the strong $\epsilon$-neck
is zero, $\tilde t'_{(a,b)}=\tilde Q_{(a,b)}^{-1}$. We denote the flow line backward in time from
 ...
 it follows from \entryref{dc5709757703} and rescaling that
...
Let $D$ be the universal constant given in \entryref{0b07beec84c0}, so that
...
in \entryref{cf61b731c6f2} that, since we have rescaled to make
...
less than
 ...
 According to \entryref{ffae4504318a} there is $\theta_1<1$
(depending only on $D$) such that $R(q,t)\ge 8D$ for all $(q,t)$
in the standard solution with $t\ge \theta_1$.

By the fifth property of \entryref{f44d182ef27e} there is
...
is the constant given in \entryref{0a4d6f17810d}.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.3" -->
...
given in \entryref{b92e1a990021}. Then
$\tilde t_{(a,b)}'\le \theta_1$.
<!-- astrolabe:end -->

**Proof.** In this proof we shall fix $(a,b)$, so we drop these indices from
the notation.
 Consider
...
$$
 ...
 compatible with time and the vector field. First
suppose that $s<\theta_1$. Then according to
\entryref{ad4f67ea75bc} either the entire ball $B(p,0,A)$
...
\entryref{b92e1a990021} and by \entryref{ad4f67ea75bc}
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.4" -->
...
Hence, by the triangle inequality and Inequality (17.1), we have
...
$$
\psi_{(a,b)}(q_{(a,b)},\tilde t'_{(a,b)})=x_{(a,b)}.
$$
...
$\beta\epsilon/3$-neck, where $\beta$ is the constant from \entryref{a992d6a89479}.
...
given in \entryref{dc5709757703}. We can then conclude from \entryref{6ff28dba4e56}
...
\entryref{7c2758f0e2b3} that for all $b$ sufficiently large, the image $\psi_{(a,b)}(Y_{(a,b)})$
...
\entryref{778d690c4e7c}.

<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="17.5" -->
...
Now we return to the proof of \entryref{254e8dae97e6}. For each $a$, we pass
to a subsequence (in $b$) so that \entryref{778d690c4e7c} holds for all $(a,b)$. For
each $(a,b)$, let $t_{(a,b)}$ be as in that lemma. We fix a point
$x_{(a,b)}\in \mathbf{t}^{-1}(t_{(a,b)})\subset {\mathcal M}_{(a,b)}$ at which the
...
\entryref{4ce8b5da9c58}, since $\delta_{b(a)}\le \delta(r_a)$
it follows that the restriction of $(\widetilde {\mathcal
M}_a,\widetilde G_a)$ to $\mathbf{t}^{-1}(-\infty,0)$ is
...
- **(i)** There is $A<\infty$ and $t'<\infty$ such that,
...
1$ and $\mathbf{t}(\tilde x)<0$ has a strong $(C,\epsilon)$-canonical
neighborhood.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.6" -->
...
zero and hence by the last item in \entryref{cf61b731c6f2} and
...
converging to $\tilde x$ with $\mathbf{t}(\tilde y_n)<0$. Of course, for all $n$ sufficiently large
$R(\tilde y_n)>1$,
...
\entryref{0760630d3990} we see that $\tilde x$ has a strong
$(2C,2\epsilon)$-canonical neighborhood of the same type. On the other hand, if $\tilde y_n$ is the center
of a strong $\epsilon$-neck for all $n$, then according to \entryref{d74749898b25},
the limit point $\tilde x$ is the center
of a strong $2\epsilon$-neck.

Since we have chosen $\epsilon>0$ sufficiently small so that \entryref{a9e023d082dc}
...
$\kappa$-solution. By \entryref{e724298e5bc3} this contradicts
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="17.7" label="lemstdcan" -->
**Lemma.** Suppose that there are $A',D',t'<\infty$ such that the following
...
bounded independent of $a$.
 ...
 By \entryref{0b07beec84c0}, the fact
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.8" -->
**Claim.** There is $\theta_1<1$, depending on $D'$ and $t'$, such that for all
$a$ sufficiently large we have $t_a< \theta_1\bar h_a^2$.
<!-- astrolabe:end -->

**Proof.** We consider two cases: either $t_a\le \bar h_a^2/2$ or $\bar
...
\entryref{ffae4504318a} fix $1/2<\theta_1<1$ so that every point of
...
from \entryref{b92e1a990021} and $\delta''_0$ is the constant
from \entryref{ad4f67ea75bc}.
...
\entryref{ad4f67ea75bc}. But this is impossible since
...
\entryref{f44d182ef27e} we set $A''\ge (9\widetilde
...
to zero which means, by \entryref{ad4f67ea75bc}, that for all $a$ sufficiently
...
Equation (17.2), this means that for all $a$ sufficiently large
...
\entryref{51a2afee4465}, the distance at time $0$ between $\rho_a(p_0,0)$ and
...
close to $\bar h_a^{-2}$. Hence, by \entryref{6ff28dba4e56}
...
C$-neighborhood of $x_a$. If either of the first two cases in \entryref{6ff28dba4e56} occurs
for a subsequence of $a$ tending to infinity, then by
\entryref{7c2758f0e2b3} and the fact that $\widetilde C>\mathit{max}(C,\epsilon^{-1})$, we see that there is a subsequence of $a$
...
\entryref{a992d6a89479}. Applying this result tells us that $x_a$
...
\entryref{111551d7831b}.

There are several steps required to complete the proof of
\entryref{254e8dae97e6}. The first step helps us apply the previous claim to
find strong $(C,\epsilon)$-canonical neighborhoods.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.9" label="D(A)delta(A)" -->
**Claim.** Given any $A<\infty$ there is $D(A)<\infty$ and $\delta(A)>0$ such that for all $a$
...
r^{-2}_a$ and $\mathbf{t}(y)<\mathbf{t}(x_a)$ have strong
$(C,\epsilon)$-canonical neighborhoods, and since $R(x_a)=r^{-2}_a$,
we see that all points $y\in (\widetilde{\mathcal M}_a,\widetilde
G_a)$ with $\mathbf{t}(y_a)<0$ and with $R_{\tilde G_a}(y_a)\ge 1$ have
strong $(C,\epsilon)$-canonical neighborhoods. It follows that all
points in $(\widetilde {\mathcal M}_a,\widetilde G_a)$ with $\mathbf{t}(y)\le 0$ and $R(y)>1$ have strong $(2C,2\epsilon)$-canonical
neighborhoods. Also, since $\delta_a\le \delta(r_a)$, where
$\delta(r_a)$ is the constant given in \entryref{4ce8b5da9c58},
and since the condition of being $\kappa$-non-collapsed is a closed
constraint, it follows from \entryref{4ce8b5da9c58} that these
Ricci flows with surgery are $\kappa$-non-collapsed for a fixed
$\kappa>0$. It is now immediate from \entryref{ecd62fc2ea2b} that there
...
Arguing as in \entryref{0ddfe578961e} we see that there is a
...
it follows that there is a bound $D(A)$ depending only on $D'(A)$ to $|\mathit{Rm}|$ on the same flow lines.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.10" -->
...
then, by \entryref{7549bace9302},
there are constants $D(A)<\infty$ and $\delta(A)>0$ such that, replacing $t(A)$ by $\mathit{min}(t(A),\delta(A))$,
 $|\mathit{Rm}|$ is bounded by $D(A)$ on $P$.
...
$\mathit{lim}_{a\rightarrow \infty}(t_a)=0$. Then, by the previous claim, for all $a$ sufficiently large, the
...
applying \entryref{111551d7831b} produces the strong
...
M}_a$ and there is a bound, depending only on $A$ for $|\mathit{Rm}|$ on
this backward parabolic neighborhood. Applying \entryref{d9f3522dec12} we see that, after passing to a
...
Lastly, we show that $(M_\infty,g_\infty)$ has bounded curvature. By Part 3 of \entryref{7c2758f0e2b3}
...
or is contained in the core of a $(2C,2\epsilon)$-cap. According to \entryref{cb9ca6d6db9f}
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="17.11" -->
...
$(M_\infty,g_\infty,x_\infty)$. Then by \entryref{0ddfe578961e}
 there is a constant
...
\entryref{111551d7831b} we see that for all $a$ sufficiently large the
...
\entryref{a9e023d082dc}, after passing to a subsequence, there is
...
a surgery cap. Invoking \entryref{0ddfe578961e} again, we see that
...
This completes the proof of \entryref{254e8dae97e6}.

## Surgery times don't accumulate

Now we turn to the proof of \entryref{703e4cfeece7}.
 Given surgery parameter
sequences

$$
\Delta_i=\{\delta_0,\ldots,\delta_i\}
$$

$$
\mathbf{r_i}=\{r_0,\ldots,r_i\}
$$

$$
\mathbf{K_i}=\{\kappa_0,\ldots,\kappa_i\},
$$

 we let
$r_{i+1}$ and $\delta_{i+1}$ be as in \entryref{254e8dae97e6} and
then set $\kappa_{i+1}=\kappa(r_{i+1})$ as in
\entryref{4ce8b5da9c58}.
 Set

$$
{\mathbf{r}_{i+1}}=\{\mathbf{r_i},r_{i+1}\}
$$

$$
{\mathbf{K}_{i+1}}=\{\mathbf{K_i},\kappa_{i+1}\}
$$
...
Ricci flow with surgery satisfies the conclusion of \entryref{703e4cfeece7} with
...
the conclusions of \entryref{703e4cfeece7} on its entire interval of definition.
...
to a later time $T'$. By \entryref{4ce8b5da9c58} and
\entryref{254e8dae97e6} this extension will continue to satisfy the
conclusions of \entryref{703e4cfeece7} on its entire interval of
...
\entryref{f9cff326f854} this means that $T$ is not a surgery time and we
...
at time $T$. According to \entryref{c8e7931dbf65} the union along
...
$\delta(r_{i+1})$, the constant from \entryref{4ce8b5da9c58},
 for all $t\in [T_{i-1},T]$, since $T\le T_{i+1}$,
and since the restriction of $({\mathcal M},G)$ to $\mathbf{t}^{-1}([0,T_i))$ satisfies \entryref{4ce8b5da9c58}, we see by
\entryref{254e8dae97e6} that the extended Ricci flow with surgery
satisfies the conclusion of \entryref{703e4cfeece7} on its entire time
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="17.12" label="bddsurgery" -->
**Lemma.** Given a Ricci flow with surgery $({\mathcal M},G)$ defined on $[0,T)$ with
...
hypotheses of \entryref{703e4cfeece7} on its entire time-domain of definition, there
...
a surgery time, then $\mathit{Vol}(t)=\mathit{Vol}(M_t,g(t))$ is a smooth function
of $t$ near $t_0$ and

$$
\frac{d\mathit{Vol}}{dt}(t_0)=-\int_{M_{t_0}}Rd\mathit{vol},
$$

 so that, because of the curvature
pinching toward positive hypothesis, we have $\frac{d\mathit{Vol}}{dt}(t_0)\le
6\mathit{Vol}(t_0)$. If $t_0$ is a surgery time, then either $M_{t_0}$ has fewer
...
where $K<\infty$ is the universal constant given in \entryref{0b07beec84c0}. Since
...
$\overline\delta(T_{i+1})$, $T_{i+1}$, $r_{i+1}$ and $\mathit{Vol}(M_0,g(0))$ to
...
completes the proof of \entryref{703e4cfeece7}.

To sum up, we have sequences $\Delta$, $\mathbf{K}$ and $\mathbf{r}$ as given in
\entryref{703e4cfeece7}. Let $\overline \delta\colon [0,\infty)\to \Ar$ be a
...
surgery is $\mathbf{K}$-non-collapsed and satisfies the strong
$(C,\epsilon)$-canonical neighborhood theorem with respect to the parameter
$\mathbf{r}$. It also has curvature pinched toward positive. Lastly, for any $T\in
