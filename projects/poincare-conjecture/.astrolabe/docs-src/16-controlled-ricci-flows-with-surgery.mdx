...
# Chapter 15 -- Controlled Ricci flows with surgery
...
- **(1)** $|\mathit{Rm}(x,0)|\le 1$ for all $x\in M$ and
- **(2)** for every $x\in M$ we have $\mathit{Vol} B(x,0,1)\ge \omega/2$ where
...
\entryref{e6b1bd9fa0d7}, \entryref{b6bc786f7f44}, \entryref{ed4b72caf9f9},
and \entryref{c1330c5a8be3}.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="15.1" label="newkappa0r0t0" -->
**Claim.** There is $\kappa_0$ such that the following holds. Let $(M,g(0))$ be a normalized initial
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="15.2" label="neckglue" -->
**Proposition.** There is $0<\beta<1/2$ such that the following holds for any
...
proved the result. See Fig. 15.1.
...
\mathit{min}(1/200,\left(\sqrt{D}(A_0+5)\right)^{-1},\bar\epsilon_1/2,\bar
\epsilon'/2,\epsilon_0)$ where $\bar\epsilon_1$ is the constant from
\entryref{cb9ca6d6db9f}, $\bar\epsilon'$ is the constant from
\entryref{c2be59f6090c}, $\epsilon_0$ is the constant from Section 10.1,
and $A_0$ and $D$ are the constants from
\entryref{0b07beec84c0}.
 We fix $\beta<1/2$, the constant from
\entryref{a992d6a89479}. Then we let $C$ be the maximum of the
constant $C(\epsilon)$ as in \entryref{fc2d2036a96c} and
$C'(\beta\epsilon/3)+1$ as in \entryref{6ff28dba4e56}.

For all such $\epsilon$, \entryref{ecd62fc2ea2b} holds for $\epsilon$ and
\entryref{cb9ca6d6db9f}, \entryref{7c2758f0e2b3} and
Corollaries \entryref{fc2d2036a96c} and \entryref{e724298e5bc3} and Theorems \entryref{d51aab8a8065} and \entryref{a9e023d082dc}
 ...
 **Assumption (4). Topology of the exposed regions:** *At all singular times $t$ we require that $E_{t}$ be a finite
...
Suppose that $t$ is a surgery time. Let ${\mathcal M}_{(-\infty,t)}$ be $\mathbf{t}^{-1}((-\infty,t))$
and let $(\widehat {\mathcal M}_{(-\infty,t)},\widehat G)$ be the maximal extension of
$({\mathcal M}_{(-\infty,t)},G)$ to time $t$, as given in \entryref{8651cf20bd2d}.

**Assumption (5). Boundary components of the exposed
regions:** *There is a surgery control parameter
function,
...
**Assumption (6). Control on the disappearing region:** *For any
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="15.3" label="surgerytoptype" -->
**Proposition.** Suppose that $({\mathcal M},G)$ is a generalized Ricci flow satisfying Assumptions
...
sufficiently small it follows from \entryref{fa8d9a43f34a}
...
$\epsilon$-neck, then according to \entryref{ba681e15e1a6}
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="15.4" label="Tgood0good" -->
**Corollary.** Let $({\mathcal M},G)$ be a generalized Ricci flow satisfying Assumptions
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="15.5" label="hrestricted" -->
**Definition.** We have: (i) *a canonical neighborhood parameter*,
 $r(t)>0$, and
...
function given by \entryref{074e02b067b8}. We require that $h(0)\le
R_0^{-1/2}$ where $R_0$ is the constant from
\entryref{cf61b731c6f2}.
 ...
 **For any singular time $t$ the surgery at time $t$ is
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="15.6" -->
...
initial conditions. Then
 according to
\entryref{f5e180ecfd18} the flow exists on $[0,T_1]$ and the norm
...
by \entryref{f5e180ecfd18} there is a $\kappa_0>0$ so
that $\mathit{Vol} B(x,t,r)\le \kappa_0r^3$ for every $t\le T_1$ and
$x\in M_t$ and every $r\le \epsilon$.

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="15.7" label="surgeryparams" -->
**Definition.** *Surgery parameter sequences* are sequences

- **(i)** $\mathbf{r}=r_0\ge r_1\ge r_2\ge \cdots >0$, with $r_0=\epsilon$,
- **(ii)** $\mathbf{K}=\kappa_0\ge \kappa_1\ge \kappa_2\ge \cdots >0$
with $\kappa_0$ as in \entryref{f5e180ecfd18}, and
- **(iii)** $
...
from \entryref{cf61b731c6f2} and $\beta<1/2$ is the constant from
\entryref{a992d6a89479}, $\epsilon$ is the constant that we have
...
satisfies the strong $(C,\epsilon)$-canonical neighborhood assumption with parameter $\mathbf{r}$ if it satisfies this condition with respect to the step function $r(t)$
associated with $\mathbf{r}$. This means that any $x\in {\mathcal M}$ with
$R(x)\ge r^{-2}(\mathbf{t}(x))$ has a strong $(C,\epsilon)$-canonical neighborhood in
...
if for some $r\le \epsilon$, the parabolic neighborhood $P(x,\mathbf{t}(x),r,-r^2)$ exists in ${\mathcal M}$
and the norm of the Riemann curvature is bounded on this backward
parabolic neighborhood by $r^{-2}$, then $\mathit{Vol} B(x,\mathbf{t}(x),r)\ge
\kappa r^3$.
We say that a Ricci flow with surgery is $\mathbf{K}$-non-collapsed on scales
...
Having fixed surgery parameter sequences $\mathbf{K}$, $\mathbf{r}$ and
...
sequences $\mathbf{r}$, $\mathbf{K}$ and $\Delta$ with the property that
...
$t\le T_{i+1}$ with $\delta_0=\mathit{min}(\alpha\epsilon/3,\delta_0',K^{-1},D^{-1})$ as above. Suppose
...
\entryref{af00774dbaa9} we know that there is a maximal extension
...
$h(T)=h(\rho(T),\overline\delta(T))$ as in \entryref{074e02b067b8}.
Since $\overline\delta(T)\le \delta_0<1$, we see that
$\rho<r_{i+1}$. According to \entryref{01f0d3baa28f} there are
finitely many components of $\Omega(T)$ that meet $\Omega_\rho(T)$.
Let $\Omega^\mathit{big}(T)$ be the disjoint union of all the
components of $\Omega(T)$ that meet $\Omega_\rho(T)$.
\entryref{01f0d3baa28f} also tells us that $\Omega^\mathit{big}(T)$
...
with $R(y_i)=h^{-2}(T)$. According to \entryref{074e02b067b8} for each
...
the complement of $\coprod_{i=1}^j{\mathcal H}^+_i$ in $\Omega^\mathit{big}(T)$. Then we do surgery on these necks as described in
Section 13.1, using the constant $q=q_0$ from
\entryref{cf61b731c6f2},
...
space-time. According to \entryref{b186908cc3b4} the horizontal
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="15.8" -->
...
## Statements about the existence of Ricci flow with surgery
...
parameter $\mathbf{r}=\{r_i\}$ and of the non-collapsing step function $\mathbf{K}=\{\kappa_i\}$.

Here is the statement that we shall establish.

<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="15.9" label="MAIN" -->
**Theorem.** There are surgery parameter sequences

$$
\mathbf{K}=\{\kappa_i\}_{i=1}^\infty,\Delta=\{\delta_i\}_{i=1}^\infty,\mathbf{r}=\{r_i\}_{i=1}^\infty
$$
...
- **(1)** the generalized flow has curvature
pinched toward positive,
- **(2)** the flow satisfies the strong $(C,\epsilon)$-canonical neighborhood assumption
with parameter $\mathbf{r}$ on $[0,T)$, and
- **(3)** the flow is $\mathbf{K}$
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="15.10" label="RFSexists" -->
**Corollary.** Let $\mathbf{K}$, $\mathbf{r}$ and $\Delta$ be surgery parameter sequences
...
**Proof.** (Assuming \entryref{703e4cfeece7})
...
According to \entryref{c1330c5a8be3} and the definitions of
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="15.11" label="14.26" -->
**Lemma.** Suppose that $({\mathcal M},G)$ is a Ricci flow with surgery going
...
**Proof.** It is immediate from the construction and \entryref{b186908cc3b4} that
...
the vector field. For any $p\in M_{t'}\setminus\psi(\mathit{int} C_T\times \{t'\})$ the limit as $t$ tends to $T$ from below of
the flow line $p(t)$ at time $t$ through $p$ either lies in
$\Omega(T)$ or it does not. In the latter case, by definition we
have

$$
\mathit{lim}_{t\rightarrow T^-}R(p(t))=\infty.
$$
...
$$
\mathit{lim}_{t\rightarrow T^-}R(p(t))>\rho^{-2}>r_i^{-2}.
$$

Since $M_{t'}\setminus \psi(\mathit{int} C_T\times \{t'\})$ is compact
for every $t'$, there is $T_1<T$ such that $R(p(t))>r_i^{-2}$ for
all $p\in M_{t'}\setminus \psi(\mathit{int}C_T\times \{t'\})$ and all
...
Assumption (7) holds for the new Ricci flow with surgery.
 ...
 From \entryref{cf61b731c6f2} the
...
from \entryref{fc44d8653f35} that the Ricci flow defined on $[T,T')$
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="15.12" label="surgerydist" -->
**Proposition.** Suppose that $({\mathcal M},G)$ is a Ricci flow with surgery
...
\entryref{22c40a5664ac}, and suppose that the scale of the
surgery $h(T)$ is less than $R_0^{-1/2}$ where $R_0$ is the constant
from \entryref{cf61b731c6f2}. Fix $t'<T$ sufficiently close to
...
**Proof.** This is immediate from the third item in \entryref{cf61b731c6f2}.

<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="15.13" -->
...
The other two inductive properties in \entryref{703e4cfeece7} -- that the result is
$\mathbf{K}$-non-collapsed and also that it satisfies the strong
$(C,\epsilon)$-canonical neighborhood assumption with parameter $\mathbf{r}$ -
...
## Outline of the proof of Theorem \entryref{703e4cfeece7}

Before giving the proof proper of \entryref{703e4cfeece7} let us outline
how the argument goes. We shall construct the surgery parameter
sequences $\Delta$, $\mathbf{r}$, and $\mathbf{K}$ inductively. Because of
\entryref{c1330c5a8be3} we have the beginning of the inductive
...
extension is possible. In Chapter 16 we establish the
...
In Section 17.1 we show that the strong $(C,\epsilon)$-canonical neighborhood
assumption extends for some parameter $r_{i+1}$, assuming again that
$\overline\delta(t)\le \delta(r_{i+1})$ for all $t\in [T_{i-1},T)$.

Lastly, in Section 17.2 we complete the proof by
