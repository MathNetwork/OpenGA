...
# Chapter 12 -- The standard solution
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="12.1" label="defn:stinmetr" -->
**Definition.** A *standard initial metric*
...
fourth condition, namely:
 ...
 (iv) $g_0$ is asymptotic at infinity in the
...
\entryref{c5e007547ff3}.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.2" label="stdinit" -->
**Lemma.** There is a standard initial metric.
<!-- astrolabe:end -->

**Proof.** We construct our Riemannian manifold as follows. Let
$(x_0,x_1,x_2,x_3)$ be Euclidean coordinates on $\Ar^4$. Let
$y=f(s)$ be a function defined for $s\ge 0$ and satisfying:

- **(1)** $f$ is $C^\infty$ on $(0,\infty)$
- **(2)** $f(s)>0$ for all $s>0$.
- **(3)** $f''(s)\le 0$ for all $s>0$.
- **(4)** There is $s_1>0$ such that $f(s)=\sqrt{2}$ for all $s\ge
s_1$.
- **(5)** There is $s_0>0$ such that
...
$$
 ...
 in the $(x_0,x_1)$-plane. We define $\Sigma(f)$ by
rotating $\Gamma$ about the $x_0$-axis in four-space:

$$
\Sigma(f)=\{(x_0,x_1,x_2,x_3)\bigl|\bigr. x_0\ge 0 \text{ and }
x_1^2+x_2^2+x_3^2=f(x_0)^2\}.
$$

 Because of the last condition on
...
$$
\int_0^2h(s)\lambda(s)ds<\int_0^{1/2}h(s)<\sqrt{2}.
$$
...
$$
f(s)=\int_0^sh(\sigma)\lambda(\sigma-s_0)d\sigma.
$$
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.3" label="A_0" -->
**Lemma.** There is $A_0<\infty$ such that
...
### Uniqueness and properties: The statement

**Fix once and for all a standard initial metric $g_0$ on
$\Ar^3$.**

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="12.4" -->
**Definition.** A *partial standard Ricci flow* is a Ricci flow $(\Ar^3,g(t)),\ 0\le t<T$, such
that $g(0)=g_0$ and such that the curvature is locally bounded in
time. We say that a partial standard Ricci flow is a *standard
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="12.5" label="stdsoln" -->
**Theorem.** There is a standard Ricci flow defined for some
...
curvature, see [ChenZhu], but we preferred to give the more
...
applied to Equation (3.5), $\partial R/\partial t=\triangle
R+|\mathit{Ric}|^2$, then implies by \entryref{37d7d059a526} that
the maximum of $R$ at time $t$, $R_\mathit{max}(t)$ obeys the
inequality $\partial R_\mathit{max}/\partial t\le R_\mathit{max}(t)^2$,
and integrating this inequality (i.e., invoking
\entryref{67c378f87457}) one finds a positive constants $t_0$ and
$Q_0$ such that for each $n$, the norm of the scalar curvature of
$g_{R_n}(t)$ are bounded by $Q_0$ on the interval $[0,\mathit{max}(t_0,T_n))$. By \entryref{0db415314964}, for each $n$ the sectional
curvature of the flow $(S^3_{R_n},g_{R_n}(t)),\ 0\le t<T_n$ is
non-negative, and hence the sectional curvature of this flow is
also bounded by $Q_0$ on $[0,\mathit{max}(t_0,T_n))$. It now follows
from \entryref{f9cff326f854} and the fact that the $T_n$ are
...
\entryref{a0212d0eefcf} that there is a geometric limiting flow
...
\entryref{f44d182ef27e}:

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.6" label="third" -->
**Lemma.** For each $t_0\in [0,T)$ the Riemannian manifold $(\Ar^3,g(t_0))$ is
complete and of positive curvature.
<!-- astrolabe:end -->

**Proof.** Fix $t_0\in [0,T)$. By hypothesis $(\Ar^3,g(t)),\ 0\le t\le t_0$ has bounded
...
Recall from Section 4.2.1 that ${\mathcal T}$ is the curvature
...
[0,t_0]$. It follows from \entryref{37d7d059a526} that
$f(t)=\mathit{min}_{x\in M}\hat\mu(x,t)$ is a continuous function of
...
\entryref{0c451aa3e5cc} for the evolution of ${\mathcal T}$ in an evolving
orthonormal frame:

$$
\begin{aligned}
\frac{\partial \widehat{\mathcal T}}{\partial t} & = &
\frac{\partial{\mathcal T}}{\partial t}+\frac{\partial
\varphi}{\partial t}g-2\varphi \mathit{Ric}(g) \\
& = & \triangle {\mathcal T}+{\mathcal T}^2+{\mathcal T}^\#+\frac{\partial \varphi}{\partial t}g-2\varphi \mathit{Ric}(g) \\
& = & \triangle \widehat{\mathcal T}+{\mathcal T}^2+{\mathcal
T}^\#+\left(\frac{\partial \varphi}{\partial t}-\triangle
\varphi\right)g-2\varphi \mathit{Ric}(g)\\
&\ge & \triangle \widehat{\mathcal T}+{\mathcal T}^2+{\mathcal
T}^\#+\left(Cg-2\mathit{Ric}(g)\right)\varphi.
\end{aligned}
$$
...
\frac{\partial \widehat{\mathcal T}}{\partial t}\ge
 \triangle \widehat{\mathcal T}+(Cg/2-2\mathit{Ric}_g)\varphi.
$$

 Once again assuming that $C$ is
...
$$
 ...
 Thus, at any local minimum $x\in M$ for
...
This immediately implies by \entryref{37d7d059a526} that
$\psi(t)=\mathit{min}_{x\in M}\hat\mu(x,t)$ is a non-decreasing
...
Indeed, by \entryref{d21a675e4ea8} it has strictly positive
...
\entryref{e0b6a1f646a7} that there are uniform $C^\infty$
point-wise bounds for the curvatures of $g(t)$ restricted to
$\psi_k(S^2\times (-R,R))$. Thus,
 by \entryref{4673eb9f4424}, after passing to a subsequence, the flows
...
flow have two ends, it follows from \entryref{e5b196773169} that
every manifold in the flow is a product of a compact surface with
$\Ar$. According to \entryref{d21a675e4ea8} this implies that the
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="12.7" label="item5" -->
**Proposition.** Given $T'<T$ and $\epsilon>0$ there is a compact ball $B$ centered
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.8" label="1" -->
**Corollary.** The maximal time $T$ is $\le 1$.
<!-- astrolabe:end -->

**Proof.** If $T>1$, then we can apply the above result to $T'$ with
...
non-compact manifolds due to Chen and Zhu ([ChenZhu]), but we
...
Let $\mathit{Ric}_{ij}$ be the Ricci tensor and $\mathit{Ric}^i_k= g^{ij}
\mathit{Ric}_{jk}$ be the dual tensor. Let $X$ be a vector field
evolving by

$$
\frac{\partial}{\partial t}X=\triangle X+\mathit{Ric}(X,\cdot)^*.
$$
...
$$
\frac{\partial}{\partial t}X^{i}=(\Delta X)^{i}+\mathit{Ric}_{k}^{i}X^{k}
$$
...
$$
\frac{\partial X^*}{\partial t}=\triangle X^*-\mathit{Ric}(X,\cdot),
$$

or in local coordinates

$$
\frac{\partial X^*_i}{\partial t}=(\triangle X^*)_i-\mathit{Ric}_{ij}X^j.
$$

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.9" label="12.9" -->
**Lemma.** With $X$ and its dual $X^*$ evolving by the above equations, set
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="12.10" -->
**Remark.** The covariant derivative acts on one-forms $\omega$ in such a way
...
**Proof.** (of \entryref{78cab9a6159b})
...
\frac{\partial}{\partial t}X^*\right) \\ \nonumber
 &= &
\left(-\nabla^{l}\mathit{Ric}_{kj}+\nabla_{k}\mathit{Ric}_j^l+\nabla_{j}\mathit{Ric}_k^l\right)X^*_{l}dx^k\otimes
dx^j \\
& & +\nabla\left(  \Delta X^*-\mathit{Ric}(X,\cdot)\right)
.
\end{aligned}
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="12.11" -->
**Claim.** For any tensor $\phi$ we have

$$
\nabla(\nabla_r\phi)=\nabla_r(\nabla\phi)+dx^k\otimes
\mathit{Rm}(\partial_k,\partial_r)(\phi)-\nabla_r(dx^l)\otimes
\nabla_l(\phi).
$$
<!-- astrolabe:end -->
...
& & \left. +dx^k\otimes \mathit{Rm}(\partial_k,\partial_r)(\nabla_sX^*)-\nabla_r(dx^l)\otimes
\nabla_l\nabla_sX^*-\nabla(\Gamma_{rs}^l\nabla_lX^*)\right).
\end{aligned}
$$
...
\nabla_lX^*\right)
\\ & = & \nabla_r(dx^k)\otimes
\mathit{Rm}(\partial_k,\partial_s)X^*+dx^k\otimes\nabla_r(\mathit{Rm}(\partial_k,\partial_s))X^*
\\
& & + dx^k\otimes \mathit{Rm}(\partial_k,\partial_s)\nabla_rX^*-\nabla_r\nabla_sdx^l\otimes\nabla_lX^*-\nabla_sdx^l\otimes
\nabla_r\nabla_lX^*
\end{aligned}
$$
...
\nabla_r\left(dx^k\otimes \mathit{Rm}(\partial_k,\partial_s)X^*-\nabla_sdx^l\otimes
\nabla_lX^*\right)
\\ & = & dx^k\otimes\nabla_r(\mathit{Rm}(\partial_k,\partial_s))X^* + dx^k\otimes
\mathit{Rm}(\partial_k,\partial_s)\nabla_rX^*-\nabla_r\nabla_sdx^l\otimes\nabla_lX^*.
\end{aligned}
$$
...
\begin{aligned}
\nabla(\triangle X^*) & = & \triangle(\nabla X^*)+g^{rs}\left(
dx^k\otimes\nabla_r(\mathit{Rm}(\partial_k,\partial_s))X^* \right. \\
 & & \left. + 2dx^k\otimes
\mathit{Rm}(\partial_k,\partial_s)\nabla_rX^*
-\nabla_r\nabla_sdx^l\otimes\nabla_lX^*
 -d\Gamma_{rs}^l\otimes\nabla_lX^*\right).
...
& & -\mathit{Ric}_k^lV_{lj}dx^k\otimes dx^j.
\end{aligned}
$$
...
$$
\begin{aligned}
\nabla(\triangle X^*) -\nabla(\mathit{Ric}(X,\cdot)^*)= \\ &  &
\triangle(\nabla X^*)- g^{rs}{{(\nabla_rR)_{ks}}^l}_jX^*_ldx^k\otimes
dx^j-2{{R_k}^{rl}}_jV_{rl}dx^k\otimes dx^j
\\
& & -\left(\mathit{Ric}_k^lV_{lj}+\nabla_k(\mathit{Ric})_j^lX^*_l+\mathit{Ric}_j^lV_{kl}\right)dx^k\otimes dx^j,
\end{aligned}
$$

and consequently, plugging back into Equation (12.4), and
...
dx^j-2{{R_k}^{rl}}_jV_{rl}dx^k\otimes dx^j
\\
& & -\left(\mathit{Ric}_k^lV_{lj}+\mathit{Ric}_j^lV_{kl}\right)dx^k\otimes
dx^j.
\end{aligned}
$$
...
$$
\frac{\partial}{\partial t}h_{ij}=\Delta_{L}h_{ij},
$$

where by definition $\Delta_{L}h_{ij}=\Delta
h_{ij}+2{{R_i}^{kl}j}h_{kl}-\mathit{Ric}_i^k h_{kj}-\mathit{Ric}_j^kh_{ki}$ is the **Lichnerowicz
...
\right\vert ^{2}+C\left\vert h_{ij}\right\vert ^{2}.
\end{aligned}
$$

Note that $X(t)$ is a Killing vector field for $g(t)$ if and only if
$h_{ij}(t)=0$. Since Equation (12.1) is linear and since the
curvature is bounded on each time-slice, for any given bounded
Killing vector field $X(0)$ for metric $g(0)$, there is a bounded
solution $X^{i}\left( t\right) $ of Equation (12.1) for
$t\in[0,T]$. Then $\left \vert h_{ij}\left( t\right) \right
\vert^2 $ is a bounded function satisfying (12.6) and $\left
\vert h_{ij} \right \vert^2 (0)=0$. One can apply the maximum
principle to (12.6) to conclude that $h_{ij}(t)=0$ for all
...
virtue of (12.6) and \entryref{37d7d059a526}, the
...
\entryref{67c378f87457}, since $h(0)=0$ and $h\ge 0$, it follows
...
$\Delta X^i +R^i_kX^k =0$ for all $t$. Hence (12.2) gives
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.12" -->
**Corollary.** The standard solution $g(t),\ t\in [0,T)$, consists of a family of metrics
all of
which are
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="12.13" label="stdkappa" -->
**Proposition.** For any $r>0$ sufficiently small, there is a $\kappa>0$ such that
the standard flow is $\kappa$-non-collapsed on all scales $\le r$.
<!-- astrolabe:end -->

**Proof.** Since the curvature of the standard solution is non-negative, it
follows directly that $2|\mathit{Ric}|^2\le R^2$. By
Equation (3.5) this gives

$$
\frac{\partial R}{\partial t}=\triangle R+2|\mathit{Ric}|^2\le \triangle
R+R^2.
$$

 Let $C=\mathit{max}(2,\mathit{max}_{x\in \Ar^3}R(x,0))$. Suppose
that $t_0< T$ and $t_0<1/C$.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="12.14" -->
**Claim.** For all $x\in \Ar^3$ and $t\in [0,t_0]$ we have
...
**Proof.** By the asymptotic condition, there is a compact subset $X\subset
\Ar^3$ such that for any point $p\in \Ar^3\setminus X$ and for any
$t\le t_0$ we have $R(p,t)<2/(1-t)$. Since $C\ge 2$, for all $t$ for
which $\mathit{sup}_{x\in \Ar^3}R(x,t)\le 2/(1-t)$, we also have
...
$R_\mathit{max}(t_1)=C/(1-Ct_1)$. Since $C\ge \mathit{sup}_{x\in
...
\entryref{37d7d059a526} for all $t\in [t_1,t_2]$ we have

$$
R_\mathit{max}(t)\le G(t)
$$
...
$B(x,0,r)$ on which $|\mathit{Rm}|\le r^{-2}$ we have $\mathit{Vol} B(x,r)\ge V r^3$. Since there is a uniform bound on the
curvature on $[0,1/2C]$, it follows that there is $V'>0$ so that any
ball $B(q,t,r)$ with $t\le 1/2C$ on which $|\mathit{Rm}|\le r^{-2}$
satisfies $\mathit{Vol} B(q,t,r)\ge V'r^3$. Set $t_0=1/4C$. For any
point $x=(p,t)$ with $t\ge 1/2C$ there is a point $(q,t_0)$ such
that $l_x(q,t_0)\le 3/2$; this by \entryref{aebc3ff2bd3b}. Since
$B(q,0,1/\sqrt{R_\mathit{max}(0)})\subset \Ar^3$ has volume at least
$V/R_\mathit{max}(0)^{3/2}$, and clearly $l_x$ is bounded above on
$B(q,0,1/\sqrt{R_\mathit{max}(0)})$ by a uniform constant, we see that
the reduced volume of $B(q,0,1/\sqrt{R_\mathit{max}(0)})$ is uniformly
bounded from below. It now follows from \entryref{3defa9b2209c} that there
is $\kappa_0>0$ such that if $|\mathit{Rm}|$ is bounded by $r^{-2}$ on
...
[LuTian]. The presentation here is the analogy in the context
of the standard solution of DeTurck's argument presented in
Section 3.3.

### From Ricci flow to Ricci-DeTurck flow
...
,g(t_0) }\psi_{t}, \quad \psi_{t_0}  =\mathit{Id}.
\end{aligned}
$$
...
solution of 12.8 with $\psi_{t_0}=\mathit{Id}$. Then $\psi\left(
...
$$
\frac{\partial}{\partial t}\hat{g}_{ij} =-2\widehat{\mathit{Ric}}_{ij}+\hat{\nabla} _{i}W_{j}(t)+\hat{\nabla}_{j}W_{i}(t) \quad
\quad \hat{h}\left( 0\right) =h(0),
$$

where $\widehat{\mathit{Ric}}_{ij}$ and $\hat{\nabla}_{i}$ are the
 ...
 **Ricci-DeTurck flow** (see [DeTurck], or [ChowKnopf]
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.15" label="12.7" -->
**Lemma.** Suppose that $g(t)$ solves the Ricci flow equation and suppose that
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.16" label="dtuniqtouniq" -->
**Corollary.** Suppose that $(M,g_1(t)),\ t_0\le t\le T$, and $(M,g_2(t)),\ t_0\le
...
**Proof.** Since $\psi_{a,t}$ satisfies the equation
...
time-dependent ODE and since $\psi_{1,t_0}=\psi_{2,t_0}=\mathit{Id}$,
...
existence of the harmonic flow (12.7) and its asymptotic
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="12.17" label="cl12.20" -->
**Claim.** For any fixed $t$ the function $r\colon \Ar^n \to[0,\infty)$ is a
...
**Proof.** Write the metric $g(t)=g_{ij}dx^idx^j$ and let
...
we say that such a function is *smooth* we mean that for each
...
$$
\frac{\partial f_{nm}}{\partial t}=f_{n(m+1)}
$$

- **(3)**

$$
\frac{\partial f_{nm}}{\partial w}=f_{(n+1)m},
$$

where in Item (3) the partial derivative along the boundary $w=0$ is
a right-handed derivative only. We say such a function is *even*
if $f_{(2k+1)m}(0,t)=0$ for all $k\ge 0$.

We have the following elementary lemma:

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.18" -->
**Lemma.** (a) Suppose that $f(w,t)$ is a smooth function defined for $w\ge 0$.
...
-r$. When we say a function $f(r,t)$ *defines a smooth family of
...
### The properties of $r$ as a function of $\hat r$ and $t$
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.19" label="rcor" -->
...
$$
B(w,t)=\frac{1}{2}\int_0^w\xi(u,t)du.
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="12.20" -->
**Claim.** $f(r,t)$ is a smooth function defined for $r\ge 0$. It is an odd
function of $r$.
<!-- astrolabe:end -->
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.21" -->
**Corollary.** There is a smooth function $h(w,t)$ defined for $w\ge 0$ so that
$f(r,t)=rh(r^2,t)$. In particular, $h(r^2,t)$ defines a smooth
function on all of $\Ar^n$. Clearly, $h(w,t)>0$ for all $w\ge 0$ and
all $t$.
<!-- astrolabe:end -->

We set $\tilde h(w,t)=\mathit{log}(h(w,t))$, so that $f(r,t)=re^{\tilde
...
### The harmonic map flow equation
.

Let $\psi(t)\colon \Ar^n\to \Ar^n$ be a smoothly varying family of
smooth functions as given in Equation (12.13). Using
(12.11) and (12.13) it is easy to calculate the energy
...
letting $d\mathit{vol}_\sigma$ denote the standard volume element on
$S^{n-1}$, we have
...
\rho}f^{n-3}(r,t)w \right] dr \cdot \int_{S^{n-1}} d\mathit{vol}_\sigma \\
...
\right] dr \cdot \int_{S^{n-1}} d\mathit{vol}_\sigma\\ &=
...
$$
\delta_w\left(\frac{1}{2}\int_{\Ar^n}|\nabla_{g(t),g(t_0)}\psi|^2d\mathit{vol}=\int_{\Ar^n}\langle
w,-\triangle_{g(t),g(t_0)}\psi\rangle d\mathit{vol}\right).
$$
...
$$
 ...
 where we have written this expression
...
Equation (12.13) the harmonic map flow equation (12.7)
...
### An equation equivalent to the harmonic map flow equation

We will solve (12.15) for solutions of the form

$$
\rho(r,t)= re^{\tilde{\rho}(r,t)}, \ \ \ t\ge t_0;\ \ \ \tilde \rho(r,t_0)=0.
$$

For $\psi$ as in Equation (12.13) to define a diffeomorphism,
...
calculation shows that (12.15) becomes
...
h}{\partial w}(\rho^2,t_0) -2\xi(w,t),
\end{aligned}
$$

where $\xi$ is the function from \entryref{e353993489e9}. Then
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="12.22" -->
**Remark.** The whole purpose of this rewriting of the PDE for $\tilde \rho$ is
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="12.23" label="asympt" -->
**Claim.** For any fixed $t$ we have the following asymptotic expansions at
spatial infinity.

- **(1)** $e^{\tilde{h}(r^2,t)}$ is asymptotic to $\frac{1}{(1-t)r}$.
- **(2)** $\tilde{h}(r^2,t)$ is asymptotic to $-\log r$.
- **(3)** $\frac{\partial \tilde{h}}{\partial w}(r^2,t)$ is asymptotic to
 $-\frac{1}{2r^2}$.
- **(4)** $r^{-1} \frac{\partial r}{\partial t}$ is asymptotic to
$\frac{C}{r}$.
- **(5)** $\frac{\partial B(r^2,t)}{\partial r}$ is asymptotic to $C$.
- **(6)** $|G( \tilde{\rho},r^2,t)| \leq C_*< \infty$ where
$C_* =C_*\left(\sup\{|\tilde{\rho}|,\tilde{h}\}\right)$ is a
constant depending only on $\sup\{|\tilde{\rho}|,\tilde{h}\}$.
<!-- astrolabe:end -->

**Proof.** The first item is immediate from \entryref{77ca4669ffb3}. The
second and third follow immediately from the first. The fourth is a
consequence of the fact that by \entryref{77ca4669ffb3} $dr/dt$ is
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="12.24" -->
**Proposition.** For any $t_0\ge 0$ for which there is a standard solution $g(t)$
...
be the heat kernel. We solve (12.18) by successive approximation
[LiTam].
...
\entryref{ac4ac4fb06f8} that $G(\tilde{\rho}_{i-1},\mathbf{x},t)$ is bounded
...
and also because of \entryref{ac4ac4fb06f8} both $|\nabla B|$ and
...
$$
\begin{aligned}
|\tilde{\rho}_{i}| \leq C_1 \qquad \mathit{and} \qquad  |\nabla
\tilde{\rho}_{i}| \leq C_2.
\end{aligned}
$$

We prove the convergence of $\tilde{\rho}_i$ to a solution of
(12.18) via proving that it is a Cauchy sequence in
...
& (\tilde{\rho}_i- \tilde{\rho}_{i-1})(x,0)=   0.
\end{aligned}
$$
...
& + G( \tilde{\rho}_{i-1},\mathbf{x},t)-G(\tilde{\rho}_{i-2},\mathbf{x},t)
\end{aligned}
$$

By lengthy but straightforward calculations one can verify the
Lipschitz property of $G(\tilde{\rho},\mathbf{x},t)$
...
From Equations (12.22) and (12.23) we can estimate
...
uniformly. Hence we get from (12.19),

$$
\begin{aligned}
\tilde{\rho}_\infty=\int_0^t \int_{\mathbb{R}^{n+2}} H(x,y ,t-s)F(y,\tilde{ \rho}_{\infty},
\nabla\tilde{\rho}_{\infty} ,t)dyds
\end{aligned}
$$

The next argument is similar to the argument in [LiTam], p.21.
The function $\tilde{\rho}_{i}$ is a smooth solution of
(12.20) with $\tilde{\rho}_i(x,0)=0$. Also, both
 $\tilde{\rho}_{i}$
and $F(x,\tilde{\rho}_{i_1},\nabla \tilde{\rho}_{i-1},t)$ are
uniformly bounded on $\mathbb{R}^{n+2} \times[0,T]$. Thus, by
Theorem 1.11 [LSU], p.211 and Theorem 12.1 [LSU], p.223,
...
it is $\alpha$-Hölder continuous in space and
$\alpha/2$-Hölder continuous.

From (12.24) we conclude that $\tilde{\rho}_{\infty}$ is
a solution of (12.18) on $\mathbb{R}^{n+2} \times
...
follows from (12.21) and \entryref{ac4ac4fb06f8} that there is
a constant $C_8$ independent of $i$ such that

$$
\begin{aligned}
& | G(\tilde{\rho}_i,\mathbf{x},t) | \leq \frac{C_8}{(1+|x|)^2} \\
...
then (12.26) hold for all $i$. From the definition of
...
(n-1)\tilde{h} -B] \cdot \nabla \upsilon
+G( \tilde{\rho}_\infty,\mathbf{x},t) \\
& \upsilon(x,0)=0.
\end{aligned}
$$

From (12.25) and \entryref{ac4ac4fb06f8} we know that $\nabla [
(n-1)\tilde{h} -B+ \tilde{\rho}_\infty ]$ has
$C^{\alpha,\alpha/2}$-Hölder-norm bounded (this means
$\alpha$-Hölder norm in space and the $\alpha/2$-Hölder norm in
time). By some lengthy calculation we
 get

$$
| G(\tilde{\rho}_\infty,\mathbf{x},t)|_{C^{\alpha,\alpha/2}} \leq
\frac{C_9}{(1+|x|)^2}.
$$
...
& |\nabla G(\tilde{\rho}_\infty,\mathbf{x},t)|_{C^{\alpha,\alpha/2}}
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="12.25" label="harmapflow" -->
**Proposition.** For a standard solution $(\mathbb{R}^n,g(t)),\ 0\le t<T$, and for
any $t_0\in [0,T)$ there is a rotationally symmetric solution
$\psi_t(\mathbf{x)} = x e^{\tilde{\rho}(\mathbf{x},t)}$ to the harmonic
map flow

$$
\frac{\partial \psi_t}{\partial t}=\Delta_{g(t),g(t_0)}\psi(t)
\qquad \psi(t_0)(\mathbf{x})=\mathbf{x},
$$

and $ |\nabla^i \tilde{\rho}|(\mathbf{x},t)\leq \frac{C_{14}}{(1+|\mathbf{x}|)^2}$ for $0 \leq i \leq 3$ defined on some non-degenerate
interval $[t_0,T']$.
<!-- astrolabe:end -->

### The uniqueness for the solutions of Ricci-DeTurck flow

We prove the following general uniqueness result for Ricci-DeTurck
flow on open manifolds.

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="12.26" label="RDuniq" -->
**Proposition.** Let $\hat g_1(  t)  $ and $\hat g_2(  t),\ 0 \leq t\leq T $, be two
...
$g$, then, using the Ricci-DeTurck flow (12.10) for
...
g_2(x, t') \right\vert _{g}^{2} \leq \epsilon
$$

where $\epsilon >0$ is a constant to be chosen later.

Recall we have the initial condition $\left\vert \hat g_1( 0) -\hat
g_2(0)  \right\vert _{g}^{2}=0$. Using Equation (12.28) and applying the maximum principle to $\left\vert \hat g_1(
...
By \entryref{5fbeadbba61c} there are $\psi_1(t)$ and
...
$\hat{g}_1(t_0)$ as required in \entryref{c5e225b067f1}. It follows
from \entryref{77ca4669ffb3} and the decay estimate in
\entryref{5fbeadbba61c} that $\hat{g}_1(t)$ and $\hat{g}_2(t)$
are bounded solutions and that they have same sequential asymptotic
behavior at infinity. We can apply \entryref{c5e225b067f1} to
conclude $\hat{g}_1(t)=\hat{g}_2(t)$ on $t_0 \leq t \leq T'$. We
have proved:

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.27" label="duniq" -->
**Corollary.** Let $g_1(t)$ and $g_2(t)$ be standard solutions. Suppose that
...
**Case 3: $I$ is a closed interval $I=[0,t_0]$.**

In this case, of course, $t_0<\mathit{min}(T_1,T_2)$. Hence we apply
\entryref{5fbeadbba61c} to construction solutions $\psi_1$ and
...
### $T=1$ and existence of canonical neighborhoods

At this point we have established all the properties claimed in
\entryref{f44d182ef27e} for the standard flow except for the fact that
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="12.28" label="R_0cannbhd" -->
**Theorem.** Fix $0<\epsilon<1$. Then there is $r>0$ such that for any point
...
**Proof.** Take an increasing sequence of times $t'_n$ converging to $T$.
...
$(x_n,t_n)$. To prove the theorem we must show that $\overline\mathit{lim}_{n\rightarrow\infty}R(x_n,t_n)<\infty$. Suppose the contrary.
By passing to a subsequence we can suppose that $\mathit{lim}_{n\rightarrow \infty}R(x_n,t_n)=\infty$. We set
$Q_n=R(x_n,t_n)$. We claim that all the hypotheses of
\entryref{a9e023d082dc} apply to the sequence
$(\Ar^3,g(t),(x_n,t_n))$. First, we show that all the hypotheses of
\entryref{d51aab8a8065} (except the last) hold. Since $(\Ar^3,g(t))$
has non-negative curvature all these flows have curvature pinched
toward positive. By \entryref{ed069175cdf9} there are $r>0$ and
...
\entryref{a9e023d082dc} (which includes the last hypothesis of
\entryref{d51aab8a8065}) is satisfied. This is clear since
$t_n\rightarrow T$ as $n\rightarrow \infty$ and $Q_n\rightarrow
\infty$ as $n\rightarrow \infty$. Applying \entryref{a9e023d082dc} we
conclude that after passing to a subsequence there is a limiting
flow which is a $\kappa$-solution. Clearly, this and
\entryref{e724298e5bc3} imply that for all sufficiently large
...
### Completion of the proof of Theorem \entryref{f44d182ef27e}

The next proposition establishes the last of the conditions claimed
in \entryref{f44d182ef27e}.

<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="12.29" -->
**Theorem.** For the standard flow $T=1$.
<!-- astrolabe:end -->

**Proof.** We have already seen in \entryref{2db418fba1c1} that $T\le 1$. Suppose now
that $T<1$. Take $T_0<T$ sufficiently close to $T$. Then according
to \entryref{77ca4669ffb3} there is a compact subset $X\subset
...
X$. Because of \entryref{85e3c925177e} and the definition of
$(C(\epsilon),\epsilon)$-canonical neighborhoods, it follows that at
any point $(x,t)$ with $R(x,t)\ge r^{-2}$, where $r>0$ is the
constant given in \entryref{85e3c925177e}, we have $\partial
...
\entryref{e0b6a1f646a7} and the fact that the standard flow is
...
$(\Omega,g(t)|_{\Omega})$. Of course, $\mathit{lim}_{t\rightarrow
T}V(t)=\mathit{Vol}_{g(T)}(\Omega)>0$.
...
exist. This means that $\overline\mathit{lim}_{t\rightarrow
...
\entryref{85e3c925177e} the second hypothesis in the statement of
\entryref{d51aab8a8065} holds for the sequence
$(\Ar^3,g(t),(x,t_n))$. All the other hypotheses of this theorem as
well as the extra hypothesis in \entryref{a9e023d082dc} obviously
hold for this sequence. Thus, according to \entryref{a9e023d082dc}
the based flows $(\Ar^3,Q_ng(Q_n^{-1}t'+t_n),(x,0))$ converge
smoothly to a $\kappa$-solution. Since the asymptotic volume of any
$\kappa$-solution is zero (see \entryref{7468282bd017}), we see that
for all $n$ sufficiently large, the following holds:

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="12.30" -->
**Claim.** For any $\epsilon>0$, there is $A<\infty$ such that for all $n$
sufficiently large we have

$$
\mathit{Vol}(B_{Q_ng}(x,t_n,A))<\epsilon A^3.
$$
<!-- astrolabe:end -->
...
\entryref{51a2afee4465} that the distance is a non-increasing
function of $t$, so that for all $t\in [0,T)$ we have $\Omega\subset
B(x,t,A_1)$. Applying the above, for any $\epsilon>0$ for all $n$
sufficiently large we have

$$
\mathit{Vol} (\Omega,g(t_n))\le \mathit{Vol}_g B(x,t_n,A_1)<\epsilon
(A_1)^3.
$$
...
$$
\mathit{lim}_{n\rightarrow
\infty}\mathit{Vol} \Omega,g(t_n)=\mathit{Vol} (\Omega,g(T))>0.
$$

 This
contradiction proves that $T=1$.

This completes the proof of \entryref{f44d182ef27e}.
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="12.31" label="Restim" -->
**Proposition.** There is a constant $c>0$ such that for all $(p,t)$ in the standard
solution we have
...
**Proof.** First, let us show that there is not a limiting metric $g(1)$
...
\entryref{0ddfe578961e} we see that the curvature of $g(T)$ must be
unbounded at spatial infinity. On the other hand, by
\entryref{7c2758f0e2b3} every point of $(\Ar^3,g(1))$ of
...
contradicts \entryref{cb9ca6d6db9f}. (One can also rule this
...
The next step is to see that for any $p\in \Ar^3$ we have $\mathit{lim}_{t\rightarrow 1}R(p,t)=\infty$. Let $\Omega\subset \Ar^3$ be
the subset of $x\in \Ar^3$ for which $\mathit{liminf}_{t\rightarrow
1}R(x,t)<\infty$. We suppose that $\Omega\not=\emptyset$. According
to \entryref{af00774dbaa9} the subset $\Omega$ is open and the metrics
...
is $p\in \Ar^3$ with $\mathit{lim}_{t\rightarrow 1}R(p,t)=\infty$.
Take a sequence $t_n$ converging to $1$ and set $Q_n=R(p,t_n)$. By
\entryref{a9e023d082dc} we see that, possibly after passing to a
subsequence, the based flows $(\Ar^3,Q_ng(t'-t_n),(p,0))$ converge
to a $\kappa$-solution. Then by \entryref{7468282bd017} for any
$\epsilon>0$ there is $A<\infty$ such that $\mathit{Vol} B_{Q_ng}(p,t_n,A)<\epsilon A^3$, and hence after rescaling we
have $\mathit{Vol} B_g(p,t_n,A/\sqrt{Q_n})<\epsilon (A/\sqrt{Q_n})^3$.
By the Bishop-Gromov inequality (\entryref{1a1b2c4fdc9b}) it
follows that for any $0<A<\infty$, any $\epsilon>0$ and for all $n$
sufficiently large, we have $\mathit{Vol} B_g(p,t_n,A)<\epsilon A^3$.
Take a non-empty subset $\Omega'\subset \Omega$ with compact
closure. Of course, $\mathit{Vol} (\Omega',g(t))$ converges to $\mathit{Vol} (\Omega',g(T))>0$ as $t\rightarrow T$. Then there is
...
$\mathit{Vol} (\Omega',g(t))<\epsilon A^3$. This completes the proof
that for every $p\in \Ar^3$ we have $\mathit{lim}_{t\rightarrow
1}R(p,t)=\infty$.
...
$$
R(p,t)<\mathit{max}\left(\left[(C'-C)(1-t_0)\right]^{-1},\left[r^{-2}-C(1-t_0)\right]^{-1}\right).
$$
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="12.32" label="stdsolncannbhd" -->
**Theorem.** For any $\epsilon>0$ there is $C'(\epsilon)<\infty$ such that for
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="12.33" -->
**Remark.** At first glance it may seem impossible for a point $(x,t)$ in the
...
**Proof.** By \entryref{85e3c925177e}, there is $r_0$ such that if $R(x,t)\ge
...
for rescaled time $(1+\epsilon)$. By \entryref{ffae4504318a}, there
is $\theta<1$ such that if $R(x,t)\le r_0^{-2}$ then $t\le \theta$.
By \entryref{77ca4669ffb3}, there is a compact subset $X\subset \Ar
...
the core of a $(C',\epsilon)$-cap.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.34" label="stdsolncap" -->
**Corollary.** Fix $\epsilon>0$. Suppose that $(q,t)$ is a point in the standard
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="12.35" -->
**Remark.** Recall that $p_0$ is the origin in $\Ar^3$ and hence is the tip of
the surgery cap. Also, $A_0$ is defined in \entryref{0b07beec84c0}.
<!-- astrolabe:end -->

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="12.36" label="stdsolnlimit" -->
**Corollary.** For any $\epsilon>0$ let $C'=C'(\epsilon)$ be as in
\entryref{6ff28dba4e56}. Suppose that we have a sequence of
generalized Ricci flows $({\mathcal M}_n,G_n)$, points $x_n\in
{\mathcal M}_n$ with $\mathbf{t}(x_n)=0$, neighborhoods $U_n$ of $x_n$
...
- **(1)** $y_n$ is contained in the core of a
$(C'(\epsilon),\epsilon)$-cap
- **(2)** $y_n$ is the center of a strong $\epsilon$-neck
- **(3)** $y_n$ is the center of an evolving $\epsilon$-neck whose
initial time-slice is at time $0$.
<!-- astrolabe:end -->

**Proof.** This follows immediately from \entryref{6ff28dba4e56} and
\entryref{7c2758f0e2b3}.
...
\entryref{f1d3021d32b8} we have the following:

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="12.37" label="decrease" -->
**Lemma.** Let $S^2$ be the unit sphere in $T_0\Ar^3$. Equip it with the metric
...
**Proof.** Clearly, the metric $\rho^*g_0$ is rotationally symmetric and its
