...
the constants $r_i$, $C$ and $\epsilon$, contained in $\mathbf{t}^{-1}([T_{i-1},T_i))$ and on which the curvature is bounded by
...
\entryref{3defa9b2209c} we transfer this conclusion to a non-collapsing
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="16.1" label="KAPPALIMIT" -->
**Proposition.** Suppose that for some $i\ge 0$ we have surgery parameter sequences $\delta_0\ge
...
\entryref{703e4cfeece7} with respect to the given sequences. Suppose also that the
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="16.2" -->
**Remark.** Implicitly,
...
## The proof of non-collapsing when $R(x)=r^{-2}$ with $r\le r_{i+1}$
...
satisfying the hypothesis of \entryref{4ce8b5da9c58}. Here is
the result that establishes the non-collapsing in this case.

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="16.3" -->
**Proposition.** Let $x\in {\mathcal M}$ with $\mathbf{t}(x)=t$ and with $R(x)=r^{-2}\ge r_{i+1}^{-2}$. Then there is $\kappa>0$ depending only
...
supremum of $|\mathit{Rm}|$ on $B(x,t,r')$ is $(r')^{-2}$.
Then, by the
definition
 of a $(C,\epsilon)$-cap,
$\mathit{vol} B(x,t,r')\ge C^{-1}(r')^3$.
...
## Minimizing ${\mathcal L}$-geodesics exist when $R(x)\le r^{-2}_{i+1}$: the statement
...
flows given in Chapter 8. That is to say, in this case
...
unlike the case of Ricci flows treated in Chapter 8,
...
hypothesis of \entryref{4ce8b5da9c58} with respect to the given
...
Chapter 6 to show that for each $t\in [T_{i-1},T)$
...
Given \entryref{89a60eb69c3f}, arguments from
Chapter 8 will be applied to complete the proof of
\entryref{4ce8b5da9c58}.

## Evolution of neighborhoods of surgery caps

 We begin this analysis required to prove \entryref{89a60eb69c3f}
 by studying the
evolution of surgery caps.
\entryref{ad4f67ea75bc} below is the main result along these lines.
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="16.5" label="altern" -->
**Proposition.** Given $A<\infty$, $\delta''>0$ and $0<\theta<1$, there is
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.6" -->
...
**Proof.** Since $(N,g_N)$ is a strong $\delta'$-neck of scale $h$, there is a
...
(\entryref{ed4b72caf9f9}). The bound on the $k^{th}$-derivatives of the
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="16.7" -->
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="16.8" -->
...
\entryref{f44d182ef27e} that in fact the limiting flow is
...
to \entryref{0ddfe578961e} there is an $\eta>0$ and a constant
...
[0,\mathit{min}(\theta'+\eta,\theta_n))$ is bounded by $Q'$. Because of
...
(\entryref{e0b6a1f646a7}), it follows that there are uniform bounds
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="16.9" label="goodtotheta" -->
**Corollary.** With the notation and assumptions of the previous lemma, for all
...
\entryref{0ddfe578961e}, we see that the scalar curvature of $G_n'$
...
(\entryref{e0b6a1f646a7}), we see that there are uniform bounds on
...
Now we turn to the proof proper of \entryref{ad4f67ea75bc}. We fix
$A<\infty$, $\delta''>0$ and $\theta<1$. We are free to make $A$
larger so we can assume by \entryref{77ca4669ffb3} that for the
...
\entryref{712124d7ea02}, for all $n$ sufficiently large, the
...
the time interval $[0,\theta_n)$. If $\theta_n=\mathit{min}(\theta,(T_n'-t_n)/h_n^2)$, then the first conclusion of
\entryref{ad4f67ea75bc} holds for $({\mathcal M}_n,G_n)$ for all
...
M}_n,G_n)$. If on the other hand $\theta_n<\mathit{min}(\theta,(T_n'-t_n)/h_n^2)$, we need only show that all of
$B(x_n,t_n,Ah_n)$ disappears at time $t_n+h_n^2\theta_n$ in order to
show that the second conclusion of \entryref{ad4f67ea75bc} holds
...
So now let us suppose that $\theta_n<\mathit{min}(\theta,(T'_n-t_n)/h_n^2)$. Since there is no further extension
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.10" -->
**Claim.** Suppose that $\theta_n<\mathit{min}(\theta,(T'_n- t_n)/ h_n^2)$. Let
...
solution. Using \entryref{712124d7ea02} we see that for all $n$
...
\entryref{712124d7ea02} there is an $\epsilon$-neck of rescaled
diameter approximately $Ah_n/2$ contained in $\rho_n(B(p_n,t_n,A
h_n)\times \{t\})$. By \entryref{98c6a95f8556} the spheres coming
from the neck structure in
...
t_n+ h_n^2\theta_n$. This shows that if $\theta_n<\mathit{min}(\theta,T'_n- t_n/h_n^{2})$, and if there is no extension of
...
\entryref{ad4f67ea75bc} holds, giving a contradiction.

This completes the proof of \entryref{ad4f67ea75bc}.

<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="16.11" -->
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="16.12" label="defnoverlinedelta0" -->
**Definition.** Let $c>0$ be the constant from \entryref{ffae4504318a}. Fix
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="16.13" label="bigell" -->
**Proposition.** For any $\ell<\infty$ there is $A_0=A_0(\ell)<\infty$,
$0<\theta_0=\theta_0(\ell)<1$, and for any $A\ge A_0$ for the
constant
$\delta''=\delta''(A)=\delta''_0(A,\theta_0,\overline\delta_0)>0$
from \entryref{ad4f67ea75bc} the following holds. Suppose that
...
to \entryref{ffae4504318a}, for all $x\in \Ar^3$ and all $t\in
[0,1)$ we have $R_{g_0}(x,t)\ge c/(1-t)$. By \entryref{ad4f67ea75bc} and
...
& \ge & \int_{t'}^{\theta_0}
\frac{c}{2(1-t)}dt \\
& = & \frac{-c}{2}\left(\mathit{log}(1-\theta_0)-\mathit{log}(1-t')\right)dt \\
& \ge &  \frac{-c}{2}\left(\mathit{log}(1-\theta_0)+\mathit{log}(2)\right).
\end{aligned}
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.14" -->
...
**Proof.** This is immediate from \entryref{77ca4669ffb3} and the fact that
$\theta_0<1$.

Now fix $A_0=\mathit{max}(A_0', 10\sqrt{\ell})$ and let $A\ge A_0$.

Since $\overline\delta_0<1/4$ and since $T'\le \theta_0$, for $\overline\delta(\bar t)\le
\delta''_0(A,\theta_0,\overline\delta_0)$
by \entryref{ad4f67ea75bc}
...
$$
\mathit{Ric}_g(X,X)\ge -\frac{1}{4}|X|_{g_0}^2\ge -\frac{1}{2}|X|^2_g,
$$
...
$$
 ...
 Since $T'-t'<1$, it
immediately follows from this and Equation (16.1) that
...
$$
 ...
 Since $A\ge A_0\ge 10\sqrt{ \ell}$, this
expression is $>\ell$. This completes the proof of
\entryref{0a4d6f17810d}

### Paths with short ${\mathcal L}_+$-length avoid the surgery caps
...
with surgery, and let $x\in {\mathcal M}$ be a point with $\mathbf{t}(x)=T\in (T_i,T_{i+1}]$. We suppose that these data satisfy the
hypothesis of \entryref{89a60eb69c3f} with respect to the given
sequences and $r\ge r_{i+1}>0$. In particular, the parabolic
neighborhood $P(x,T,r,-r^2)$ exists in ${\mathcal M}$ and $|\mathit{Rm}|$ is
...
function. We set $R_+(y)=\mathit{max}(R(y),0)$.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="16.15" label="L_0" -->
**Lemma.** Given $L_0<\infty$, there is
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="16.16" -->
...
**Proof.** We define $\ell=L_0/\sqrt{\tau'}$, then define $A=\mathit{max}(A_0(\ell),2(50+A_0))$ and $\theta=\theta_0(\ell)$. Here,
$A_0(\ell)$ and $\theta_0(\ell)$ are the constants in
\entryref{0a4d6f17810d}.
 Lastly, we require $\overline\delta_1\le \delta''(A)$ from
\entryref{0a4d6f17810d}. Notice that, by construction,
 $\delta''(A)=\delta''_0(A,\theta,\overline\delta_0)$ from
\entryref{ad4f67ea75bc}. Thus, if $p$ is the tip of a surgery cap at time $\bar t$
...
that $P(x,T,r,-r^2)$ exists in ${\mathcal M}$ and that $|\mathit{Rm}|\le
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.17" -->
**Claim.** $\int_0^{\tau''}|X_\gamma(\tau)|d\tau> r/2\sqrt{2}$.
<!-- astrolabe:end -->

**Proof.** Since $|\mathit{Rm}|\le r^{-2}$ on $P(x,T,r,-r^2)$, we have $|\mathit{Ric}|\le
...
$$
\mathit{exp}(-2r^{-2}\tau'')\langle v,v\rangle_{G(T)}\le \langle
v,v\rangle_{G(T-\tau)} \le \mathit{exp}(2r^{-2}\tau'')\langle
v,v\rangle_{G(T)}.
$$

 Since $\tau''\le \tau'$ and $r\ge r_{i+1}$ by
the assumption on $\tau'$ we have

$$
\mathit{exp}(2r^{-2}\tau'')\le \mathit{exp}(2\sqrt[3]{2})< 2.
$$
...
\begin{aligned}
\nonumber
\int_0^{\tau_0}\sqrt{\tau}\left(R_+(\gamma(\tau))+|X_\gamma(\tau)|^2\right)d\tau & & \\
& \ge &
...
parabolic neighborhood $P(p,\bar t,A\bar h,s)$ embedded in $\mathbf{t}^{-1}((-\infty,T])\subset {\mathcal M}$. Let $\Delta t_1=\mathit{min}(\theta \bar h^2,\Delta t)$. We consider $P(p,\bar t,A\bar
...
\entryref{ad4f67ea75bc}, at the time $\bar t+\Delta t_1$ there is a
...
\entryref{0a4d6f17810d} and the fact that $R\ge 0$ on $P(p,\bar t,A\bar
...
$$
 ...
 which contradicts our hypothesis. This
completes the proof of \entryref{86043191a3c2}.

### Paths with small energy avoid the disappearing regions
...
M},G)$ satisfying the hypothesis of \entryref{89a60eb69c3f}, in
this case the estimates will depend on the Ricci flow with surgery.
First, let us fix some notation.

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="16.18" label="J" -->
**Definition.** Suppose that $\bar t$ is a surgery
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="16.19" label="15.17" -->
**Lemma.** Fix a Ricci flow with surgery $({\mathcal M},G)$, a point $x\in
{\mathcal M}$ and constants $r\ge r_{i+1}>0$ as in the statement of
\entryref{89a60eb69c3f}. For any $1<\ell<\infty$ the following
...
$$
\gamma(0)\in
M_{\bar t}\setminus \cup_iB(p_i,\bar t,(50+A_0)\bar h),
$$
...
### Limits of a sequence of paths with short ${\mathcal L}$-length

Now using Lemmas \entryref{86043191a3c2} and \entryref{5ed7c639bd97} we show that it is possible to take limits
of certain types of sequences of paths parameterized by backward time to create
minimizing ${\mathcal L}$-geodesics.

We shall work with a compact subset of $\mathbf{t}^{-1}([T_{i-1},T])$ that is
obtained by removing appropriate open neighborhoods of the exposed regions.

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="16.20" -->
**Definition.** Fix $\ell<\infty$. Let $\theta_0=\theta_0(\ell)$ be as in
\entryref{0a4d6f17810d}. For each surgery time $\bar t\in
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="16.21" label="Lgeoexist" -->
**Proposition.** Fix $0<L<\infty$. Set
...
\entryref{86043191a3c2}. Suppose that $\gamma_n$ is a sequence of paths in
...
as in \entryref{86043191a3c2} and then define $\ell=L_0/\sqrt{\tau'}$. We
also let $A=\mathit{min}(2(50+A_0),A_0(\ell))$ and
$\theta_0=\theta_0(\ell)$ as in \entryref{0a4d6f17810d}. Lastly, we
let
$\overline\delta_1(L_0,r_{i+1})=\delta''(A)=\delta''(A,\theta_0,\overline\delta_0)$
from Propositions \entryref{0a4d6f17810d} and \entryref{ad4f67ea75bc}. We suppose that
...
[\bar t,\bar t+\mathit{min}(\bar h^2/2,\Delta t({\mathcal C})))\right).
$$

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.22" -->
**Claim.** Any path $\gamma$ beginning at $x$ and parameterized by backward
time misses $P_0({\mathcal C})$ if ${\mathcal L}(\gamma)<L$.
<!-- astrolabe:end -->

**Proof.** Set $\tau_0=T-\bar t$. Of course, $\tau_0\le T-T_{i-1}\le T_{i+1}-T_{i-1}$.
Consider the restriction of $\gamma$ to $[0,\tau_0]$.
We have

$$
\begin{aligned}
\int_0^{\tau_0}\sqrt{\tau}\left(R_+(\gamma_n(\tau))+|X_{\gamma_n}(\tau)|^2\right)d\tau
  \\
& \le &
...
The claim now follows immediately from \entryref{86043191a3c2}.
...
\entryref{86043191a3c2} that the restriction of the path $\gamma$ to $[0,\tau']$ lies
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.23" -->
...
The claim is now immediate from \entryref{5ed7c639bd97}.
...
\entryref{d98ad25d9bc3}.
 By \entryref{86043191a3c2} the
restriction of each $\gamma_n$ to the interval $[0,\mathit{min}(\bar\tau,\tau')]$ is contained in $P(x,T,r/2,-r^2)$. The
arguments in the proof of \entryref{cd062526b91a} (which involve
changing variables to $s=\sqrt{\tau}$) show that, after passing to a
subsequence, the restrictions of the $\gamma_n$ to $[0,\mathit{min}(\bar\tau,\tau')]$ converge uniformly to a path $\gamma$ defined
on the same interval. Furthermore,

$$
\int_0^{\mathit{min}(\bar\tau,\tau')}\sqrt{\tau}|X_\gamma(\tau)|^2d\tau\le \mathit{liminf}_{n\rightarrow\infty}\int_0^{\mathit{min}(\bar\tau,\tau')}\sqrt{\tau}|X_{\gamma_n}(\tau)|^2d\tau,
$$
...
$$
\begin{aligned}
\int_0^{\mathit{min}(\bar\tau,\tau')}\sqrt{\tau}\left(R(\gamma(\tau)+|X_\gamma(\tau)|^2\right)d\tau\le \\
& & \mathit{liminf}_{n\rightarrow\infty}\int_0^{\mathit{min}(\bar\tau,\tau')}\sqrt{\tau}\left(R(\gamma_n(\tau))+|X_{\gamma_n}(\tau)|^2\right)d\tau.\nonumber
\end{aligned}
$$
...
$$
Y=\mathbf{t}^{-1}([T-t',T-\bar t])\setminus
\left(J(\bar t-\tau_1(\bar t),\bar t)\cup (E(\bar t)\cup J_0(\bar
t))\right).
$$

 This is a compact subset with the property that any
...
$$
 ...
 Thus, after passing to a subsequence, the $\gamma_n$ converge
...
$$
\int_{T-\bar t}^{T-t'}|X_{\gamma}(\tau)|^2d\tau\le \mathit{liminf}_{n\rightarrow
...
\mathit{liminf}_{n\rightarrow\infty}\int_{\tau'}^{\bar\tau}\sqrt{\tau}\left(R(\gamma_n(\tau))+
...
Inequalities 16.5 and 16.6 we see that

$$
{\mathcal L}(\gamma)\le \mathit{liminf}_{i\rightarrow \infty}{\mathcal
L}(\gamma_n).
$$
...
have the same endpoint $y\in M_{T-\tau_0}$ and that $\mathit{lim}_{n\rightarrow\infty}{\mathcal L}(\gamma_n)$ is less than or equal
...
L}(\gamma)\le \mathit{lim}_{n\rightarrow\infty}{\mathcal L}(\gamma_n)$.
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="16.24" label="bardelta1" -->
**Corollary.** Given $L<\infty$, let
$\overline{\delta}_1=\overline\delta_1(L+4(T_{i+1}^{3/2},r_{i+1})$ be as given in \entryref{86043191a3c2}.
If
...
### Completion of the proof of Proposition \entryref{89a60eb69c3f}
...
too large, we are in a position to prove \entryref{89a60eb69c3f}, which states
the existence of a minimizing ${\mathcal L}$-geodesics in ${\mathcal M}$ from $x$
and gives estimates on their ${\mathcal L}$-lengths.

**Proof.** (of \entryref{89a60eb69c3f}).
Fix $r\ge r_{i+1}>0$. Let $({\mathcal M},G)$ and $x\in {\mathcal M}$ be
as in the statement of \entryref{89a60eb69c3f}. We set
$L=8\sqrt{T_{i+1}}(1+T_{i+1})$, and we set

$$
\delta=\mathit{min}\left(\delta_i,\overline \delta_1(L+4(T_{i+1})^{3/2},r_{i+1})\right),
$$

where $\overline\delta_1$ is as given in \entryref{86043191a3c2}.
Suppose that $\overline\delta(t)\le
\delta$ for all $t\in [T_{i-1},T_{i+1})$. We set $U$ equal to the
subset of $\mathbf{t}^{-1}([T_{i-1},T))$ consisting of all points $y$
...
\entryref{ae8837c505cc} for any $y\in U$ there is a minimizing
...
\entryref{d98ad25d9bc3}, we can pass to a subsequence that
converges to a limit, $\gamma$, connecting $x$ to some point $y\in
M_t$ and ${\mathcal L}(\gamma)\le \mathit{inf}_{n}{\mathcal L}(\gamma_n)<L$. Hence, $y\in
...
${\mathcal L}_x(y)\le {\mathcal L}_x(y')$ for all $y'\in  U_{\mathbf{t}(y)}$.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.25" -->
...
**Proof.** Fix a compact interval $I\subset [T_{i-1},T)$. Let $\{z_n\}$ be a
sequence in $Z'\cap \mathbf{t}^{-1}(I)$. By passing to a subsequence we
can assume that the sequence $\mathbf{t}(z_n)=t_n$ converges to some
...
M_t$. Clearly,
 $\mathit{lim}_{n\rightarrow\infty }z_n=y$.

It is also the case that $\mathit{lim}_{n\rightarrow\infty}{\mathcal L}(\widehat\gamma_n)=
\mathit{lim}_{n\rightarrow\infty}{\mathcal L}(\gamma_n)=D\le L/2$. This
...
$Z'\cap \mathbf{t}^{-1}(I)$ is compact, establishing the claim.

At this point we have established that Properties (1),(2), and (4);
So it remains only to prove Property (3) of \entryref{89a60eb69c3f}.
 To do this we define the reduced length function
$l_x\colon U\to \Ar$ by

$$
l_x(q)=\frac{{\mathcal L}_x(q)}{2\sqrt{T-\mathbf{t}(q)}} \ \ \mathit{and} \ \
l_x^\mathit{min}(\tau)=\mathit{min}_{q\in M_t}l_x(q).
$$

 We consider the
subset ${\mathcal S}$ of $\tau'\in (0,T-T_{i-1}]$ with $l_x^\mathit{min}(\tau)\le L/2$ for all $\tau\le \tau'$. Recall that by the
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="16.26" label="lvalue" -->
**Lemma.** Let $l_x^\mathit{min}(\tau')$ be the minimum value of $l_x$ on
...
\entryref{89a60eb69c3f}, this is immediate from
\entryref{71669e7fd5ec}.
...
claim, we have $l_x^\mathit{min}(\tau)\le 3/2$ so that there is an ${\mathcal
...
point $z\in M_t$. Clearly,

$$
{\mathcal L}_x(z)\le \mathit{limsup}_{n\rightarrow\infty}{\mathcal L}_x(z_n)\le 3\sqrt{T-t},
$$
...
\entryref{89a60eb69c3f}. This completes the proof of that proposition.

## Completion of the proof of Proposition \entryref{4ce8b5da9c58}

Now we are ready to establish \entryref{4ce8b5da9c58}, the
...
bounded. Recall that in Section 16.2 we established it when
$R(x)=r^{-2}$ with $r\le r_{i+1}<\epsilon$. Here we assume that
$r_{i+1}<r\le \epsilon$. Fix $\delta=\delta(r_{i+1})$ from
\entryref{89a60eb69c3f} and set $L=8\sqrt{T_{i+1}}(1+T_{i+1})$.

First of all, in \entryref{f5e180ecfd18} we have seen that there
is $\kappa_0$ such that $\mathbf{t}^{-1}[0,T_1]$ is $\kappa_0$
non-collapsed on scales $\le \epsilon$. Thus, we may assume that
$i\ge 1$.

Recall that $\mathbf{t}(x)=T\in (T_{i},T_{i+1}]$. Let $\gamma$ be an
...
proved in \entryref{89a60eb69c3f}. We shall find a point $y$ on
...
we can apply the results from Chapter 8 to establish the
$\kappa$ non-collapsing.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.27" label="tau0cl" -->
**Claim.** There is $\tau_0$ with $\mathit{max}(\epsilon^2,T-T_i)\le \tau_0\le
T-T_{i-1}-\epsilon^2$ such that $R(\gamma(\tau_0))< r_i^{-2}$.
<!-- astrolabe:end -->

**Proof.** Let $T'=\mathit{max}(\epsilon^{2},T-T_i)$ and let
...
\ge \frac{2}{3}r_i^{-2}\left((T'')^{3/2}-(T')^{3/2}\right).
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="16.28" -->
...
**Proof.** Since $T_i-T_{i-1}\ge t_0$ and $T\ge T_i$, we see that
...
Now fix $\tau_0$ satisfying \entryref{8aac9c08c058}.
 Let $\gamma_1$ be the
...
define $\Delta=\mathit{min}(r_i^2/16C,\epsilon^2)$. According to
\entryref{0ddfe578961e} every point $z$ on a backward flow line
...
\overline \delta\le \delta_0\le \mathit{min}(D^{-1},1/10)$, it follows
...
Set $\hat r=\mathit{min}(\frac{r_i}{32C},\Delta_1/2)$, so that $\hat r$ depends
...
$$
P(y,t'',\hat r,-\Delta_1)\subset P(y,t',\frac{r_i}{2 C},-\Delta),
$$

 so that the
Riemann curvature is bounded above by $C'$ on the parabolic
neighborhood $P(y,t'',\hat r,-\Delta_1)$. We set $r'=\mathit{min}(\hat r,(C')^{-1/2},\sqrt{\Delta_1}/2)$, so that $r'$ depends only on
$r_i$, $C$, and $\epsilon$. Then the parabolic neighborhood
$P(y,t'',r',-(r')^2)$ exists in ${\mathcal M}$ and $|\mathit{Rm}|\le
...
Let us consider the other possibility when $\mathit{Vol} B(y,t'',r')\ge
\kappa_i(r')^3$.
 For each $z\in B(y,t'',r')$ let

$$
\mu_z\colon [T-t',T-t'']\to  B(y,t',\frac{r_i}{2C})
$$
...
|X_{\mu_z}(\tau)|_{G(T-\tau)}\le
 \frac{\sqrt{2}r'}{\Delta_1}
$$

 for all $\tau\in [T-t',T-t'']$.
...
Putting this together with the estimate, Equation (16.7),
for ${\mathcal L}(\gamma_1)$ tells us that for each $z\in
B(y,t'',r')$ we have

$$
{\mathcal L}(\gamma_1*\tilde\mu_z)\le
4(T_{i+1})^{1/2}+4(T_{i+1})^{3/2}\le L/2.
$$

 Hence, by
\entryref{89a60eb69c3f} and the choice of $L$, there is a
...
\entryref{d98ad25d9bc3} there is a compact subset $Y$ of
the open subset of smooth points of ${\mathcal M}$ that contains all
the minimizing ${\mathcal L}$-geodesics from $x$ to points of
$B(y,t'',r')$.

Then, by \entryref{7aad50e49af2} (see also,
\entryref{db4ced24b05f}), the intersection, $B'$, of ${\mathcal U}_x$
with $B(y,t'',r')$
 is an open subset of full measure in $B(y,t'',r')$.
Of course, $\mathit{Vol} B'=\mathit{Vol} B(y,t'',r')\ge \kappa_i(r')^3$
and the function $l_x$ is bounded by $L/2$ on $B'$. It now follows
from \entryref{3defa9b2209c} that there is $\kappa>0$ depending only on
...
\entryref{86043191a3c2}. Since $L$ depends only on $i$ and $t_0$, this shows
