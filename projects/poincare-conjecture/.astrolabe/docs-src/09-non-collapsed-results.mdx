...
## A non-collapsing result for generalized Ricci flows

The main result of this chapter is a
$\kappa$-non-collapsed result.

<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="8.1" label="THM" -->
**Theorem.** Fix positive constants $\bar\tau_0<\infty$, $l_0<\infty$, and $V>0$.
...
$$
 ...
 By the
monotonicity result (\entryref{6a52537bef2f}) it follows that for any
$\tau\le \tau_0$, and in particular for any $\tau\le r^2$, we have

$$
\widetilde V_x(W(\tau))\ge
\bar\tau_0^{-n/2}Ve^{-l_0}.
$$

Let $\varepsilon=\sqrt[n]{\mathit{Vol}(B(x,T,r))}/r$, so that $\mathit{Vol} B(x,T,r)=\varepsilon^nr^n$. The basic result we need to
establish in order to prove this theorem is the following:

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="8.2" label="step" -->
...
\frac{1}{3^{2/n}\bar\tau_0}V^{2/n}e^{-2l_0/n}.
$$

 Since
$\kappa=\varepsilon^n$, this proves the theorem.

**Proof.** We divide $\widetilde W$
into

$$
\widetilde W_\mathit{sm}=\widetilde W\cap \left\{Z\in
...
$$
\widetilde W_\mathit{lg}=\widetilde W\setminus \widetilde W_\mathit{sm},
$$

(see Fig. 8.2).

We set $W_\mathit{sm}(\tau_1)={\mathcal L}\mathit{exp}_x^{\tau_1}(\widetilde W_\mathit{sm})$ and $W_\mathit{lg}(\tau_1)={\mathcal L}\mathit{exp}_x^{\tau_1}(\widetilde W_\mathit{lg}).$ Clearly, since $W(\tau_1)$ is the union of $W_\mathit{sm}(\tau_1)$ and $W_\mathit{lg}(\tau_1)$ and since these subsets are
disjoint measurable subsets, we have

$$
\widetilde V_x(W(\tau_1))=\widetilde V_x(W_\mathit{sm}(\tau_1))+\widetilde V_x(W_\mathit{lg}(\tau_1)).
$$

 We shall show that there is $\varepsilon_0$ such that either
$\varepsilon>\varepsilon_0$ or $\widetilde V_x(W_\mathit{sm}(\tau_1))\le
2\varepsilon^{n/2}$ and $\widetilde V_x(W_\mathit{lg}(\tau_1))\le
\varepsilon^{n/2}$. This will establish \entryref{12d93b430400} and hence
\entryref{3defa9b2209c}.

### Upper bound for $\widetilde V_x(W_\mathit{sm}(\tau_1))$

The idea here is that ${\mathcal L}$-geodesics with initial vector
in $\widetilde W_\mathit{sm}$ remain in the parabolic neighborhood
...
the reduced volume of $W_\mathit{sm}(\tau_1)$ would be arbitrarily
small.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="8.3" label="small" -->
**Lemma.** Setting $\tau_1=\varepsilon r^2$, there is a constant
...
$$
\widetilde V_x(W_\mathit{sm}(\tau_1))=\int_{W_\mathit{sm}(\tau_1)}\tau_1^{-n/2}e^{-l(q,\tau_1)}d\mathit{vol}_{g(\tau_1)}=\int_{\widetilde W_\mathit{sm}}\tau_1^{-n/2}e^{-\tilde
l(Z,\tau_1)}{\mathcal J}(Z,\tau_1)dZ,
$$

 so that it will follow
immediately from the lemma that:

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="8.4" -->
**Corollary.** There is a constant $\varepsilon_0>0$ depending on $\bar\tau_0$
...
**Proof.** *(Of \entryref{26e1a6292fef})*
In order to establish \entryref{26e1a6292fef} we need two preliminary
estimates:

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="8.5" label="nabR" -->
**Claim.** There is a universal positive constant $\varepsilon'_0$ such that, if
...
$$
(1-C_1\varepsilon)\le \frac{g(y,t)}{g(y,T)}\le (1+C_1\varepsilon).
$$
<!-- astrolabe:end -->

**Proof.** Recall that by hypothesis $|\mathit{Rm}(y,t)|\le 1/r^2$ on
...
$|\mathit{Rm}|\le 1$ on the entire parabolic neighborhood
$B(x,T,1)\times [T-1,T]$. Then according to \entryref{ed4b72caf9f9} there
...
$\varepsilon'_0\le 1/2$ gives the first item in the claim.
 ...
 Since $|\mathit{Ric}|\le (n-1)/r^2$ for all
...
W_\mathit{sm}$ the ${\mathcal L}$-geodesic $\gamma_Z$ (the one having
$\mathit{lim}_{\tau\rightarrow 0}\sqrt\tau X_{\gamma_Z}(\tau)=Z$)
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="8.6" -->
**Claim.** There is a
...
**Proof.** First we make sure that $\varepsilon_0$ is less than or equal to the
universal constant $\varepsilon_0'$ of the last claim. For all
$(y,t)\in B(x,T,r)\times [T-r^2,T]$ we have $|\mathit{Rm}(y,t)|\le
...
$C_1/r^3>(n-1)r^{-2}\ge |\mathit{Ric}(y,t)|$ for all $(y,t)\in
B(x,T,r)\times [T-r^2,T]$. Thus, we can take the constant $C_0$ in
the hypothesis of \entryref{333be2e58780} to be $C_1/r^3$. We take the
constant $\bar\tau$ in the hypothesis of that lemma to be
$\varepsilon r^2$. Then, we have that

$$
\mathit{max}_{0\le \tau\le \tau_1'}\sqrt{\tau}|X(\tau)|\le
...
$$
|Z|\le e^{2C_1\varepsilon^2}\mathit{min}_{0\le \tau\le
...
$$
\mathit{max}_{0\le \tau\le \tau_1'}\sqrt{\tau}|X(\tau)|_{g(T-\tau)}\le
(1+\frac{\varepsilon}{2})|Z|+\frac{\varepsilon}{2},
$$

 and

$$
|Z|\le (1+\frac{\varepsilon}{2})\mathit{min}_{0\le \tau\le
\tau_1'}\sqrt{\tau}|X(\tau)|_{g(T-\tau)}+\frac{\varepsilon}{2}.
$$
...
follows from the second inequality in \entryref{bf8f356cfd7a}

Now we are ready to establish that the ${\mathcal L}$-geodesics
whose initial conditions are elements of $\widetilde W_\mathit{sm}$ do
not leave $B(x,T,r/2)\times [T-r^2,T]$ for any $\tau\le\tau_1$.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="8.7" label="inside" -->
**Claim.** Suppose $\varepsilon_0\le 1/4n(n-1)$ is the constant from the last
...
**Proof.** Since $\varepsilon\le \varepsilon_0\le 1/4n(n-1)\le 1/8$, by the
last claim we have

$$
|\sqrt{\tau}X(\tau)|_{g(T)}\le (1+2\varepsilon )|Z|+2\varepsilon\le
\frac{5}{4}|Z|+\frac{3}{32\sqrt\varepsilon},
$$
...
$$
\widetilde V_x(W_\mathit{sm}(\tau_1)) = \int_{W_\mathit{sm}(\tau_1)}
(\tau_1)^{-\frac{n}{2}}e^{- l(q,\tau_1)}d\mathit{vol}_{g(\tau_1)}.
$$

 In
order to do this we estimate $ l_x(q,\tau_1)$ on $ W_\mathit{sm}(\tau_1)$. By hypothesis $|\mathit{Rm}|\le 1/r^2$ on
$B(x,T,r/2)\times [0,\tau_1]$ and by \entryref{0a1eac346e21} every
...
-n(n-1)/r^2$. Thus, for any $q\in W_\mathit{sm}(\tau_1)$ we have

$$
L_x(q,\tau_1) =
\int_{0}^{\tau_1}\sqrt{\tau}(R+|X(\tau)|^{2})d \tau \geq
-\frac{2n(n-1)}{3r^{2}}(\tau_1)^{\frac{3}{2}}=
-\frac{2n(n-1)}{3}\varepsilon^{\frac{3}{2}}r,
$$

 and hence

$$
l+x(q,\tau_1)=\frac{L_x(q,\tau_1)}{2\sqrt{\tau_1}}\ge -\frac{n(n-1)}{3}\varepsilon.
$$

 Since $W_\mathit{sm}(\tau)\subset B(x,T,r/2)\subset B(x,T,r)$, we have:

$$
\begin{aligned}
\widetilde V_x(W_\mathit{sm}(\tau_1)) & \leq &
\varepsilon^{-\frac{n}{2}}r^{-n}e^{n(n-1)\varepsilon/3}\mathit{Vol}_{g(T-\tau_1)} W_\mathit{sm}(\tau) \\
& \le & \varepsilon^{-\frac{n}{2}}r^{-n}e^{n(n-1)\varepsilon/3}\mathit{Vol}_{g(T-\tau_1)} B(x,T,r).\nonumber
\end{aligned}
$$

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="8.8" label="volvomp" -->
**Claim.** There is a universal constant $\varepsilon_0>0$ such that if
...
**Proof.** This is immediate from the second item in \entryref{bf8f356cfd7a}.

Now assume that $\varepsilon_0$ also satisfies this claim. Plugging
this into Equation (8.2), and using the fact that
$\varepsilon\le \varepsilon_0\le 1/4n(n-1)$, , so that
$n(n-1)\varepsilon/3\le 1/12$, and the fact that from the definition
we have $\mathit{Vol}_{g(T)} B(x,T,r)=\varepsilon^nr^n$, gives

$$
\widetilde V_x(W_\mathit{sm}(\tau_1))\leq \varepsilon^{-\frac{n}{2}}r^{-n}e^{n(n-1)\varepsilon/3}
(1.1)\mathit{Vol}_{g(T)}B(x,T,r)\le(1.1)
\varepsilon^{\frac{n}{2}}e^{\frac{1}{12}}.
$$

 Thus,

$$
\widetilde V_x(W_\mathit{sm}(\tau_1))\le
2\varepsilon^{\frac{n}{2}}.
$$

This completes the proof of \entryref{26e1a6292fef}.

### Upper bound for $\widetilde V_x( W_\mathit{lg}(\tau_1))$
...
Recall that $\mathit{Vol} B(x,T,r)=\varepsilon^nr^n$ and
$\tau_1=\varepsilon r^2$.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="8.9" label="large" -->
**Lemma.** There is a universal positive constant $\varepsilon_0>0$ such that
...
**Proof.** By the monotonicity result (\entryref{4dc40f37cd6d}), we see
...
$$
\widetilde V_x(W_\mathit{lg}(\tau_1))\le \int_{{\widetilde U}(\tau_1)\setminus
\widetilde U(\tau_1)\cap B(0,\frac{1}{8}\varepsilon^{-1/2})}2^n
e^{-|Z|^2}dZ\le \int_{T_p{\mathcal M}_T\setminus
B(0,\frac{1}{8}\varepsilon^{-1/2})}2^ne^{-|Z|^2}dZ.
$$
...
$$
\widetilde V_x(W_\mathit{lg}(\tau_1))\le \int_{\Ar^n}2^ne^{-|Z|^2}dZ-I(1/8\sqrt\varepsilon)
...
$$
\widetilde V_x(W_\mathit{lg}(\tau_1))\le
(4\pi)^{n/2}\frac{n}{2}e^{-1/(64n\varepsilon)}.
$$

 There is $\varepsilon_0>0$ such that the expression on the
right-hand side is less than $\varepsilon^{n/2}$ if $\varepsilon\le
\varepsilon_0$. This completes the proof of \entryref{4b41e7ff8317}.

Putting Lemmas \entryref{26e1a6292fef} and \entryref{4b41e7ff8317} together, establishes
\entryref{12d93b430400}.

As we have already remarked, \entryref{12d93b430400} immediately
implies \entryref{3defa9b2209c}. This completes the proof of
\entryref{3defa9b2209c}.

## Application to compact Ricci flows
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="8.10" -->
**Theorem.** Fix positive constants $\omega>0$ and $T_0<\infty$. Then there is
...
**Proof.** Fix any $x=(p,t_0)\in M\times [0,T]$. First, we claim that we can
...
rescaled flow we have $\mathit{Vol} B\ge \omega'$. The reason for the
latter fact is the following: By the Bishop-Gromov inequality
(\entryref{1a1b2c4fdc9b}) there is $\omega'>0$ depending only on
$\omega$ such that for any $q\in M$ and any $r\le 1$ we have $\mathit{Vol} B(q,0,r)\ge \omega'r^3$. Of course, the rescaling increases
...
$$
\mathit{Vol} B(p,T,r)\ge \mathit{Vol} B(p,T,r')\ge \kappa(r')^3\ge
\kappa\left(\frac{r}{2}\right)^3=\frac{\kappa}{8}r^3.
$$

From now on we assume that $t_0\ge 1$ and $r\le\sqrt{t_0}/2$.
According to \entryref{c1330c5a8be3} for any $(p,t)\in M\times
[0,2^{-4}]$ we have $|\mathit{Rm}(p,t)|\le 2$ and $\mathit{Vol} B(p,t,r)\ge \kappa_0 r^3$ for all $r\le 1$.

Once we know that $|\mathit{Rm}|$ is universally bounded on $M\times
...
By \entryref{aebc3ff2bd3b} there is a point $q_0\in M$ and an ${\mathcal
...
By \entryref{16fbe263a7c8}, the open subset ${\mathcal
...
t_0/4<\tau_0$, \entryref{3defa9b2209c} now gives the result. (See Fig. 8.3.)
