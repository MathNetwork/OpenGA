...
(\entryref{703e4cfeece7}), immediately yields \entryref{9999efa2be2c},
thus completing the proof of the Poincaré Conjecture and the
$3$-dimensional space-form conjecture.

## The result

<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="18.1" label="extinct" -->
**Theorem.** Let $(M,g(0))$ be a compact, connected normalized Riemannian $3$-manifold.
...
\entryref{703e4cfeece7}. This Ricci flow with surgery becomes extinct after a finite
...
\entryref{9999efa2be2c}.

**Proof.** (of \entryref{9999efa2be2c} assuming
\entryref{4b8a44aa316f}). Fix a normalized metric $g(0)$ on $M$, and let
$({\mathcal M},G)$ be the Ricci flow with surgery defined for all $t\in
[0,\infty)$ produced by \entryref{703e4cfeece7} with initial conditions $(M,g(0))$.
According to \entryref{4b8a44aa316f} there is $T>0$ for which the time-slice
$M_T$ is empty. By \entryref{7822d06083e0}, if there is $T$ for which $M_T$
...
\entryref{9999efa2be2c}. In particular, if $M$ is simply connected, then $M$ is
diffeomorphic to $S^3$, which is the statement of the Poincaré Conjecture.
...
\entryref{4b8a44aa316f} which will then complete the proof of
\entryref{9999efa2be2c}.
...
maps see [SacksUhlenbeck], [SchoenYau], and [Jost]. Let us
sketch the argument. For a compact Riemannian manifold $(M,g)$ every non-zero
element $\bbeta\in \pi_2(M)$ has associated with it an area, denoted
$W_2(\bbeta,g)$, which is the infimum over all maps $S^2\to M$ in the free
homotopy class of $\bbeta$ of the energy of the map. We find it convenient to
set $W_2(g)$ equal to the minimum over all non-zero homotopy classes $\bbeta$
of $W_2(\bbeta,g)$. In the case of a Ricci flow $g(t)$ there is an estimate
...
distance-decreasing property for surgery in \entryref{9021a4eca03f} we
...
satisfying the conclusion of \entryref{703e4cfeece7}.
...
the group-theoretic hypothesis of \entryref{4b8a44aa316f}, at some
later time $T'>T_0$ the time-slice $M_{T'}$ is empty. The argument
here is similar in spirit. There are two approaches. The first
approach is due to Perelman [Perelman3]. Here, one represents a
...
(\entryref{9021a4eca03f}) says that the inequality remains
...
The other approach, by Colding-Minicozzi [ColdingMinicozzi], is
to represent a non-trivial element in $\pi_3(M_T)$ as a non-trivial
element in $\pi_1(\mathit{Maps}(S^2,M))$, and associate to such an
...
(\entryref{9021a4eca03f}) implies that this inequality is
...
that occupy most of Perelman's analysis in [Perelman3].
 ...
 represents a point in the space $\mathit{Maps}(S^2,M)$ that is
...
approach, and that is the one we present here. In [Perelman3]
there were two points that we felt required quite a bit of argument
beyond what Perelman presented. In Section 2.2 on page 4 of
[Perelman3], Perelman asserts that there is a local, pointwise
curvature estimate that can be obtained by adapting arguments in the
literature; see Lemmas \entryref{38712e8cea3d} and \entryref{ff5d51a483d9} for the precise
statement. To implement this adaption required further non-trivial
arguments. We present these arguments in Section 18.7. In Section
2.5 on page 5 of [Perelman3] Perelman asserts that an
elementary argument establishes a lower bound on the length of a
boundary curve of a minimal annulus; see
\entryref{6e9739b0c59b} for a precise statement. While the
...
[HamiltonNSRF3M]. He was considering a situation where a
...
that given a Ricci flow with surgery as in \entryref{703e4cfeece7} there
...
hypothesis of \entryref{4b8a44aa316f}, there is a $T_2<\infty$ such
...
### Existence of the Ricci flow with surgery

Let $(M,g(0))$ be as in the statement of \entryref{4b8a44aa316f}, so that $M$ is
...
[Hatcher]) the first homology of the factor in question maps onto $\Zee$
...
Thus, by \entryref{703e4cfeece7}, for any compact $3$-manifold $M$ whose
...
conclusion of \entryref{703e4cfeece7} with $(M,g(0))$ as the initial conditions.

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.2" -->
**Definition.** Let $I$ be an interval (which is allowed to be open or closed at
...
point is a surgery time. Then the intersection of ${\mathcal X}$ with $\mathbf{t}^{-1}(I')$ is the Ricci flow on the time interval $I'$ applied to ${\mathcal
X}(t)$ for any $t\in I'$. Thus, for such intervals $I'$ the intersection,
${\mathcal X}(I')$, of ${\mathcal X}$ with $\mathbf{t}^{-1}(I')$ is determined by
...
## Disappearance of components with non-trivial $\pi_2$

Let $({\mathcal M},G)$ be a Ricci flow with surgery satisfying the
conclusions of \entryref{703e4cfeece7}. We make no assumptions about the
...
show that if $t$ is a surgery time, then $W_2(t)\le \mathit{liminf}_{t'\rightarrow t^-}W_2(t)$. Lastly, we show that at any
point $t\ge T_0$ we have

$$
\frac{dW_2}{dt}(t)\le -4\pi-\frac{1}{2}R_\mathit{min}(t)W_2(t),
$$

in the sense of forward difference quotients. It follows easily from
the bound $R_\mathit{min}(t)\ge -6/(4t+1)$ that there is $T_1({\mathcal
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.3" label="grouptheory" -->
**Lemma.** Suppose that $G$ is a finitely generated group,
...
**Proof.** This is a consequence of Grushko's theorem [Stallings], which
...
### Homotopically non-trivial families of $2$-spheres

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.4" -->
**Definition.** Let $X$ be a compact $3$-manifold (possibly disconnected). An
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.5" -->
**Lemma.** Let $X$ be a compact $3$-manifold (possibly disconnected). Then
...
**Proof.** Clearly, without loss of generality we can assume that $X$ is
connected. If $F$ is a homotopically essential family of $2$-spheres
in $X$, then by van Kampen's theorem, see p. 40 of [Hatcher],
...
$H_1(X)$,
 applying
\entryref{858af0fef413} and using the fact that the fundamental group of a
compact manifold is finitely presented establishes the result.

### Two-sphere surgeries are trivial after finite time

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.6" -->
**Definition.** Let $({\mathcal M},G)$ be a Ricci flow with surgery. We say that a
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="18.7" label="homess" -->
**Proposition.** Let $({\mathcal M},G)$ be a Ricci flow with surgery satisfying
...
**Proof.** Associate to each compact $3$-manifold $X$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.8" -->
**Claim.** Let $({\mathcal M},G)$ be a Ricci flow with surgery and for each $t$ set
...
\entryref{46f9d27654be} is immediate from this claim and the
previous lemma.

### For all $T$ sufficiently large $\pi_2(M_T)=0$

We have just established that given any Ricci flow with surgery $({\mathcal
M},G)$ satisfying the conclusion of \entryref{703e4cfeece7} there is $T_0<\infty$,
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="18.9" label="pi2" -->
**Proposition.** Let $({\mathcal M},G)$ be a Ricci flow with surgery satisfying the conclusion
of \entryref{703e4cfeece7}. Then there is some time $T_1<\infty$ such that every
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.10" -->
**Lemma.** Let $X$ be a compact Riemannian manifold with $\pi_2(X)\not=0$. Then
...
**Proof.** The first statement is Theorem 3.3 in [SacksUhlenbeck]. As for
...
$$
 ...
 According to
[SacksUhlenbeck] this energy functional is Palais-Smale on the
...
maps. According to [SacksUhlenbeck], after passing to a
...
[SacksUhlenbeck] pre-composing with a sequence of conformal
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.11" label="W_2" -->
**Lemma.**
...
Let us show how this lemma implies \entryref{4415092c2079}. Because
the curvature is pinched toward positive, we have

$$
R_\mathit{min}(t)\ge (-6)/(1+4t).
$$
...
and $w_2(T_0)=W_2(T_0)$. Then by \entryref{67c378f87457} and
\entryref{7bb0fa0c9412} we have $W_2(t)\le w_2(t)$ for all $t\ge T_0$. On
the other hand, we can integrate to find

$$
w_2(t)=w_2(T_0)\frac{(4t+1)^{3/4}}{(4T_0+1)^{3/4}}+4\pi
(4T_0+1)^{1/4}(4t+1)^{3/4}-4\pi (4t+1).
$$

 Thus, for $t$ sufficiently
large, $w_2(t)<0$. This is a contradiction since $W_2(t)$ is always
positive, and $W_2(t)\le w_2(t)$.

This shows that to complete the proof of \entryref{4415092c2079} we
need only establish \entryref{7bb0fa0c9412}.

**Proof.** (of \entryref{7bb0fa0c9412})
Let $f\colon S^2\to (X(t_0),g(t_0))$ be a minimal $2$-sphere.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.12" label="varareaform" -->
**Claim.**

$$
\frac{d\mathit{Area}_{g(t)}(f(S^2))}{dt}(t_0)\le -4\pi -\frac{1}{2}
R_\mathit{min}(g(t_0))\mathit{Area}_{g(t_0)}f(S^2).
$$
<!-- astrolabe:end -->

**Proof.** Recall that, for any immersed surface $f\colon S^2\to (M,g(t_0))$, we
have ([Ha])

$$
\begin{aligned}
\frac{d}{dt}\mathit{Area}_{g(t)}(f(S^2))\bigl|_{t=t_0}\bigr. & = &
 \int_{S^2}\frac{1}{2}\mathit{Tr}|_{S^2}\Bigl(\frac{\partial g}{\partial t}\Bigr)\Bigl|_{t=t_0}\Bigr.da \\
 & = &
 -\int_{S^2}(R-\mathit{Ric}(\mathbf{n},\mathbf{n}))da \nonumber
\end{aligned}
$$

 where $R$ denotes the scalar curvature of $M$, $\mathit{Ric}$ is the
Ricci curvature of $M$, and $\mathbf{n}$ is the unit normal vector field of $\Sigma$
in $M$. Now suppose that $f(S^2)$ is minimal. We can rewrite this as

$$
\begin{aligned}
\frac{d}{dt}\mathit{Area}_{g(t)}(f(S^2))\big |_{t=0}
 &= &
 -\int_{S^2}K_{S^2}da-\frac{1}{2}\int_{S^2}(|A|^2+R)da,
\end{aligned}
$$
...
(18.2) still holds when the integral on the right is replaced
by the integral over the immersed part of $f(S^2)$. Then by the
Gauss-Bonnet theorem we have

$$
\begin{aligned}
\frac{d}{dt}\mathit{Area}_{g(t)}(f(S^2))\bigl|_{t=t_0}\bigr.  & \leq &
-4\pi-\frac{1}{2}\mathit{Area}_{g(t_0)}(S^2)\min_{x\in M}\{R_{g}(x,t_0)\}.
\end{aligned}
$$

Since $f(S^2)$ is a homotopically non-trivial sphere in ${\mathcal X}(t)$ for
all $t$ sufficiently close to $t_0$ we see that $W_2(t)\le \mathit{Area}_{g(t)}f(S^2)$. Since $\mathit{Area}_{g(t)}f(S^2))$ is a smooth function of
$t$, the forward difference quotient statement in \entryref{7bb0fa0c9412} follows
immediately from \entryref{db9ef2c8d5c1}.
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.13" -->
**Claim.** Let $(M,g(t)),\ t_0\le t\le t_1$, be a Ricci flow on a compact
...
**Proof.** The rate of change of the area of $f(S^2)$ at time $t$ is

$$
\int_{f(S^2)}\frac{\partial g}{\partial t}(t)da=-2\int_{f(S^2)}\mathit{Tr}|_{TS^2}(\mathit{Ric}_{g(t)})da\le
4D\mathit{Area}_{g(t)}f(S^2) .
$$

 Integrating from $t_0$ to $t_1$ gives
the result.
...
sufficiently large, and hence $W_2(t')\le \mathit{liminf}_{n\rightarrow\infty}W_2(t_n)$. This is the lower
semi-continuity.
...
Section 15.4, we write ${\mathcal X}(t)$ as the union
...
\entryref{9021a4eca03f} that for any $\eta>0$ for all $t'<t$
...
$$
W_2(t)\le \mathit{liminf}_{t'\rightarrow t^-}W_2(t').
$$

This establishes all three statements in \entryref{4415092c2079} and
completes the proof of the proposition.

As an immediate corollary of \entryref{4415092c2079}, we obtain the
sphere theorem for closed $3$-manifolds.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.14" label="spherethm" -->
**Corollary.** Suppose that $M$ is a closed, connected $3$-manifold containing no
...
**Proof.** Let $M$ be as in the statement of the corollary. Let $g$ be a
normalized metric on $M$, and let $({\mathcal M},G)$ be the Ricci
flow with surgery defined for all time with $(M,g)$ as initial
conditions. According to \entryref{4415092c2079} there is $T<\infty$
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="18.15" -->
**Remark.** Notice that it follows from the list of disappearing components that
...
conclusion of \entryref{703e4cfeece7} and also has initial condition $M$ that is a
connected $3$-manifold whose fundamental group satisfies the hypothesis of
\entryref{4b8a44aa316f}. The argument showing that components with non-trivial
...
### Forward difference quotient for $\pi_3$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.16" -->
**Claim.** Suppose that $\pi_2(M,x_0)=0$. Then $\pi_2(\Lambda M,*)\cong
...
**Proof.** An element in $\pi_2(\Lambda M,*)$ is represented by a map
$S^2\times S^1\to M$ that sends $\{\mathit{pt}\}\times S^1$ to $x_0$.
Hence, this map factors through the quotient of $S^2\times S^1$
obtained by collapsing $\{\mathit{pt}\}\times S^1$ to a point. The
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.17" label="Wdefn" -->
**Definition.** Fix a homotopically trivial loop $\gamma\in \Lambda M$. We set
...
Now let us formulate the analogue of \entryref{4415092c2079} for
...
X}(t)$ given by \entryref{9021a4eca03f} is either a homotopy
...
\entryref{9021a4eca03f} sends $\xi(t')$ to $\xi(t)$ for all
...
Here is the result that is analogous to \entryref{7bb0fa0c9412}.

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="18.18" label="W_3fordiff" -->
**Proposition.** Suppose that $({\mathcal M},G)$ is a Ricci flow with surgery as in
\entryref{703e4cfeece7}. Let ${\mathcal X}$ be a path of components of ${\mathcal
...
### Proof of Theorem \entryref{4b8a44aa316f} assuming Proposition \entryref{b187c06d1432}

 According to \entryref{4415092c2079} there is $T_1$ such that
every component of $M_T$ has trivial $\pi_2$ for every $T\ge T_1$.
Suppose that \entryref{4b8a44aa316f} does not hold for this Ricci flow
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.19" -->
**Claim.** ${\mathcal X}(T_1)$ has non-trivial $\pi_3$.
<!-- astrolabe:end -->

**Proof.** By hypothesis the fundamental group of $M_0$ is a free product of
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.20" -->
**Claim.** Let $X$ be a compact $3$-manifold. If $\pi_1(X)$ is a non-trivial
free product or if $\pi_1(X)$ is isomorphic to $\Zee$, then
$\pi_2(X)\not=0$.
<!-- astrolabe:end -->

**Proof.** See [Hempel], Theorem 5.2 on page 56 (for the case of a copy of $\Zee$)
and [Hempel] Theorem 7.1 on page 66 (for the case of a free product
decomposition).
...
fundamental group. Of course, by Poincaré duality any simply
connected $3$-manifold is a homotopy $3$-sphere. It follows
immediately that $\pi_3({\mathcal X}(T_1))\cong \Zee$. This
completes the proof of the claim.

Now we can apply \entryref{b187c06d1432} to our path of components
${\mathcal X}$ defined for all $t\in [T_1,T_2]$. First recall by
\entryref{703e4cfeece7} that the curvature of $({\mathcal M},G)$ is pinched toward
positive which implies that $R_\mathit{min}(t)\ge (-6)/(1+4t)$. Let $w(t)$ be the
...
\entryref{4b8a44aa316f} modulo \entryref{b187c06d1432}.

Thus, to complete the argument for \entryref{4b8a44aa316f} it remains
only to prove \entryref{b187c06d1432}.

### Continuity of $W_\xi(t)$

In this subsection we establish the two continuity conditions for
$W_\xi(t)$ stated in \entryref{b187c06d1432}.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.21" -->
**Claim.** If $t$ is not a surgery time, then $W_\xi(t)$ is continuous at $t$.
<!-- astrolabe:end -->

**Proof.** Since $t$ is not a surgery time, a family $\Gamma(t)\colon S^2\to
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.22" -->
**Claim.** Suppose that $t$ is a surgery time. Then
...
**Proof.** This is immediate from the fact from \entryref{9021a4eca03f} that for any
$\eta>0$ for every $t'<t$ sufficiently close to $t$ there is a homotopy
equivalence ${\mathcal X}(t')\to {\mathcal X}(t)$ which is a
$(1+\eta)$-Lipschitz map.

To prove \entryref{b187c06d1432} and hence
\entryref{4b8a44aa316f}, it remains to prove the forward difference
quotient statement for $W_\xi(t)$ given in
\entryref{b187c06d1432}.

### A further reduction of Proposition \entryref{b187c06d1432}
...
$\Gamma$ in order to establish \entryref{b187c06d1432}. Now we
...
\entryref{b187c06d1432}. Here is the result that shows a
deformation as required exists.

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.23" -->
**Definition.** Let $(M,g(t)),\ t_0\le t\le t_1$, be a Ricci flow on a compact
...
<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="18.24" label="XI" -->
**Proposition.** Let $(M,g(t)),\ t_0\le t\le t_1$, be a Ricci flow on a compact
...
quotient result in \entryref{b187c06d1432}. Let ${\mathcal X}$ be a path
...
all the ${\mathcal X}(t)$ are identified under the Ricci flow.
 Let
$w(t)$ be the solution to Equation (18.4) with value
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.25" -->
**Definition.** Let $A(t)=\int_{t'}^t\frac{1}{2}R_\mathit{min}(s)ds$.
<!-- astrolabe:end -->

Direct integration shows the following:

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.26" label="vclaim" -->
**Claim.** We have
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.27" label="shortpi2triv" -->
**Lemma.** Given a compact Riemannian manifold $(X,g)$ with $\pi_2(X)=0$. Then
...
**Proof.** We choose $\zeta$ smaller than the injectivity radius of $(X,g)$.
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.28" label="shlnsharea" -->
**Corollary.** Let $(X,g)$ be a compact Riemannian manifold. Given $\eta>0$ there is a
$0<\zeta<\eta/2$ such that any $C^1$-loop $c\colon S^1\to X$ of length less
than $\eta$ bounds a disk in $X$ of area less than $\eta$.
<!-- astrolabe:end -->

Now we return to the proof that \entryref{fd85732525c1} implies
\entryref{b187c06d1432}. We consider the restriction of the
...
identified with each other under the flow.
 Let $w(t)$ be the solution to Equation (18.4)
with initial condition $w(t_0)=W_\xi(t_0)$. There are two cases to
consider: (i) $w(t_1)\ge 0$ and $w(t_1)< 0$.

Suppose that $w(t_1)\ge 0$. Let $\eta>0$ be given. Then by \entryref{eaf31795ca89}
and \entryref{37b374f0934d}, there is $0<\zeta<\eta/2$ such that the
following two conditions hold:

- **(a)** Any loop in ${\mathcal X}(t_1)$ of length less than $\zeta$
bounds a disk of area less than $\eta$.
- **(b)** For every $a\in [0,W_\xi(t_0)+2\zeta]$
...
\entryref{fd85732525c1} there is a one-parameter family
...
- **(i)** the length of $\widetilde\Gamma(t_1)(c)$ is less than $\zeta$, or
- **(ii)**
...
- **(a)** $\widetilde\Gamma(t_1)(c)$ has length less than $\zeta$ and hence bounds a disk
of area less than $\eta$, or
- **(b)** $A(\widetilde\Gamma(t_1)(c))<w_{W_\xi(t_0)+2\zeta}(t_1)+\zeta<w(t_1)+\eta/2+\eta/2=
w(t_1)+\eta$.
...
\entryref{fd85732525c1} when $w(t_1)\ge 0$.
...
such that $w(t_1)+\eta<0$. Then using \entryref{ed0e92ce8f57} and
\entryref{eaf31795ca89}, we fix $\zeta$ with $0<\zeta<\eta/2$ such that:

- **(i)** If $\Gamma\colon S^2\to \Lambda {\mathcal X}(t_1)$ is a family
of loops and each loop in the family is of length less than $\zeta$,
then the family is homotopically trivial.
- **(ii)** For any $a\in [0,W_\xi(t_0)+2\zeta]$ we have $w_a(t_1)<w(t_1)+\eta/2$.

We fix a map $\Gamma\colon S^2\to {\mathcal X}(t_0)$ with
$[\Gamma]=\xi$ and with $W(\Gamma)<W_\xi(t_0)+\zeta$. Now according
to \entryref{fd85732525c1} there is a family of maps
...
$$
A(\widetilde\Gamma(t_1)(c))< w(t_1)+\eta/2+\zeta<w(t_1)+\eta<0
$$
...
length less than $\zeta$. By \entryref{ed0e92ce8f57} this implies that
...
proof of \entryref{4b8a44aa316f} is to establish \entryref{fd85732525c1}.
The rest of this chapter is devoted to doing that.

## Curve-shrinking flow

Given $\Gamma$, the idea for constructing the one-parameter family
$\widetilde \Gamma(t)$ required by \entryref{fd85732525c1} is to evolve
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.29" label="curvshrexist" -->
**Claim.** For any immersed $C^2$-curve $c\colon S^1\to (M,g(t_0))$ there is a
...
For a proof of this result, see Theorem 1.13 in [AG].

### The proof of Proposition \entryref{fd85732525c1} in a simple case
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.30" label="embcase" -->
**Lemma.** Suppose that $c\in \Lambda M$ is a homotopically trivial, embedded
...
$$
\frac{dA}{dt}(t)\le -2\pi-\frac{1}{2}R_\mathit{min}(t)A(t)
$$

in the sense of forward difference quotients.
<!-- astrolabe:end -->

**Proof.** According to results of Hildebrandt and Morrey, [Hildebrandt]
and [Morrey], for each $t\in [t_0,t_1]$, there is a smooth
minimal disk spanning $c(\cdot,t)$. Fix $t'\in [t_0,t_1)$ and
consider a smooth minimal disk $D\to (M,g(t'))$ spanning
$c(\cdot,t)$. It is immersed, see [HardtSimon] or [GL].
The family $c(\cdot,t)$ for $t$ near $t'$ is an isotopy of
$c(\cdot,t')$. We can extend this to an ambient isotopy
$\varphi_t\colon M\to M$ with $\varphi_{t'}=\mathit{Id}$. We impose
...
$$
\frac{d}{dt}\bigl|_{t=t'}\bigr.\mathit{Area}(\varphi_t(D))
=\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{\mathit{det}(h_{\alpha\beta})(t)}da.
$$
...
$$
\begin{aligned}
\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{\mathit{det}(h_{\alpha\beta}(t))}da & = & -\int_{\varphi_{t'}(D)}\Bigl(\mathit{Tr} \mathit{Ric}^T\Bigr)\sqrt{\mathit{det}(h_{\alpha\beta}(t))}da \\
& & +\int_{\varphi_{t'}(D)}\mathit{div}\left(\frac{\partial
\varphi_{t'}}{dt}\right)^T\sqrt{\mathit{det}(h_{\alpha\beta}(t))}da.
\end{aligned}
$$
...
$$
\begin{aligned}
-\int_{\varphi_{t'}(D)}\Bigl(\mathit{Tr} \mathit{Ric}^T\Bigr)\sqrt{\mathit{det}(h_{\alpha\beta}(t'))}da  \\
 & = &
-\int_{\varphi_{t'}(D)}K_{\varphi_{t'}(D)}da
-\frac{1}{2}\int_{\varphi_{t'}(D)}(|\hat A|^2+R)da \\
& \le & -\int_{\varphi_{t'}(D)}K_{\varphi_{t'}(D)}\sqrt{\mathit{det}(h_{\alpha\beta}(t'))}da-\frac{1}{2}\mathit{Area}{\varphi_{t'}(D)}\min_{x\in M}\{R(x,t')\}.
\end{aligned}
$$
...
$$
\int_{\varphi_{t'}(D)}\mathit{div}\left(\frac{\partial
\varphi_{t'}}{dt}\right)^T\sqrt{\mathit{det}(h_{\alpha\beta}(t'))}da=-\int_{ \varphi_{t'}(\partial
D)}\left(\frac{d\varphi_t}{dt}\bigl|_{t=t'}\bigr.\right)\cdot n
ds,
$$

 where $n$ is the inward pointing normal vector to
...
$$
\left(\frac{d\varphi_t}{dt}\bigl|_{t=t'}\bigr.\right)\cdot n=k_\mathit{geod}.
$$
...
$$
\begin{aligned}
\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{\mathit{det}(h_{\alpha\beta}(t))}da  \\\ & \le &
...
$$
 ...
 Of course, the Gauss-Bonnet
theorem allows us to rewrite this as

$$
\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{\mathit{det}(h_{\alpha\beta}(t))}da \le -2\pi-\frac{1}{2}R_\mathit{min}(t')\mathit{Area}(\varphi_{t'}(D)).
$$

Let $\psi(t)$ be the solution to the ODE

$$
\psi'(t)=-2\pi-\frac{1}{2}R_\mathit{min}(t)\psi(t)
$$

with $\psi(t^-)=A(t^-)$. The following is immediate from the
previous lemma and \entryref{67c378f87457}.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.31" -->
**Corollary.** With notation and assumptions as above, if the curve-shrinking flow
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.32" label="nonembeasycase" -->
**Lemma.** Suppose that the dimension of $M$ is at least $3$ and that
...
**Proof.** We first remark that continuity has already been established.
To show the inequality, we begin with a claim.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.33" -->
**Claim.** It suffices to prove the following for every $\delta>0$. There is a
...
**Proof.** (of the claim) Suppose that for each $\delta$ there is such
...
$$
\mathit{lim}_{n\rightarrow \infty}A(\hat c_n(\cdot, t^\pm))=A(c(\cdot,t^\pm)).
$$
...
- **(1)** the difference of the curvature of $\hat c$ and of $c$ at
every $(x,t)$ is a vector of length less than $\delta$,
- **(2)** the difference of $\partial \hat c/\partial t$ and $\partial
c/\partial t$ is a vector of length less than $\delta$,
- **(3)** the ratio of the arc lengths of $\hat c$ and $c$ at every
...
\psi_{\delta,\hat c}(t_{i+1})-\psi_{\delta,\hat c}(t_i)
$$
...
given the proof of \entryref{f98ccf7ea70d}. According to this argument
we have

$$
\frac{d}{dt}A(\hat c(t))|_{t=t'}\le -2\pi -\frac{1}{2}R_\mathit{min}(t')A(c(t'))+\int_{c(x,t')}\left[k_\mathit{geod}(\hat
c)-\left(\frac{d\varphi_t}{dt}|_{t=t'}\right)\cdot n\right] ds
$$
...
$$
\left|k_\mathit{geod}(\hat c)-\left(\frac{d\varphi_t}{dt}|_{t=t'}\right)\cdot
n\right|<2\delta.
$$
...
$$
\frac{d}{dt}A(\hat c(t))|_{t=t'}\le -2\pi -\frac{1}{2}R_\mathit{min}(t')A(c(t'))+2\delta L_{\hat c}(t).
$$

 The result is then
immediate from \entryref{67c378f87457}.
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.34" label="1stcurvshr" -->
**Lemma.** Assume that $(M,g(t)),\ t_0\le t\le t_1$, is a Ricci flow and that
...
**Proof.** Notice that as $t$ varies $|X|_{c^*g}^2$ is not the norm of the
...
$$
H(|X|_{c^*g}^2)(x,t)=-2\mathit{Ric}_{c^*g(t)}(X(x,t),X(x,t))
+2\langle\nabla_HX(x,t),X(x,t)\rangle_{c^*g(t)}.
$$
...
\frac{-1}{2\left(|X|^2_g\right)^{3/2}}H(|X|_{c^*g})^2)X.
$$
...
$$
[H,S](x,t)=\left(k^2+\mathit{Ric}_{c^*g(t)}(S(x,t),S(x,t))\right)S(x,t).
$$
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.35" label="2ndcurvshr" -->
**Lemma.**
...
**Proof.** Using the same conventions as above for the function $|H|_{c^*g}$
and but leaving the metric implicit, we have

$$
\frac{\partial }{\partial t}k^2=\frac{\partial }{\partial
t}(|H|^2_{c^*g})=-2\mathit{Ric}(H,H)+2\langle\nabla_HH,H\rangle_{c^*g}.
$$

 Now we compute
(using the second equation from \entryref{4fc24fa1d31e})
...
{\mathcal R}(H,S)S \\
& = & \nabla_S\nabla_SH+2(k^2+\mathit{Ric}(S,S))H+S(k^2+\mathit{Ric}(S,S))S+ {\mathcal R}(H,S)S.
\end{aligned}
$$
...
$$
2\langle\nabla_HH,H\rangle_{c^*g}=2g(\nabla_S\nabla_SH,H)+4k^4+4k^2\mathit{Ric}(S,S))+2\mathit{Rm}(H,S,H,S).
$$
...
Plugging this into Equation (18.6) and using
Equation (18.5) yields

$$
\begin{aligned}
\frac{\partial}{\partial t}k^2 & = & -2\mathit{Ric}(H,H)+S(S\langle
H,H\rangle_{c^*g})-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle_{c^*g}
\\
& & +2k^4+4k^2\mathit{Ric}(S,S)+2\mathit{Rm}(H,S,H,S).
\end{aligned}
$$
...
$$
 ...
 where the primes refer to the derivative with
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.36" label="3rdcurvshr" -->
**Claim.** There is a constant $C_1<\infty$ depending only on an
...
**Proof.** We set $C_1=\widehat C/2$, where $\widehat C$ is as in
Inequality 18.9. It follows from Inequality (18.9) that

$$
2k\frac{\partial k}{\partial t}\le
2kk''+2(k')^2+2k^4-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle_{c^*g}+\hat
Ck^2.
$$

 Since $k^2=\langle H,H\rangle_{c^*g}$, we see
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.37" label="1835" -->
**Lemma.** There is a constant $C_2<\infty$ depending only on an
upper bound for the norm of the sectional curvatures of the ambient
...
$$
\frac{d}{dt}L=\int
\frac{1}{2|X|_{c^*g}}\left(-2\mathit{Ric}(X,X)-2k^2|X|_{c^*g}^2\right)dx.
$$

 Thus,

$$
\frac{d}{dt}L=\int (-\mathit{Ric}(S,S)-k^2)|X|_{c^*g}dx=\int(-\mathit{Ric}(S,S)-k^2)ds.
$$
...
t}|X|_{c^*g} +k\frac{\partial |X|_{c^*g}}{\partial t}\right)dx.
$$

 Thus, using
\entryref{67b855d0d6b5} and the first equation in \entryref{4fc24fa1d31e} we have

$$
\begin{aligned}
\frac{d}{dt}\Theta & \le &  \int (k''+k^3+C_1k)ds+\int
\frac{k}{2|X|_{c^*g}}(-2\mathit{Ric}(X,X)-2k^2|X|_{c^*g}^2)dx \\
& = & \int(k''+k^3+C_1k)ds-\int k(\mathit{Ric}(S,S)+k^2)ds \\& = &
\int(k''+C_1k-k \mathit{Ric}(S,S))ds.
\end{aligned}
$$
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.38" label="lncurvgrowth" -->
**Corollary.** The following holds for the constant $C_2$ as in the previous
...
### Ramp solutions in $ M\times S^1$

As we pointed out in the beginning of Section 18.4 the
...
goes back to [AG], is to replace the original situation of
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.39" label="rampcomp" -->
**Claim.**
...
**Proof.** Since $U$ is a constant vector field and hence parallel along all curves and
since $\mathit{Ric}(V,U)=0$ for all tangent vectors $V$, by \entryref{4fc24fa1d31e}
we have

$$
\begin{aligned}
\frac{\partial}{\partial t}\langle S,U\rangle_{g(t)} & = & -2\mathit{Ric}(S,U)
+\langle dc(\nabla_HS),U\rangle_{g(t)} \\
& = & \langle dc(\nabla_HS),U\rangle_{g(t)}
=  \langle dc([H,S]+\nabla_SH),U\rangle_{g(t)} \\
& = & (k^2+\mathit{Ric}(S,S))u+\langle dc(\nabla_SH),U\rangle_{g(t)} \\
& = & (k^2+\mathit{Ric}(S,S))u+S(dc(\langle H),U\rangle_{g}) \\
 & = & (k^2+\mathit{Ric}(S,S))u+S(\langle dc(\nabla_SS),U\rangle_g) \\
&  = & (k^2+\mathit{Ric}(S,S))u+S(S(u))  =  (k^2+\mathit{Ric}(S,S))u+u''.
\end{aligned}
$$

<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.40" -->
**Definition.** A curve $c\colon S^1\to  M\times S^1_\lambda$ is said to be a *ramp* if $u$ is strictly positive.
<!-- astrolabe:end -->
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.41" label="ubd" -->
**Corollary.** If $c(x,t),\ t_0\le t< t'_1<\infty$, is a solution of the curve
shrinking flow in $( M, g(t))\times (S^1_\lambda,ds^2)$ and if
$c(t_0)$ a ramp, then $c(t)$ is a ramp for all $t\in [t_0,t'_1)$.
<!-- astrolabe:end -->

**Proof.** From the equation in \entryref{2f115406780a}, we see that for $C'$ an
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.42" label="csforramps" -->
**Lemma.** Let $(M,g(t)),\ t_0\le t\le t_1$, be a Ricci flow. Suppose that
...
**Proof.** The real issue here is to show that the curve-shrinking flow exists
...
\entryref{656c8fbce129} there is a $t'_1\le t_1$ such that the
...
Using \entryref{67b855d0d6b5} and \entryref{2f115406780a} we compute

$$
\begin{aligned}
\frac{\partial}{\partial t}\left(\frac{k}{u}\right)
& = &\frac{1}{u}\frac{\partial
k}{\partial t}-\frac{k}{u^2}\frac{\partial u}{\partial t} \\
& \le & \frac{k''+k^3+C_1k}{u}-\frac{k}{u^2}\left(u''+(k^2+\mathit{Ric}(S,S))u\right)
\\
& = & \frac{k''}{u}-\frac{ku''}{u^2}+\frac{C_1k}{u}-\frac{k}{u}\mathit{Ric}(S,S).
\end{aligned}
$$
...
\left(\frac{k'u-u'k}{u^2}\right).
$$

 Plugging this in, and using the
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.43" label="mugrowth" -->
**Lemma.** Suppose that the dimension $n$ of $ M$
...
**Proof.** Fix a time $t'$. First assume that the loops $c_1(\cdot,t')$ and
$c_2(\cdot,t')$ are disjoint. Under Ricci flow the metrics on the
manifold immediately become real analytic (see [Bando]) and
furthermore, under the curve-shrinking flow the curves $c_1$ and
$c_2$ immediately become analytic (see [GageHamilton]).
...
of Hildebrandt ([Hildebrandt]) and Morrey ([Morrey]) any
...
$$
\frac{d \mathit{Area} A}{d t}(t')=\int_{A}(-\mathit{Tr}(\mathit{Ric}^T(g(t'))))da
+\int_{\partial A}-k_\mathit{geod}ds,
$$

 where $\mathit{Ric}^T$ is the
...
$$
\int_{\partial A}-k_\mathit{geod}ds =\int_{A}Kda
$$
...
$$
\int_{\partial A}-k_\mathit{geod}ds=\int_{A}Kda-\sum_{i=1}^k2\pi(n_i-1).
$$

 Thus, we see

$$
\frac{d \mathit{Area} A}{d t}(t')\le\int_{A}(-\mathit{Tr}(\mathit{Ric}^T(g(t')))+K)da.
$$
...
the Ricci curvature along $A$ is at most $2(n-1)|\mathit{max}_{x\in
M}\mathit{Rm}(x,t')|$. Hence,

$$
\frac{d\mathit{Area} A}{dt}(t')\le (2n-1)|\mathit{max}_{x\in M}\mathit{Rm}(x,t')|\mu(t').
$$
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.44" -->
**Corollary.** Given curve-shrinking flows $c_1(\cdot,t)$ and $c_2(\cdot,t)$ for
...
## Proof of Proposition \entryref{fd85732525c1}

Now we are ready to use the curve-shrinking flow for ramps in
$M\times S_\lambda^1$ to establish \entryref{fd85732525c1} for $M$. As
...
### Approximations to the original family

The first step in the proof of \entryref{fd85732525c1} is to identify
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.45" label="goodapprox" -->
**Lemma.** Given a continuous map $\Gamma\colon S^2\to \Lambda M$ representing
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.46" -->
**Definition.** Let $c\colon S^1\to M$ be a $C^1$-map. Fix a positive integer $n$.
By a *regular $n$-polygonal approximation* to $c$ we mean the
following. Let $\xi_n=\mathit{exp}(2\pi i/n)$, and consider the points
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.47" label="onec" -->
**Claim.** Given $\zeta>0$ and a $C^1$-map $c\colon S^1\to M$ then for all $n$
...
**Proof.** The length of $c$ is the limit of the lengths of the $n$-polygonal
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.48" label="uniformc" -->
**Claim.** Let $X\subset \Lambda M$ be a compact subset and let $\zeta>0$ be
...
**Proof.** Suppose the result is false. Then for each $N$ there is $c_N\in X$
and $n\ge N$ so that the lemma does not hold for $c_N$ and $n$.
Passing to a subsequence, we can suppose that the $c_N$ converge to
$c_\infty\in X$. Applying \entryref{981d63c19e50} we see that there is $N$
such that the conclusion of \entryref{981d63c19e50} holds with $\zeta$
replaced by $\zeta/2$ for $c_\infty$ and all $n\ge N$. Clearly, then
by continuity for all $n\ge N$ the conclusion of \entryref{981d63c19e50}
holds for the $n$-polygonal approximation for every $c_l$ for all
$l$ sufficiently large. This is a contradiction.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.49" label="Gammaapprox" -->
**Corollary.** Let $\Gamma\colon S^2\to \Lambda M$ be a continuous map with the
...
**Proof.** Given $\Gamma$ there is a uniform bound over all $c\in S^2$ on the
...
follow immediately from \entryref{028d3b5182b5}.
...
- **(1)** $\psi_n$ is non-negative and vanishes to infinite order at the point $1$ on the unit circle.
- **(2)** $\psi_n$ is periodic with period $2\pi i/n$.
- **(3)** $\psi_n$ is positive on the interior of the interval $[1,\xi_n]$ on the unit circle,
and the restriction of $\psi_n$ to this interval is symmetric about
$\mathit{exp}(\pi i/n)$, and is increasing from $1$ to $\mathit{exp}(\pi
i/n)$.
- **(4)** $\int_1^{\xi_n}\psi_n(s)ds =2\pi/n$.

 Now we define a map $\tilde\psi_n\colon S^1\to S^1$ by

$$
\tilde \psi_n(x)= \int_{1}^x\psi_n(y)dy.
$$
...
**Proof.** (of \entryref{6bb9851ce528})
Given a continuous map $\Gamma\colon S^2\to \Lambda M$ and $\zeta>0$
we fix $n$ sufficiently large so that \entryref{7a8f55be175e}
...
\entryref{7a8f55be175e} that $\widetilde \Gamma$ satisfies the
conclusions of \entryref{6bb9851ce528} except possibly the last one.
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.50" -->
**Claim.** For any $0<\lambda<1$, the length of the ramp $\widetilde
\Gamma(c)^\lambda$ is at most $\lambda$ plus the length of
$\Gamma(c)$. The total curvature of $\widetilde\Gamma(c)^\lambda$ is
at most $n\pi$.
<!-- astrolabe:end -->

**Proof.** The arc length element for $\widetilde \Gamma(c)^\lambda$ is
...
first-half subinterval $[\xi_n^k,\xi_n^k\cdot\mathit{exp}(\pi i/n)]$.
The tangent vector $X(x)$ is $(\lambda/2\pi) v$ at the initial point
of this subinterval and is $L\psi_n(\xi_n^k\cdot\mathit{exp}(\pi
...
on the second-half subinterval $[\xi_n^k\cdot\mathit{exp}(\pi
...
\entryref{11ec367eae93} with initial data the ramp
...
\entryref{fd85732525c1} for the fixed $\Gamma$ and $\zeta>0$. We do
this in steps. First, we show that fixing one $c\in S^2$, for
$\lambda$ sufficiently small (depending on $c$) an analogue of
\entryref{fd85732525c1} holds for the one-parameter family of loops
...
Equation (18.4) with initial condition
$v(t_0)=A(\widetilde\Gamma(c))$. (Actually, we establish a slightly
stronger result, see \entryref{9df99ae6f4cb}.) The next step in the
...
\entryref{fd85732525c1} for individual curves (or rather the slightly
stronger version of it) holds for every $\hat c\in {\mathcal S}$.
Then we complete the proof of \entryref{fd85732525c1} using the fact
...
### The case of a single $c\in S^2$

According to \entryref{6bb9851ce528}, for all $\lambda\in (0,1)$ the
lengths and total curvatures of the $\widetilde\Gamma^\lambda(c)$
are uniformly bounded for all $c\in S^2$. Hence, by
\entryref{156a3da68fbd} the same is true for
$\widetilde\Gamma_c^\lambda(t)$ for all $c\in S^2$ and all $t\in
[t_0,t_1]$.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.51" label="areachange" -->
**Claim.** There is a constant $C_4$ depending on $t_1-t_0$, on the curvature
...
**Proof.** All the constants in this argument are allowed to depend on
...
swept out by $c(x,t),\ t'\le t\le t''$. We denote by $\mathit{Area} (S^\lambda_c[t',t''])$ the area of this surface with respect
...
$$
\int_{S^\lambda_c[t',t'']}-\mathit{Tr} \mathit{Ric}^Tda
$$

 where $\mathit{Ric}^T$ is the restriction of the
...
$\int_{c(x,t'')}|H|ds$. According to \entryref{88b8b3366b19} there is a
...
first term is bounded above by $C''\mathit{Area} S^\lambda_c[t',t'']$
...
$C_1'$. Since its value at $t'$ is zero, we see that $\mathit{Area} S^\lambda_c[t',t'']\le C'_1(t''-t')$. It follows that the
...
Equation (18.11) that there is a constant $C_5$ (we take
$C_5>1$) depending only on the curvature bound of the ambient
manifolds and the family $\Gamma$ such that for any $c\in S^2$ we
have

$$
\int_{t_0}^{t_1}\int_{\widetilde\Gamma_c^\lambda(t)}
k^2dsdt\le C_5.
$$

 Thus, for any constant $1<B<\infty$
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.52" label="CSSHI" -->
**Lemma.** Let $(M,g(t)),\ t_0\le t\le t_1$, be a Ricci flow. Then there exist
...
Section 4 of [AG]. Once $k^2$ is bounded by $\widetilde
C_0/(t-t')$ the higher derivative statements are standard, see
[Altschuler]. For completeness we have included the proof of
...
these intervals. According to the construction and \entryref{38712e8cea3d}
these sets $J_B(c,\lambda)$ satisfy the following:

- **(1)** $J_B(c,\lambda)\subset [t_0,t_1-B^{-1}]$ is a union of a bounded number of intervals
(the bound being independent of $c\in S^2$ and of $\lambda$) of
length $\delta^5B^{-2}/2$.
- **(2)** The measure of $J_B(c,\lambda)$ is at least
$t_1-t_0-3C_5B^{-1}$.
- **(3)** For every $t\in J_B(c,\lambda)$ either the length of $\widetilde\Gamma_c^\lambda(t)$ is
...
- **(1)** $J_B(c)\subset J_B(c,\lambda_n)$ for all $n$ sufficiently
large.
- **(2)** $J_B(c)$ covers all of $[t_0,t_1]$ except a subset of length
$4C_5B^{-1}$.
...
- **(3)** there are uniform bounds for the curvature and all its
derivatives for the curves $\widetilde\Gamma_c^{\lambda_n}(t)$, for
all $ t\in J_i$ and all $n$, or
- **(4)** for each $n$ there is $t_n\in J_i$ such that the length of
...
\entryref{fd85732525c1} holds for $p_1\widetilde\Gamma_c^\lambda(t)$.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.53" label="xistronger" -->
**Lemma.** Given $\zeta>0$, there is $1<B<\infty$, with $B>(t_1-t_0)^{-1}$,
...
solution to Equation (18.4) with initial condition
...
**Proof.** In order to establish this lemma we need a couple of claims about
...
Equation (18.4) on these sub-intervals. In both claims the
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.54" label="C1C4delta" -->
**Claim.** Fix $C_4$ as in \entryref{52123e3c5a27} and fix a constant
...
**Proof.** We write $J$ as a union of disjoint intervals $J_1,\ldots,J_k$ so
...
such that $R_\mathit{min}(t)\ge -2C_6$ for all $t\in [t_0,t_1]$. Let
$V(a)$ be the maximum value of $|w_{a,t_0}|$ on the interval
$[t_0,t_1]$ and let $V=\mathit{max}_{a\in [0,\widetilde A]}V(a)$. let
$C_7=C_4+2\pi+C_6V$.
 We shall prove by
induction that
...
\prod_{\ell=j+1}^{i-1}e^{C_6|J_\ell|}\right)
$$
...
$\beta_i=f(b_i)-w_{f(t_0),t_0}(b_i)$. Then by \entryref{eaf31795ca89} we
have
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.55" label="deltaprime" -->
**Claim.** Fix $\zeta>0$, $A$ and $C_6, C_4$ as in the last claim, and let
...
**Proof.** We define $C_7$ as in the previous proof. We use the notation
...
Now let us return to the proof of \entryref{9df99ae6f4cb}. Recall
...
\entryref{9df99ae6f4cb}. Now it is time to fix $B$. First, we fix
$\widetilde A=W(\Gamma)+\zeta$, we let $C_2$ be as in
\entryref{156a3da68fbd}, $C_4$ be as in \entryref{52123e3c5a27},
$C_5$ be as in Equation (18.13), and $C_6$ be as in the proof
of \entryref{7e0b4b1362c0}. Then we have $\delta'$ depending on
$C_6,C_4,\widetilde A$ as in \entryref{7e0b4b1362c0}. We fix $B$ so
that:

- **(1)** $B\ge 3C_5(\delta')^{-1}$,
- **(2)** $B\ge 3e^{C_2(t_1-t_0)}\zeta^{-1}$, and
- **(3)** $B>C_2/(\mathit{log}4-\mathit{log}3)$.

The first step in the proof of \entryref{9df99ae6f4cb} is the
following:

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.56" label="limitcs" -->
**Claim.** After passing to a subsequence of $\{\lambda_n\}$, either:
...
According to \entryref{656c8fbce129} the maximum of the norms of
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="18.57" label="xicomp" -->
**Remark.** Notice that if the first case holds then by the choice of $B$ we
...
trivial. It now follows from \entryref{27b452d6cb48},
\entryref{07ac984fbe88} and \entryref{012eefbbc44a} that one of the
following two conditions holds:

- **(1)** for some $t\in J_B(c)$ the length of $\widetilde\Gamma(c)(t)$ is less than or equal to
 $e^{-C_2(t_1-t_0)}\zeta/3$ or
- **(2)** the function $A(t)$ that assigns to each $t\in
J_B(c)$ the area of the minimal spanning disk for
$p_1\widetilde\Gamma(c)(t)$ satisfies

$$
\frac{dA(t)}{dt}\le -2\pi-\frac{1}{2}R_\mathit{min}(t)A(t)
$$
...
- **(1)** there is $t_n\in J_B(c)$ such that the length of $\widetilde\Gamma_c^{\lambda_n}(t_n)$
is less than $e^{-C_2(t_1-t_0)}\zeta/2$, or
- **(2)** for every $t\in J_B(c)$, the areas of the minimal
spanning disks for $p_1(\widetilde\Gamma_c^{\lambda_n}(t))$ satisfy

$$
\frac{dA(p_1\widetilde\Gamma_c^{\lambda_n}(t))}{dt}\le -2\pi-\frac{1}{2}R_\mathit{min}(t)A(p_1\widetilde\Gamma_c^{\lambda_n}(t))+\delta''
$$
...
\entryref{52123e3c5a27} that for every $t'<t''$ in $[t_0,t_1]$ the
areas satisfy
...
$$
 ...
 Since the total length of the complement $J_B(c)$ in
$[t_0,t_1]$ is at most $3C_5B^{-1}$, it follows from our choice of
$B$ that this total length is at most the constant $\delta'$ of
\entryref{7e0b4b1362c0}. Invoking \entryref{b87df0b67468} and the fact
...
first inequality in \entryref{156a3da68fbd} to see that the
length of $\widetilde\Gamma_c^{\lambda_n}(t)<\zeta/2$ for every
$t\in [t_1-B^{-1},t_1]$. This completes the proof of
\entryref{9df99ae6f4cb}.

### The completion of the proof of Proposition \entryref{fd85732525c1}

Now we wish to pass from \entryref{9df99ae6f4cb} which deals with an
individual $c\in S^2$ to a proof of \entryref{fd85732525c1} which deals
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.58" label="muarea" -->
**Lemma.** There is a $\mu>0$ such that the following holds. Let $c,\hat c\in
...
**Proof.** First of all we require that $\mu<e^{-(2n-1)C'(t_1-t_0)}\zeta/4$
where $C'$ is an upper bound for the norm of the Riemann curvature
tensor at any point of the ambient Ricci flow. By
\entryref{6590af0738c7} the fact that the area of the minimal annulus
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.59" label="BARMU" -->
**Lemma.** There is $\delta>0$ such that for any $r>0$ there is $\bar\mu>0$,
...
the proof of \entryref{fd85732525c1} assuming it.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.60" label="mulength" -->
**Claim.** There is $\mu>0$ such that the following holds. Suppose that $c,\hat
...
**Proof.** The proof is by contradiction: Suppose that the length of
...
It then follows from
 \entryref{156a3da68fbd} that for every $t\in
...
follows from Equation (18.11) that
...
\entryref{156a3da68fbd} we see that there is a constant $C_8$
...
\entryref{8e814eab5ddf}. Then we set $\bar \mu$ equal to the constant
...
c}^\lambda(t')$. Now we have all the hypotheses of \entryref{8e814eab5ddf}
...
Now we complete the proof of \entryref{fd85732525c1}.

**Proof.** (of \entryref{fd85732525c1}.)
Fix $\mu>0$ sufficiently small so that \entryref{10dace7fb165} and
\entryref{52f36e456355} hold. Then we choose a $\mu/2$-net $X$ for
$\widetilde\Gamma$. We take $\lambda$ sufficiently small so that
\entryref{9df99ae6f4cb} holds for every $\hat c\in {\mathcal S}$. We
...
\entryref{9df99ae6f4cb} either $A(p_1\widetilde\Gamma_{\hat
...
\entryref{52f36e456355} implies that the length of
$p_1\widetilde\Gamma_c^\lambda(t_1)$ is less than $\zeta$. In the
first case, \entryref{10dace7fb165} tells us that
$A(p_1\widetilde\Gamma_c^\lambda(t_1))<v_c(t_1)+\zeta$. This
completes the proof of \entryref{fd85732525c1}.

## Proof of Lemma \entryref{8e814eab5ddf}: annuli of small area
...
\entryref{6e9739b0c59b} below gives the precise result along
these lines. Before stating that proposition, we show that its
hypotheses hold in the situation that arises in \entryref{8e814eab5ddf}.
Let us recall the situation of \entryref{8e814eab5ddf}. We have ramps
...
$\gamma\coprod \hat \gamma$. According to [Hildebrandt], $\psi$
is a real analytic map and the only possible singularities
(non-immersed points) of the image come from the *branch points*
...
[White] the only branch points on the boundary are false branch
...
at each interior branch point.
 Since the geodesic
curvature $k_\mathit{geod}$ of the boundary of the annulus is given by $k\cdot n$
where $n$ is the unit normal vector along the boundary pointing into $A$, we
see that the restriction of the geodesic curvature to $\gamma$, $k_\mathit{geod}\colon \gamma\to \Ar$ has the property that for any sub-arc $I$ of
$\gamma$ of length $r$ we have

$$
\int_I|k_\mathit{geod}|ds<\delta.
$$

 Lastly, because the map of $A$ into $M\times
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.61" -->
**Claim.** Let $\psi\colon A\subset M\times S^1_\lambda$ be an area-minimizing
...
**Proof.** Fix an interior branch point $p$.
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.62" -->
**Claim.** For all $\epsilon>0$ the Gaussian curvature of $(f+\epsilon)|dz|^2$
is at most $2C''$.
<!-- astrolabe:end -->

**Proof.** We see that $-\triangle f\le C''f^2$, so that
...
$$
h_\epsilon=(f+\epsilon \rho(r))|dz|^2.
$$

 The above computation shows that the
...
\entryref{8e814eab5ddf}, then replacing $\hat \gamma$ by a close $C^2$
...
that establishing the conclusion of \entryref{8e814eab5ddf} for a
...
With all these preliminary remarks, we see that \entryref{8e814eab5ddf}
follows from:

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="18.63" label="annlengthest" -->
**Proposition.** Fix $0<\delta<1/100$. For each $0<r$ and $C''<\infty$ there is a
...
it holds for any $0<r'<r$. This allows us to assume that $r<\mathit{min}((C'')^{-1/2},1)$. Now let us scale the metric by $4r^{-2}$. The
...
The function $k_\mathit{geod}\colon c_0\to \Ar$ is smooth. We choose a
regular value $\alpha$ for $k_\mathit{geod}$ with $1<\alpha<1.1$. In
this way we divide $c_0$ into two disjoint subsets, $Y$ where
$k_\mathit{geod}>\alpha$, and $X$ where $k_\mathit{geod}\le \alpha$. The
subset $Y$ is a union of finitely many disjoint open intervals and
$X$ is a disjoint union of finitely many closed intervals.

<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="18.64" label="Ylengthrem" -->
**Remark.** The condition on $k_\mathit{geod}$ implies that for any arc $J$ in
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.65" label="expcomp" -->
**Claim.** There is $\delta'>0$ such that the following holds.
 The
...
Now we fix $0<\delta'<1/10$ so that \entryref{47e16059fb12} holds, and
...
By a *focusing triangle* we mean the following. We have distinct
...
called the *base* of the focusing triangle and with, respect to
an orientation of $c_0$, if $x$ is the initial point of $\xi$ then
$D'_x$ is called the *left-hand side* of the focusing triangle
and $D'_y$ is called its *right-hand side*. See Fig. 18.1.
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.66" label="laest" -->
**Lemma.** Suppose that we have a focusing triangle ${\mathcal T}$ with base $\xi$ bounding a disk $B$
 in $A$.
...
**Proof.** We begin with a preliminary computation. We denote by $a(B)$ the
area of $B$. We define

$$
t_{\xi}=\int_{\xi}k_\mathit{geod}ds \ \ \ \mathit{and} \ \ \ T_\xi=\int_{\xi}|k_\mathit{geod}|ds.
$$

Recall that given a piecewise smooth curve, its *total turning*
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.67" label="theta/Kbd" -->
**Claim.** The angle $\theta_B$ between $D'_x$ and $D'_y$ at the vertex $v$
satisfies
...
$\int_BKda=\int_BK^+da+\int_BK^-da$, where $K^+=\mathit{max}(K,0)$ and
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.68" label="avgeo" -->
**Claim.** Let $a\in \mathit{int} \xi$. There is a shortest path in $B$ from $a$
to $v$. This shortest path is a geodesic meeting $\partial B$ only
in its end points. It has length $\le(1/2)+\delta'$.
<!-- astrolabe:end -->

**Proof.** The length estimate is obvious: Since $\xi$ has length at most $1$,
...
$B$ from $a$ to $v$. Fix $a\in \mathit{int}(\partial A\cap B)$. It is
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.69" -->
**Claim.** For any $a\in (\partial A\cap B)$ there is a unique minimal geodesic
in $B$ from $a$ to $v$.
<!-- astrolabe:end -->

**Proof.** Suppose not; suppose there are two $\gamma$ and $\gamma'$ from $v$
...
<!-- astrolabe:begin kind="remark" src="morgan-tian" mtref="18.70" -->
**Remark.** The same argument shows that from any $a\in (\partial A\cap B)$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.71" -->
**Claim.**
...
**Proof.** First of all notice that, by construction, the curve $\xi$, which
...
$$
l'(t)=\int_{\lambda(t)}k_\mathit{geod}(\psi)h(\psi)|\mathit{cos}(\theta(\psi,t))|ds
$$

 where $\theta(\psi,t)$ is the angle at
...
$$
\int_{W(t)}Kda+\int_{\lambda(t)}k_\mathit{geod}ds=\theta_B.
$$

From \entryref{512c74b9ad1f} we conclude that

$$
\int_{\lambda(t)}k_\mathit{geod}ds\le t_\xi+a(B).
$$

Of course, by \entryref{ea7c58d04fc7} we have $\mathit{max}_{\psi\in
...
This completes the proof of \entryref{45c783d384c1}.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.72" label="muepsbd" -->
**Corollary.** Suppose that ${\mathcal T}$ is a focusing triangle with base $\xi$
...
### No $D_x$ is an embedded arc with both ends in $c_0$

One thing that we must show is that the geodesics $D_x$ are
embedded. Here is a special case that will serve some of our
purposes.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.73" label="noloops" -->
**Lemma.** For each $x\in X$, there is no non-trivial sub-geodesic of $D_x$
which is a homotopically trivial embedded loop in $A$.
<!-- astrolabe:end -->

**Proof.** Were there such a loop, its total turning would be $\pi$ minus the
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.74" label="Dxacross" -->
**Lemma.** There is no $x$ for which $D_x$ is an embedded arc with both
endpoints on $c_0$ and otherwise disjoint from $\partial A$.
<!-- astrolabe:end -->

**Proof.** Suppose that there were such a $D_x$. Then $D_x$ separates $A$ into
...
$$
3\pi/2+\int_{c_0'}k_\mathit{geod}ds<3\pi/2+\delta,
$$
...
geodesic contained in the ball $B$. \entryref{b6fb2b9c364b} then shows
that this geodesic is embedded. This leads to:

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.75" label="locemb" -->
**Claim.** For any component $c$ of $X_J$, the restriction of the exponential
mapping to $S_c(B)=\left(c\times [0,\delta')\right)\cap S_{X_J}(B)$
is an embedding.
<!-- astrolabe:end -->

**Proof.** Since the geodesics that make up $S_c(B)$ have length at most
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.76" label="limittri" -->
**Claim.** Any focusing triangle for $J$ must contain a component of $J\setminus X_J $. If
...
**Proof.** The first statement is immediate from \entryref{14aeee986b49}.
...
Using \entryref{2ff35cef8589} we see that if there is a focusing
...
\entryref{2ff35cef8589}, that there is a focusing triangle ${\mathcal
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.77" -->
**Claim.** The interior of ${\mathcal T}_2$ is disjoint from the interior of
${\mathcal T}_1$.
<!-- astrolabe:end -->

**Proof.** Since by construction the interiors of the bases of ${\mathcal T}_1$
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="18.78" -->
**Definition.** We call the triangles ${\mathcal T}_1,\ldots,{\mathcal T}_k$
constructed above, the *maximal set of focusing triangles* for
$J$ relative to $B$.
<!-- astrolabe:end -->

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.79" -->
**Claim.** The length of $X'_J$ is at least $1-2\delta-\mu$.
<!-- astrolabe:end -->

**Proof.** Since the interiors of the ${\mathcal T}_i$ are disjoint, according
to \entryref{9c8d4163f700}, we have $\sum_il(\xi_i)<\delta+\mu$. We
also know by \entryref{bb88301ceeda} that the length of $X_J$ is at
least $(1-\delta)$. Putting these together gives the result.

We define $S_{X'_J}(B)$ to be the intersection of $S_{X_J}(B)$ with
$S_{X'_J}$.

<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.80" -->
**Claim.** The restriction of the exponential mapping to $S_{X'_J}(B)$ is an
embedding.
<!-- astrolabe:end -->

**Proof.** Suppose that we have distinct points $x',y'$ in $X'_J$ such that
...
$$
Z=\{z\in X'_J|f_B(z)<\delta'\},
$$

 the total length of $X'_J\setminus Z$
satisfies
...
### For every $x\in X$, the geodesic $D_x$ is embedded
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.81" -->
**Lemma.** For any $x\in X$ there is no sub-geodesic of $D_x$ that is an
embedded loop in $A$.
<!-- astrolabe:end -->
...
containing $x$ that is not an embedded arc. Let $\mathit{int} B$ be
the complementary component of $D'_x$ in $A$ that contains
$c_0\setminus\{x\}$. There is a natural compactification of $\mathit{int} B$ as a disk and an immersion of this disk into $A$, an
...
puts us back in the case ruled out in \entryref{0d6f74e37efb}.

### Far apart $D_x$'s don't meet
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.82" -->
**Lemma.** Let $x,y$ be distinct points of $X$. Suppose that there are
...
**Proof.** The proof is exactly the same as in \entryref{0d6f74e37efb} except that
the part of the boundary of $B$ that one wants to avoid has length
at most $2\delta'<0.2$ instead of $\delta'$. Still, since (in the
notation of the proof of \entryref{0d6f74e37efb}) the total length of
...
in the case ruled out by \entryref{0d6f74e37efb}.

As a special case of this result we have the following.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.83" label="noholes" -->
**Corollary.** Suppose that we have an arc $\xi$ of length at most $1$ on $c_0$.
...
**Proof.** If not, then it is homotopically non-trivial in $A$ and replacing
$\xi$ by its complement, $c_0\setminus\mathit{int} \xi$, gives us
exactly the situation of the previous lemma. (The length of
$c_0\setminus\mathit{int} \xi$ is at least one since the length of
$c_0$ is at least $2$.)

Let us now summarize what we have established so far about the intersections of
the geodesics $\{D_x\}_{x\in X}$.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.84" label="focussummary" -->
**Corollary.** For each $x\in X$, the geodesic $D_x$ is an embedded arc in $A$.
...
content of \entryref{ad36aef968b6}. The same reasoning works when we
...
has both endpoints in $\partial A$. By \entryref{0d6f74e37efb}, the
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.85" -->
**Claim.** For every subinterval $J$ of length one in $c_0$ the total length of
the bases $\xi_i$ that meet $J$ is at most $2\delta+\mu<0.03$.
<!-- astrolabe:end -->

**Proof.** Since, by \entryref{9c8d4163f700}, every base of a focusing triangle
...
embedding, it follows from \entryref{47e16059fb12} and the choice of
...
## Proof of the first inequality in Lemma \entryref{38712e8cea3d}

Here is the statement that we wish to establish when the manifold
$(W,h(t))$ is the product of $(M,g(t))\times (S^1_\lambda,ds^2)$.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="18.86" label="csshi1" -->
**Lemma.** Let $(W,h(t)),\ t_0\le t\le t_1$, be a Ricci flow and fix
...
[AG] such a local estimate was established when the ambient
...
[EckerHuisken]. The passage from Euclidean space to a general
...
flow by an equivalent flow, introduced in [AG], called the
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.87" label="C0claim" -->
**Claim.** There is a constant $D_0<\infty$, depending only on $\Theta$ and the
...
**Proof.** This is immediate from \entryref{156a3da68fbd} applied to all
of $c(\cdot,t)$.
...
$$
\int_{\gamma_t}k^2 ds\le \mathit{max}_{x\in\gamma_t}k(x,t)\int_{\gamma_{t}} kds
<\sqrt{\frac{2}{t-t'}}\cdot D_0.
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.88" label="1887" -->
**Claim.** There is a constant $D_1$ depending only on the curvature bound for
...
$$
\left|\frac{\partial s(x,t)}{\partial t}\right| =
...
$$
\frac{\partial
|X(y,t)|}{\partial y}dy  = \left(-\mathit{Ric}(S(y,t),S(y,t))-k^2(y,t)\right)ds,
$$
...
Dl(\gamma_t)+\int_{x_0}^xk^2(y,t)ds(y,t),
\end{aligned}
$$

 and hence
by \entryref{1b8d34cae3a8}
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.89" label="claim1885" -->
**Claim.** There is a constant $D_2$ depending only on the curvature bound of
...
$$
 ...
 Using \entryref{80e5fc6759b2} for the first
term and \entryref{67b855d0d6b5} and arguing as in the proof of
\entryref{88b8b3366b19} for the second term, we have

$$
\left|\frac{d}{dt}\int_{\gamma_t}\varphi
kds\right|\le
D_1\left(1+\frac{1}{r\sqrt{t-t'}}\right)\int_{\gamma_t}kds+\left|\int_{\gamma_t}
\varphi k''ds\right|+\int_{\gamma_t}C'_1\varphi kds,
$$
...
curvature bound from \entryref{1b8d34cae3a8}. Since the ends of
...
\frac{D'}{r^2}\int_{c(\cdot,t)}kds\le \frac{D'D_0}{r^2}.
$$
...
$$
 ...
 for $D_2=D_0 \mathit{max}(D',D_1)$ and $D_3=C_1'$. This gives
the required estimate.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.90" -->
**Corollary.** For any $t\in [t',t_2]$ and any sub-arc $\gamma_{t'}$ of length $r$
we have
...
**Proof.** This is immediate from the previous result by integrating from $t'$
to $t_2\le t'+\delta r^2$, and using the fact that $\delta<1$ and
$r<1$, and using the fact that

$$
\int_{\gamma_{t'}}\varphi kds\le
\int_{\gamma_{t'}}kds<\delta
$$

 since $\gamma_{t'}$ has length at
most $r$.

This gives:

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="18.91" label="D_4cor" -->
**Corollary.** For $\gamma_{t'}\subset c(\cdot,t')$ a sub-arc of length at most $r$
...
**Proof.** We divide an interval $\gamma_{t'}\subset c(\cdot,t')$ of length at
...
- **(1)** $\left|\langle X,Y\rangle_{h(t)}-\langle
X,Y\rangle_{h'}\right|<\delta |X|_{h'}|Y|_{h'}$ for all tangent vectors in the
coordinate system, and
- **(2)** viewing the connection $\Gamma$ as a bilinear
...
[AG] (in the case of Euclidean background metric), is defined
...
$$
 ...
 The time partial derivative in the graph-flow is given by
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.92" label="curvflowcompare" -->
**Claim.**
...
**Proof.** In our Euclidean coordinates, $Z=(1,f_z)$ so that
$\nabla_ZZ=(0,f_{zz})+\Gamma(Z,Z)$. Thus,

$$
\langle \nabla_ZZ,\partial_z\rangle_{h'}=\langle
\Gamma(Z,Z),\partial_z\rangle_{h'}.
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.93" label="1891" -->
**Claim.** Provided that $\delta>0$ is sufficiently small,
...
**Proof.** Under the hypothesis that $|f_z|_{h'}\le 1/10$, it is easy to see
...
$|Y|=|\nabla_SS|\left(\mathit{cos}(\theta)\right)^{-1}$, where $\theta$
...
$$
\frac{\nabla_ZZ}{|Z|^2}=Y+\psi(Z)Z,
$$

 and $|Z|^2\le (1.01)(1+\delta)$, the third item is
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.94" label="psiclaim" -->
**Claim.** The following hold provided that $\delta>0$ is sufficiently small:
...
**Proof.** For the first item, we write $Z(\psi(Z))$ as a sum of terms where the
...
proof of the first equation in of \entryref{4fc24fa1d31e} we have we
have

$$
\begin{aligned}
\frac{\partial}{\partial t}|Z|^2
& = & -2\mathit{Ric}(Z,Z)+2\langle \nabla_YZ,Z\rangle \\
& = & -2\mathit{Ric}(Z,Z)+2\langle \nabla_ZY,Z\rangle
\end{aligned}
$$
...
$$
 ...
 Thus
from the \entryref{188acafae968}, we have
...
$$
V=-4|Z|^2\langle Y,\psi(Z)Z\rangle-2\psi^2(Z)|Z|^4-2\langle
\nabla_Z(\psi(Z)Z),Z\rangle-2\mathit{Ric}(Z,Z).
$$

 By Item (1) in
\entryref{d3f828726e2b} and Item (4) in \entryref{8973f9acd2ae} we have
...
\entryref{8973f9acd2ae}, the first term is at most
...
$$
 ...
 Since $|f_z(z,t')|_{h'}\le 2\delta$ and $|Z|^2$ is
...
+C_1''(\sqrt{\delta}r+\delta r^3+\delta^{3/2}r^2).
$$
...
1/20$. Since by \entryref{55b7b71de6e4}
...
$z$-axis
 ...
 We follow closely the discussion in Section 4 of [AG] (pages 293 -294).
...
Y,Z\rangle \psi(Z)|Z|^2-4\psi^2(Z)|Z|^4
\end{aligned}
$$

 Thus, it
follows from Equation (18.15) that we have
...
\frac{(|Y|^2)_{zz}}{|Z|^2}-\frac{2|\nabla_ZY|^2}{|Z|^2}
 ...
 \nabla_ZY,Z\rangle \\
 &  & -2\mathit{Ric}(Y,Y)+2\frac{\mathit{Rm}(Y,Z,Y,Z)}{|Z|^2}-2\langle \nabla_Y(\psi(Z)Z),Y\rangle.
\end{aligned}
$$
...
$$
 ...
 Hence, putting all this together and using \entryref{d3f828726e2b}
we have
...
the computations on p. 294 of [AG] and adding in the error terms.
...
From \entryref{8973f9acd2ae} we have
...
Using Item (4) of \entryref{8973f9acd2ae} this leads to
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="18.95" -->
**Claim.**
...
**Proof.** Since $Y=(0,\phi)$ for some function $\phi$, we have
...
$$
 ...
 Since both $h$ and $h_{zz}$ are positive, at any point
...
$$
 ...
 Thus, for any fixed $t$, at any local maximum for
...
continuous function $f(t)=\mathit{max}_{z\in I'}(\widetilde Q-h)(z,t)$,
...
(2-|Z(x_0,t)|^2)Q(x_0,t)< \frac{2}{(t-t')}
$$
...
[t',t_2]$. Since $x_0$ was an arbitrary point of $c(\cdot,t')$,
 ...
 proof of \entryref{ff5d51a483d9}.
