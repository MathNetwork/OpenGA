...
# Chapter 13 -- Surgery on a $\delta$-neck
...
called the *tip*
of the standard initial metric. Recall from \entryref{0b07beec84c0} that
there are $A_0>0$ and an isometry

$$
\psi\colon (S^2\times
(-\infty,4],h_0\times ds^2)\to (\Ar^3\setminus B(p_0,A_0),g_0).
$$
...
<!-- astrolabe:begin kind="claim" src="morgan-tian" mtref="13.1" label="distdecrease" -->
**Claim.** Let $\xi\colon N\to \Ar^3 $ be the map that sends $\rho(S^2\times
...
\entryref{545ca0a10566} the map $\xi$ given in the statement of the
...
$$
\hat g=\begin{cases} \mathit{exp}(-2f(s))R(x_0)\rho^*g & \ \ \mathit{on} \ \
s^{-1}(-\infty,1] \\
\mathit{exp}(-2f(s))\left(\alpha(s)R(x_0)\rho^*g+(1-\alpha(s))\eta
g_0\right)  & \ \ \mathit{on}\ \  s^{-1}([1,2]) \\
\mathit{exp}(-2f(s))\eta g_0 & \ \ \mathit{on}\ \ s^{-1}([2,A_{r_0}]\\
\left[\beta(s)\mathit{exp}(-2f(s))+(1-\beta(s))\mathit{exp}(-2f(4+A_0))\right]\eta
g_0 & \ \ \mathit{on} \ \ s^{-1}([A_{r_0},A']),\end{cases}
$$
...
<!-- astrolabe:begin kind="theorem" src="morgan-tian" mtref="13.2" label="LOCALSURGERY" -->
**Theorem.** There are constants $C_0,q,R_0<\infty$ and $\delta'_0>0$ such that the
...
be the map given in \entryref{6168ffcb3bed}. Then it is a distance
...
<!-- astrolabe:begin kind="definition" src="morgan-tian" mtref="13.3" label="surgerycap" -->
**Definition.** The image in ${\mathcal S}$ of $B_{g_0}(p_0,0,A_0+4)$ is called the *surgery cap*.
<!-- astrolabe:end -->

The following is immediate from the definitions provided that
$\delta>0$ is sufficiently small.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="13.4" label="capball" -->
**Lemma.** The surgery cap in $({\mathcal S},\tilde g)$ has a metric that
...
$$
f_{ij}=\mathit{Hess}_{ij}(f)=\partial_if_j-f_l\Gamma_{ij}^l,
$$
...
$$
|\nabla f|_h=\frac{q}{s^2}f\cdot(1+O(\delta)),
$$
...
$$
f_{i0}=\frac{q}{s^2}fO(\delta)\ \ \ \mathit{for}\ \ 1\le i\le 2,
$$

$$
f_{ij}=\frac{q}{s^2}fO(\delta)\ \ \ \mathit{for}\ \ 1\le i,j\le 2.
$$

In the following $a,b,c,d$ are indices taking values $1$ and $2$.
Substituting in Equation (13.1) yields
...
$$
q \gg (4+A_0)^2\ \ \ \mathit{and}\ \ \ \frac{q^2}{s^4}e^{-q/s} \ll 1.
$$
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="13.5" label="delta0" -->
**Corollary.** There is $\delta'_2>0$, depending on $C_0$ and $q$,
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="13.6" label="Rcompar" -->
**Corollary.** For any constant $C_0<\infty$ and any $\delta<\mathit{min}(\delta_2',C_0^{-1})$ we
have $\hat R\ge R$.
<!-- astrolabe:end -->

**Proof.** By our choice of $q$, since $C_0\delta<1$, then $f^2\ll f$ and $q^2/s^4\gg \mathit{max}(q/s^3,q/s^2)$ so that the result follows immediately from the above
formula.

Now let us compute the eigenvalues of the curvature $ R_{ijkl}(y)$
for any $y\in S^2\times I$.

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="13.7" label="newbasis" -->
**Lemma.** There is a $\delta_3'>0$ such that the following hold if $\delta\le
...
expressing the $\{f_a\}$ in terms of the $\{e_b\}$, so that $A=\mathit{Id}+O(\delta)$. The curvature of $h$ in this basis is then given by
$B^\mathit{tr}(R_{ijkl})B$ where $B=\wedge^2A$ is the induced change of basis matrix
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="13.8" label="2ndnewbasis" -->
**Corollary.** The following holds provided that $\delta\le \delta_3'$. It is
possible to choose the basis $\{f_0,f_1,f_2\}$ satisfying the
conclusions of \entryref{52d3cb562bdc} so that in addition the
...
$$
 ...
 It follows that the restriction to the $h$-unit sphere
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="13.9" label="Rhatmatrix" -->
**Lemma.** Set $\delta'_4=\mathit{min}(\delta_2',\delta_3')$. Suppose
that $\delta\le \mathit{min}(\delta'_4,C_0^{-1})$. Then in the basis
$\{f_0,f_1,f_2\}$ for $T_y(S^2\times I)$ as in \entryref{ad2042196c95} we
have
...
\entryref{ad2042196c95} and the first matrix is the expression for
$(R_{ijkl}(y))$ in this basis.
<!-- astrolabe:end -->

**Proof.** We simply conjugate the expression in Equation (13.3) by the change of
basis matrix and use the fact that by our choice of $q$ and the fact that
$C_0\delta<1$, we have $f\gg  f^2$ and $q/s^3\ll q^2/s^4$.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="13.10" label="13.10" -->
**Corollary.** Assuming that $\delta\le \mathit{min}(\delta'_4,C_0^{-1})$, there is an
...
the expression in \entryref{52d3cb562bdc} gives the result.

<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="13.11" label="improve" -->
**Corollary.** There is a constant $A<\infty$ such that the following holds for the given value of $q$
...
\entryref{347896f0385b}. Then $\{e^ff_0,e^ff_1,e^ff_2\}$ is orthonormal for $\hat
...
basis is exactly the same as in \entryref{0a41b5c61dd2} except that the factor
...
<!-- astrolabe:begin kind="corollary" src="morgan-tian" mtref="13.12" label="smeigen" -->
**Corollary.** For the given value of $q$ and for any $C_0$,
assuming that $\delta>0$ is sufficiently small, the smallest
eigenvalue of $\mathit{Rm}_{\hat h}$ is greater than the smallest eigenvalue
of $\mathit{Rm}_h$ at the same point. Consequently, at any point where $h$
...
scale one, every eigenvalue of $\mathit{Rm}_h$ is at least $-K\delta$.
We set
...
<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="13.13" label="nuprime&gt;0" -->
**Lemma.** With these choices of $q$ and $C_0$ for any $\delta>0$ sufficiently small we have
...
## The proof of Theorem \entryref{cf61b731c6f2}
...
The first conclusion of \entryref{cf61b731c6f2} follows by
applying the above considerations to the case of
$h=R_g(x_0)\rho^*g$. Namely, we have:

<!-- astrolabe:begin kind="proposition" src="morgan-tian" mtref="13.14" -->
...
**Proof.** According to \entryref{e70bacf8aab0}, the smallest eigenvalue of $\hat
h$ at any point $p$ is greater than or equal to the smallest
eigenvalue of $h$ at the corresponding point. According to
\entryref{7f0b4a692c5e}, $\hat R(p)\ge R(p)$ for every $p\in
{\mathcal S}$. Hence, $X_{\hat h}(p)\le X_h(p)$. If $X_h(p)\ge
e^3/(1+t)$, then we have

$$
\hat R(p)\ge R(p)\ge 2X_h(p)(\mathit{log}(X_h(p)+\mathit{log}(1+t)-3)\ge 2X_{\hat h}(p)
(\mathit{log}(X_{\hat h}(p)+\mathit{log}(1+t)-3).
$$
...
$$
\hat R(p)\ge R(p)\ge 0>2X_{\hat p}(p)(\mathit{log}(X_{\hat h}(p)+\mathit{log}(1+t)-3).
$$

This completes the proof in both cases.

This establishes the first item in the conclusion of
\entryref{cf61b731c6f2} for $\delta>0$ sufficiently small on
$s^{-1}(-\delta^{-1},4)$. As we have seen in \entryref{cc9110b4d2fe},
...
from \entryref{e70bacf8aab0} and the fact that $\eta g_0$ has non-negative
...
g$. On the other hand, it follows from \entryref{6168ffcb3bed} that
...
This completes the proof of \entryref{cf61b731c6f2}.

## Other properties of the result of surgery

<!-- astrolabe:begin kind="lemma" src="morgan-tian" mtref="13.15" label="stdballs" -->
**Lemma.** Provided that $\delta>0$ is sufficiently small the following holds. Let $(N,g)$
