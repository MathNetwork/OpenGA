...
$T'=T$ and $g'(t)=g(t)$.
\item[(2)] **(Time Interval):** $T=1$.
\item[(3)] **(Positive curvature):** For each $t\in (0,1)$ the metric $g(t)$ on $\Ar^3$ is complete
of strictly positive curvature.
\item[(4)] **($SO(3)$-invariance):** For each $t\in [0,T)$ the
Riemannian manifold $(\Ar^3,g(t))$  is invariant under the
$SO(3)$-action on $\Ar^3$.
\item[(5)] **(Asymptotics at $\infty$):**
...
$S^2$.
\item[(6)] **(Non-collapsing):** There are $r>0$ and $\kappa>0$ such that $(\Ar^3,g(t)),\ 0\le
...
curvature, see , but we preferred to give the more
...
applied to Equation~(), $\partial R/\partial t=\triangle
R+|Ric|^2$, then implies by Proposition~ that
the maximum of $R$ at time $t$, $R_max(t)$ obeys the
inequality $\partial R_max/\partial t\le R_max(t)^2$,
and integrating this inequality (i.e., invoking
Lemma~) one finds a positive constants $t_0$ and
$Q_0$ such that for each $n$, the norm of the scalar curvature of
$g_{R_n}(t)$ are bounded by $Q_0$ on the interval $[0,max(t_0,T_n))$. By Corollary~, for each $n$ the sectional
...
from Proposition~ and the fact that the $T_n$ are
...
Theorem~ that there is a geometric limiting flow
...
the argument is the analogue of the proof of Corollary~
...
complete and of bounded curvature, according to
 for any constants $C<\infty$ and $\eta>0$ and
...
Recall from Section~ that ${\mathcal T}$ is the curvature
...
[0,t_0]$. It follows from Proposition~ that
...
Lemma~ for the evolution of ${\mathcal T}$ in an evolving
orthonormal frame:
...
$$\frac{\partial \widehat{\mathcal T}}{\partial t}\ge
 \triangle \widehat{\mathcal T}+(Cg/2-2Ric_g)\varphi.$$ Once again assuming that $C$ is
...
This immediately implies by Proposition~ that
...
Indeed, by Corollary~ it has strictly positive
...
Theorem~ that there are uniform $C^\infty$
point-wise bounds for the curvatures of $g(t)$ restricted to
$\psi_k(S^2\times (-R,R))$. Thus,
 by Theorem~, after passing to a subsequence, the flows
...
flow have two ends, it follows from Theorem~ that
every manifold in the flow is a product of a compact surface with
$\Ar$. According to Corollary~ this implies that the
...
non-compact manifolds due to Chen and Zhu (), but we
...
$$
\frac{\partial}{\partial t}X=\triangle X+Ric(X,\cdot)^*.
$$
...
$$
\frac{\partial}{\partial t}X^{i}=(\Delta X)^{i}+Ric_{k}^{i}X^{k}
$$
...
V-\left(2{{R_k}^{rl}}_jV_{rl}+Ric_k^lV_{lj} +Ric_j^lV_{kl}\right)dx^k\otimes dx^j.
$$
...
and consequently, plugging back into Equation~(), and
...
h_{ij}+2{{R_i}^{kl}j}h_{kl}-Ric_i^k h_{kj}-Ric_j^kh_{ki}$ is the **Lichnerowicz
...
Corollary~, since $h(0)=0$ and $h\ge 0$, it follows
...
$\Delta X^i +R^i_kX^k =0$ for all $t$. Hence () gives
...
$R_max(t_1)=C/(1-Ct_1)$. Since $C\ge sup_{x\in
...
Proposition~ for all $t\in [t_1,t_2]$ we have
$$R_max(t)\le G(t)$$
...
that $l_x(q,t_0)\le 3/2$; this by Theorem~. Since
$B(q,0,1/\sqrt{R_max(0)})\subset \Ar^3$ has volume at least
$V/R_max(0)^{3/2}$, and clearly $l_x$ is bounded above on
$B(q,0,1/\sqrt{R_max(0)})$ by a uniform constant, we see that
the reduced volume of $B(q,0,1/\sqrt{R_max(0)})$ is uniformly
bounded from below. It now follows from Theorem~ that there
...
. The presentation here is the analogy in the context
...
solution of~ with $\psi_{t_0}=Id$. Then $\psi\left(
...
Equation~(); then $\hat g(t)=(\psi_t^{-1})^*g(t)$ solves
the Ricci-DeTurck flow, Equation~() and $\psi_t$
...
existence of the harmonic flow () and its asymptotic
...
\psi(t)(r,\theta) & =(\rho(r,t),\theta) \ \ \ for\ \ t\ge t_0 \\
\psi(r,t_0) & = Id \nonumber
\end{aligned}
$$
...
As a corollary of Claim~ we have:
...
smooth functions as given in Equation~(). Using
() and () it is easy to calculate the energy
...
letting $dvol_\sigma$ denote the standard volume element on
$S^{n-1}$, we have
...
\rho}f^{n-3}(r,t)w \right] dr \cdot \int_{S^{n-1}} dvol_\sigma \\
...
\right] dr \cdot \int_{S^{n-1}} dvol_\sigma\\ &=
...
$w$ we have
$$\delta_w\left(\frac{1}{2}\int_{\Ar^n}|\nabla_{g(t),g(t_0)}\psi|^2dvol=\int_{\Ar^n}\langle
w,-\triangle_{g(t),g(t_0)}\psi\rangle dvol\right).$$ Thus,
...
Equation~() the harmonic map flow equation ()
...
### An equation equivalent to the harmonic map flow
equation

We will solve ()  for solutions of the form

$$
\rho(r,t)= re^{\tilde{\rho}(r,t)}, \ \ \ t\ge t_0;\ \ \ \tilde \rho(r,t_0)=0.
$$

For $\psi$ as in Equation~() to define a diffeomorphism,
...
calculation shows that () becomes
...
where $\xi$ is the function from Corollary~. Then
...
We have the following form of equation ():
...
**Proof.** The first item is immediate from  Proposition~. The
second and third follow immediately from the first. The fourth is a
consequence of the fact that by Proposition~ $dr/dt$ is
...
to Equation~() with initial condition
...
be the heat kernel. We solve () by successive approximation
.
...
Claim~ that $G(\tilde{\rho}_{i-1},**x**,t)$ is bounded
...
and also because of Claim~ both $|\nabla B|$ and
...
$$
\begin{aligned}
|\tilde{\rho}_{i}| \leq C_1 \qquad and \qquad  |\nabla
...
() via proving that it is a Cauchy sequence in
...
& + G( \tilde{\rho}_{i-1},**x**,t)-G(\tilde{\rho}_{i-2},**x**,t)
\end{aligned}
$$
...
Lipschitz property of $G(\tilde{\rho},**x**,t)$
...
From Equations~() and~() we can estimate
...
From () we conclude that $\tilde{\rho}_{\infty}$ is
a solution of () on $\mathbb{R}^{n+2} \times
...
follows from () and Claim~ that there is
...
$$
\begin{aligned}
& | G(\tilde{\rho}_i,**x**,t) | \leq \frac{C_8}{(1+|x|)^2} \\
...
then () hold for all $i$. From the definition of
...
$$
| G(\tilde{\rho}_\infty,**x**,t)|_{C^{\alpha,\alpha/2}} \leq
\frac{C_9}{(1+|x|)^2}.
$$
...
& |\nabla G(\tilde{\rho}_\infty,**x**,t)|_{C^{\alpha,\alpha/2}}
...
$\psi_t(**x)** = x e^{\tilde{\rho}(**x**,t)}$ to the harmonic
map flow
...
and $ |\nabla^i \tilde{\rho}|(**x**,t)\leq \frac{C_{14}}{(1+|**x**|)^2}$ for $0 \leq i \leq 3$ defined on some non-degenerate
interval $[t_0,T']$.
...
submanifolds of $\Omega_{k}\subset M$, i.e., $\Omega _{k}\subsetint\Omega_{k+1}$ and $\cup\Omega_{k}=M$ such that $\hat g_1\left(
...
$g$, then, using the Ricci-DeTurck flow () for
...
g_2(0)  \right\vert _{g}^{2}=0$. Using Equation~() and applying the maximum principle to $\left\vert \hat g_1(
...
By Proposition~ there are $\psi_1(t)$  and
...
$\hat{g}_1(t_0)$ as required in Proposition~. It follows
from Lemma~ and the decay estimate in
Proposition~ that $\hat{g}_1(t)$ and $\hat{g}_2(t)$
are bounded solutions and that they have same sequential asymptotic
behavior at infinity. We can apply Proposition~ to
...
Proposition~ to construction solutions $\psi_1$ and
...
Theorem~ for the standard flow except for the fact that
...
$(x_n,t_n)$. To prove the theorem we must show that $\overlinelim_{n\rightarrow\infty}R(x_n,t_n)<\infty$. Suppose the contrary.
...
Theorem~ (which includes the last hypothesis of
Theorem~) is satisfied. This is clear since
$t_n\rightarrow T$ as $n\rightarrow \infty$ and $Q_n\rightarrow
\infty$ as $n\rightarrow \infty$. Applying Theorem~ we
conclude that after passing to a subsequence there is a limiting
flow which is a $\kappa$-solution. Clearly, this and
Corollary~ imply that for all sufficiently large
...
**Proof.** We have already seen in Corollary~ that $T\le 1$. Suppose now
that $T<1$. Take $T_0<T$ sufficiently close to $T$. Then according
to Proposition~ there is a compact subset $X\subset
...
Theorem~ and the fact that the standard flow is
...
exist. This means that $\overlinelim_{t\rightarrow
...
Theorem~ the second hypothesis in the statement of
Theorem~ holds for the sequence
$(\Ar^3,g(t),(x,t_n))$. All the other hypotheses of this theorem as
well as the extra hypothesis in Theorem~ obviously
hold for this sequence. Thus, according to Theorem~
the based flows $(\Ar^3,Q_ng(Q_n^{-1}t'+t_n),(x,0))$ converge
smoothly to a $\kappa$-solution. Since the asymptotic volume of any
$\kappa$-solution is zero (see Theorem~), we see that
...
(Proposition~) that for any $0<A<\infty$ and any
...
Lemma~ that the distance is a non-increasing
...
Lemma~ we see that the curvature of $g(T)$ must be
unbounded at spatial infinity. On the other hand, by
Proposition~ every point of $(\Ar^3,g(1))$ of
...
contradicts Proposition~. (One can also rule this
...
to Theorem~ the subset $\Omega$ is open and the metrics
...
Theorem~ we see that, possibly after passing to a
subsequence, the based flows $(\Ar^3,Q_ng(t'-t_n),(p,0))$ converge
to a $\kappa$-solution. Then by Proposition~ for any
...
By the Bishop-Gromov inequality (Proposition~) it
...
**Proof.** By Theorem~, there is $r_0$ such that if $R(x,t)\ge
...
for rescaled time $(1+\epsilon)$. By Proposition~, there
is $\theta<1$ such that if $R(x,t)\le r_0^{-2}$ then $t\le \theta$.
By Proposition~,  there is a compact subset $X\subset \Ar
...
the surgery cap. Also, $A_0$ is defined in Lemma~.

**Corollary.**
For any $\epsilon>0$ let $C'=C'(\epsilon)$ be as in
Theorem~. Suppose that we have a sequence of
generalized Ricci flows $({\mathcal M}_n,G_n)$, points $x_n\in
{\mathcal M}_n$ with $**t**(x_n)=0$, neighborhoods $U_n$ of $x_n$
...
**Proof.** This follows immediately from Theorem~ and
Proposition~.
