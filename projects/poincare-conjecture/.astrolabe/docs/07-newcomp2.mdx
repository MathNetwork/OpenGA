# Complete Ricci flows of bounded curvature
 ...
 In this chapter we establish strong results for ${\mathcal L}exp_x$ in the case of ordinary Ricci flow on complete $n$-manifolds
...
Section~ at the `smooth points' to weak
...
Throughout Chapter~ we have a Ricci flow $(M,g(t)),\
...
and the inequality in Equation~() that there is a
...
that $\int_{s'}^s|A(s)|^2ds\le lim_{i\rightarrow\infty}\int_{s'}^s|A_i(s)|^2ds$, so that
${\mathcal L}(\gamma)\le lim_{i\rightarrow\infty}{\mathcal
...
Lemma~, and the fact that the quantities $R$, $|\nabla
R|$ and $|Rm|$ are bounded on $M\times [\epsilon,T]$, that
there is a bound on $max|\gamma'(s)|$ in terms of
...
Definition~ for every $\tau>0$, the injectivity set
...
${\mathcal L}exp_{x}^\tau$ is an isomorphism at $Z$, and (iii)
...
$L_{x}$ given in Definition~. We define
...
U}_{x}(\tau)$_x(\tau)$} and the function $l_{x}$
...
on $M\times (0,T)$.
\item[(2)] ${\mathcal L}exp_{x}^\tau$
 is a diffeomorphism from $\widetilde{\mathcal U}_{x}(\tau)$ onto
an open subset ${\mathcal U}_{x}(\tau)$_x(\tau)$} of $M$.
...
**Proof.** By Shi's Theorem (Theorem~) the curvature bound on $M\times
[0,T]$ implies that for each $\epsilon>0$ there is a bound for
$|\nabla R|$ on $M\times (\epsilon,T]$. Thus,
Proposition~ shows that $L_{x}$ is a
...
second statement is contained in Proposition~, and the
last one is contained in Proposition~. It remains to
...
differential inequality given in
 Corollary~ to a differential
inequality in the weak or distributional sense for $l_{x}$ valid on
all of $M\times (0,T)$. We then use this to establish that $min_{q\in M} l^\tau_{x}(q)\le n/2$ for all $0<\tau<T$.
...
Lemma~ there is  a minimizing ${\mathcal L}$-geodesic $\gamma$
...
Consider ${\mathcal L}exp_{(q_1,t_1)}\colon T_{q_1}M\times (\tau_1,T)\to
M\times (0,t_1)$. According to Proposition~ there is a
neighborhood $\widetilde V$ of $\{\sqrt{\tau_1}X_\gamma(\tau_1)\}\times
(\tau_1,\tau]$ which is mapped diffeomorphically  by ${\mathcal L}exp_{(q_1,t_1)}\colon T_{q_1}M\times (\tau_1,\tau)\to M\times (0,t_1)$ onto a
...
$(q,\tau)$. According to Lemma~ we have
...
By Lemma~,  it follows easily that
$$lim_{\tau_1\rightarrow 0^+}
...
**Proof.** We set $l_min(\tau)=inf_{q\in
...
$(q,\tau)\in M\times I$
 for which $l_x(q,\tau)=l_min(\tau)$ is a compact set.

 First, let us assume this claim and use it to prove the
theorem. We set $l_min(\tau)=min_{q\in M}l_x(q,\tau)$.
...
Proposition~) that $l_min(\tau)$ is a
...
Since $l_x(q,\tau)=l_min(\tau)$, it follows that
$\varphi(q',\tau)\ge \varphi(q,\tau)$ for all $(q',\tau)\in U\cap
M_{T-\tau}$. This means that $\triangle \varphi(q,\tau)\ge 0$, and
we conclude that
$$\frac{d\varphi}{d\tau}(q,\tau)\le \frac{(n/2)-l_min(\tau)}{\tau}+\epsilon.$$
Since $\varphi$ is an upper barrier for $l_x$ at $(q,\tau)$ it
follows immediately that
$$limsup_{\tau'\rightarrow
\tau^+}\frac{l_x(q,\tau')-l_x(q,\tau)}{\tau'-\tau}\le
\frac{(n/2)-l_x(q,\tau)}{\tau}+\epsilon.$$ Since this is true for every
$\epsilon>0$, we see that
$$limsup_{\tau'\rightarrow
\tau^+}\frac{l_x(q,\tau')-l_x(q,\tau)}{\tau'-\tau}\le
\frac{(n/2)-l_x(q,\tau)}{\tau}.$$ Since $l_min(\tau)=l_x(q,\tau)$, the same inequality holds for the forward
difference quotient of $l_min$ at $\tau$. That is to say, we
have
$$limsup_{\tau'\rightarrow \tau^+}\frac{l_min(\tau')-l_min(\tau)}{\tau'-\tau}\le \frac{(n/2)-l_min(\tau)}{\tau}.$$
The preceding equation implies that if $l_min(\tau)\le n/2$
then $l_min(\tau')\le n/2$ for every $\tau'\ge \tau$. On the
other hand $lim_{\tau\rightarrow 0}l_min(\tau)=0$. Then
reason for this is that the path $\tau'\mapsto (P,T-\tau')$ for
$\tau'\in [0,\tau]$ has ${\mathcal L}$-length $ O(\tau^{3/2})$ as
$\tau\rightarrow 0$. It follows that $l_min(\tau)<n/2$ when
$\tau$ is small.

To complete the proof of Theorem~, it remains to prove
Claim~.
...
Inequality~() there is a constant $C$ such that for
all $t,t'\in [0,T]$ we have
$$C^{-1}g(t')\le g(t)\le Cg(t').$$
For any compact interval $I\subset (0,T)$, there is $l_0<\infty$
such that $l_min(\tau)\le l_0$ for all $\tau\in I$. According
to Corollary~, for every $\bar\tau\in I$ and all
...
$l_x(q,\tau)=l_min(\tau)$ is clearly a closed set, it follows
that the subset of $M\times I$ of all $(q,\tau)\in M\times I$ for
which $l_x(q,\tau)=l_min(\tau)$ is compact. The last thing to
...
continuity of $l_x$ we have $l_x(q,\tau)=lim_{n\rightarrow
\infty}l_x(q_n,\tau)=inf_{q'\in M}l_x(q',\tau)$, so that
...
that $x\in {\mathcal M}$ is given and set $t_0=**t**(x)$. We suppose that
there is an open subset $U\subset **t**^{-1}(-\infty,t_0)$ with the following
properties:
\begin{enumerate}
...
that $P\cap **t**^{-1}(-\infty,t_0)$ is contained in $U$.
...
$y\in **t**^{-1}(I)\cap U$ for which ${\mathcal L}(y)=inf_{z\in **t**^{-1}(**t**(y))\cap U}{\mathcal L}(z)$ is compact and non-empty.
\end{enumerate}
 Then
for every $t<t_0$ the minimum of the restriction of $l_x$ to the time-slice
$**t**^{-1}(t)\cap U$ is at most $n/2$.

### Extension of the other inequalities in
 Corollary~\protect{}

The material in this subsection is adapted from . It
...
two inequalities in Corollary~, namely
...
\phi\Bigr]\ dvol(g(t))\le 0.$$ Furthermore, equality holds in
...
l_x^\tau|^2$ are elements of $L_loc^\infty(M)$ and hence can
...
$$\int\triangle \phi\cdot l^\tau_x\ dvol(g(t))=-\int \langle \nabla \phi,\nabla
l_x^\tau\rangle\ dvol(g(t)).$$ Thus, formulas in
Theorem~ can also be taken to mean:
$$\int_{M\times \{\tau\}} \Bigl[\phi\cdot(\frac{\partial l_x}{\partial \tau}
+|\nabla l^\tau_x|^2-R+\frac{n}{2\tau})+ \langle \nabla
l_x^\tau,\nabla \phi \rangle\Bigr] dvol(g(t))\ge 0$$
$$\int_{M\times\{\tau\}} \Bigl[\phi\cdot(-|\nabla
l^\tau_x|^2+R+\frac{l^\tau_x-n}{\tau})-2\langle\nabla
l^\tau_x,\nabla\phi \rangle\Bigr] dvol(g(t))\le 0.$$
...
**Proof.** By Proposition~, $L$ is a locally Lipschitz
...
in Lemma~.) Fix $\tau_1>0$, with $\tau_1<(T-t)/2$, let
...
function $C(q,t)$ defined on all of $M\times (0,T)$.
 ...
 Now consider Inequality~() for $\gamma$ at $\bar\tau=\tau$.
...
this it follows immediately from Equation~() that $H(X,Y)$
...
At this point in the proof of Theorem~ we wish to employ
...
from Lemma~ is defined on all of $B_{(q,t)}$.
\item The constants $C(z,t)$ of Lemma~ satisfy
...
Formula~(), for any $z\in B_{(q,t)}$ and any $v\in
T_zM$, we have
...
What follows is a simple interpolation result (see ).
...
$\int_{B_0}\xi dvol_h=1$. We define
...
sufficiently small, the conclusion of Corollary~ holds
...
$$\int_{(B_{(q,t)}')^*}\psi dvol(g(t))=lim_{\epsilon\rightarrow
...
$$\int_{B_{(q,t)}'}\beta_{(q,t)}\triangle \phi dvol(g(t))\le \int_{(B_{(q,t)}')^*}\phi
\triangle \beta_{(q,t)} dvol(g(t)).$$
...
have
$$\int_{B_{(q,t)}'}\beta_{(q,t)}\triangle \phi dvol(g(t))=lim_{k\rightarrow
\infty}\int_{B_{(q,t)}'}f_k\triangle \phi  dvol(g(t)).$$ Since
...
$$\int_{\nu_\epsilon(S_{(q,t)})\cap B_{(q,t)}'}\phi\triangle f_k  dvol(g(t))\le
...
$$
\begin{aligned}
\int_{B_{(q,t)}'}f_k\triangle \phi dvol(g(t)) & = &
\int_{B_{(q,t)}'}\phi\triangle f_k  dvol(g(t)) \\
...
$\beta_{(q,t)}$ yields
$$\int_{B_{(q,t)}'}\beta_{(q,t)}\triangle \phi dvol(g(t))
\le \int_{B_{(q,t)}'\setminus \left(B_{(q,t)}'\cap
...
Let $\phi\colon B_{(q,t)}'\to \Ar$ be a non-negative, smooth
function with compact support. Then
$$\int_{B_{(q,t)}'}l^\tau\triangle \phi dvol(g(t))\le \int_{(B_{(q,t)}')^*}\phi
\triangle l^\tau dvol(g(t)).$$

**Proof.** Recall that $\beta_{(q,t)}=L^\tau+\psi_{(q,t)}$ and that
$\psi_{(q,t)}$ is a $C^\infty$-function. Hence,
$$\int_{B_{(q,t)}'}\psi_{(q,t)}\triangle \phi dvol(g(t))= \int_{(B_{(q,t)}')^*}\phi
\triangle \psi_{(q,t)} dvol(g(t)).$$ Subtracting this equality
from the inequality in the previous claim and dividing by
$2\sqrt{\tau}$ gives the result.

Now we turn to the proof proper of Theorem~.
...
$|\nabla l_x^\tau|^2$ to $B_{(q,t)}'$ is an $L_loc^\infty$-function. Similarly, $\partial l_x/\partial \tau$ is an
$L_loc^\infty$-function. Hence
...
l_x^\tau|^2-R+\frac{n}{2\tau}\right) dvol(g(t))} \\ & = &
\int_{(B_{(q,t)}')^*}\phi\left(\frac{\partial l_x}{\partial
\tau}+|\nabla l_x^\tau|^2-R+\frac{n}{2\tau}\right) dvol(g(t)).
\end{aligned}
$$
...
Corollary~ we have
$$\int_{B_{(q,t)}'}l_x^\tau\triangle \phi  dvol(g(t))\le \int_{(B_{(q,t)}')^*}\phi\triangle l_x^\tau
dvol(g(t)).$$ Putting these together we see
...
$$ on $M\times\{\tau\}$. According to Corollary~ the following equality
holdes on ${\mathcal U}_x(\tau)$:
$$2\frac{\partial l_x}{\partial \tau}+|\nabla l_x^\tau|^2-R+\frac{l_x^\tau}{\tau}=0.$$
By Proposition~ the open set ${\mathcal U}_x(\tau)$
...
functions, i.e., elements of $L_loc^\infty(M)$, and hence is
...
L}exp(X,\bar\tau)=2\sqrt{\bar\tau}X$. This means that for any
...
$$lim_{\tau\rightarrow 0}\widetilde V_x(M,\tau)=(4\pi)^{\frac{n}{2}}.$$

**Proof.** By Proposition~ ${\mathcal U}_x(\tau)$ is an open
subset of full measure in $M$. Hence, $$\widetilde
V_x(M,\tau)=\int_{{\mathcal U}_x(\tau)}\tau^{-\frac{n}{2}}exp(-l_x(q,\tau))dq.$$
 Take
linear orthonormal coordinates $(z^1,\ldots,z^n)$ on $T_pM$. It
follows from  the previous equality and Lemma~ that
$$\widetilde V_x(M,\tau)=\int_{\widetilde {\mathcal U}_{x}(\tau)}f(Z,\tau)dz^1\cdots dz^n,$$
where $f(Z,\tau)=\tau^{-\frac{n}{2}}e^{-\widetilde
l(Z,\tau)}{\mathcal J}(Z,\tau)$. By Proposition~ for
...
Fix $0<\tau_0<T$. According to Theorem~ (with $A=M\times
...
To show that $lim_{\tau\rightarrow 0}\widetilde
 ...
 the content of Corollary~.

### Converse to Lemma~\protect{}

In Lemma~ we showed that for the trivial flow on flat
...
converse to Lemma~, namely to show that if $(M,g(t)),0\le
...
Lemma~, $\widetilde V_x(M,\tau)$ is constant on the
interval $(0,\bar\tau]$. Hence, it follows from the proof of
Theorem~ that the closure of $\widetilde{\mathcal U}(\tau)$
is all of $T_pM$ for all $\tau\in (0,\bar\tau]$ and that
$f(Z,\tau)=e^{-|Z|^2}2^n$ for all $Z\in T_pM$ and all $\tau\le
\bar\tau$. In particular,
$$\frac{\partial ln(f(Z,\tau))}{\partial \tau}=0.$$
From the proof of
 Proposition~ this means that
Inequality~() is an equality and consequently, so is
Inequality~(). Thus, by Proposition~
...
l_x(\cdot,\tau)\ \ \ \ and \ \ \ \varphi_{\bar\tau}=Id.$$ We now consider
...
+\frac{\bar\tau}{\tau}\varphi_t^*2Ric(g(\tau)) \\
& = & -\frac{\bar\tau}{\tau^2}\varphi_\tau^*g(\tau) +
\frac{\bar\tau}{\tau}\varphi_\tau^*2Hess(l_x^\tau)+\frac{\bar\tau}{\tau}
\varphi_\tau^*\left(\frac{1}{\tau}g(\tau)-2Hess(l_x^\tau)\right)=0.
\end{aligned}
$$
