...
This conjecture was formulated by Henri Poincar\'e
...
prove the Poincar\'e Conjecture (and Theorem~ below
...
$3$-dimensional spherical space-form conjecture is the Poincar\'e Conjecture.
 ...
 As indicated in Remark 1.4 of ,  the arguments we present
...
introduced by Richard Hamilton in :
...
paper , the book by Chow-Knopf
, or the book by Chow, Lu, and Ni .
...
, that under the Ricci flow the manifold shrinks
...
constant positive curvature. (ii) At the other extreme, in
 Hamilton showed that if the Ricci flow exists
...
While Theorem~ is central for all applications of Ricci
flow to the topology of three-dimensional manifolds, the argument
for the $3$-manifolds described in Theorem~ is
...
product of finite groups and infinite cyclic groups\footnote{In
 Perelman states the result for $3$-manifolds
...
Corollary~.}. Let $g_0$ be any Riemannian metric on
...
$(M,g_0)$ as initial metric as described in Theorem~.
...
given by Perelman in , see also
.

We immediately deduce Theorem~ from
Theorems~ and~ as  follows: Let $M$ be a
$3$-manifold satisfying the hypothesis of Theorem~.
...
**Proof.** Repeated application of Theorem~ shows that (1) implies
(2). The implication (2) implies (3) is immediate from van Kampen's
theorem. The fact that (3) implies (1) is Theorem~.
...
on manifolds of positive Ricci curvature in .
...
topological conclusion of Theorem~ for that component,
...
Corollary~ that the manifolds that satisfy the four equivalent
...
Thurston, see ,  known as Thurston's
...
essential way on Theorem~, namely the existence of
...
(), following earlier work of Hamilton
() in a simpler case of bounded curvature. They
also involve a result (Theorem 7.4 from ) from the
theory of manifolds with curvature locally bounded below that are
collapsed, related to results of Shioya-Yamaguchi . The
Shioya-Yamaguchi results in turn rely on an earlier, unpublished
work of Perelman proving the so-called `Stability Theorem.'
Recently, Kapovich,  has put a preprint on the archive
giving a proof of the stability result. We have been examining
another approach, one suggested by Perelman in ,
avoiding the stability theorem, cf,  and
. It is our view that the collapsing results
...
beyond Theorem~ described in the previous paragraph
that are needed for the Geometrization Conjecture. Rather, we
content ourselves with presenting a proof of Theorem~
...
of Chapter 17 we have established Theorem~. Chapter 18
...
Hamilton  introduced the Ricci flow
equation,
$$\frac{\partial g(t)}{\partial t}=-2Ric(g(t)).$$
...
were proved by Hamilton () using the Nash-Moser inverse
function theorem,
 (). These results are standard for
...
general result along these lines is Shi's theorem}, see . Again, this is a standard type of
...
four-dimensional manifold ${\mathcal M}$ (space-time) with a time function $**t**\colon {\mathcal M}\to \Ar$
...
\Ar$ is an interval, in which $**t**$ is the projection onto $J$
...
$**t**$. We call $**t**^{-1}(t)$ the $t$ time-slice. It is a
smooth $3$-manifold.
\item The image $**t**({\mathcal M})$ is a connected interval $I$ in
$\Ar$, possibly infinite. The boundary of ${\mathcal M}$ is the
pre-image under $**t**$ of the boundary of $I$.
\item The level sets $**t**^{-1}(t)$ form a codimension-one foliation of
...
tangent to the level sets of $**t**$. This metric induces a
...
to Hamilton (see Section 4 of ), and derives
many of its consequences,  which are also due to Hamilton (cf.
). This principle and its consequences are at
...
Equation~() proves that the minimum of the scalar
...
$$R_min(t)\ge R_min(0)\left(\frac{1}{1-\frac{2t}{n}R_min(0)}\right),$$ and thus the equation must develop a singularity at or
before time $n/\left(2R_min(0)\right)$.
...
. Here Cheeger obtained topological results. In
 Gromov proposed that geometric limits should exist in
the Lipschitz topology and suggested a result along these lines,
which also was known to Cheeger.
 In
, Greene-Wu gave a rigorous proof of the compactness
...
. Assuming that all the derivatives of curvature are
...
Hamilton in  extended these results to Ricci
...
limits were extended to the non-smooth context by Gromov in
 where he introduced a weaker topology, called the
...
Hamilton in  had introduced the notion of
...
backward time*. See Fig.~. The ${\mathcal
...
L}$-geodesic$-geodesic}, and the reduced length
...
t\}}\bar\tau^{-3/2}e^{-\ell_{(x,t)}(q,\bar\tau)}dvol_{g(\bar\tau}(q),$$ where, as before $\bar\tau=t-\bar t$.
...
soliton*} for the
original $\kappa$-solution, see Fig.~.
...
hypothesis is essential. See Hamilton's work
...
Hamilton's maximum principle result (5) in Chapter~)
...
perform surgery. This we do in Chapter~. We fix a
...
outside this region has positive sectional curvature, see Fig.~. Any such metric will suffice for the gluing
...
flow with surgery. Thus, we shall set $C=C(\epsilon)=max(C_1(\epsilon),C_2(\epsilon))$ and we shall find $(C,\epsilon)$-canonical
...
$4$-dimensional Hausdorff singular space with a time function $**t**$ with the property that each time-slice is a compact, smooth
...
[0,1)$, see Fig.~. There is a natural
...
space-time  with the property that the restriction of $G$, $**t**$
...
Fix $\epsilon>0$ sufficiently small and let $C=max(C_1,C_2)<\infty$, where $C_1$ is the constant associated to
...
time $T$ evolve under the Ricci flow. See Fig.~.
...
This completes our outline of the proof of Theorem~.
...
find a connected open subset ${\mathcal X}$ of $**t**^{-1}([T_0,\infty))$ with the property that for each $t\ge  T_0$
...
necessary for the topological application (Theorem~),
or indeed, for any other topological application. The reason is the
sphere theorem (see ), which says that if $\pi_2(M)$ is
...
approaches to this second step: the first is due to Perelman in
 and the other due to Colding-Minicozzi in
. In their approach Colding-Minicozzi
associate to a non-trivial element in $\pi_3(M)$ a non-trivial
element in $\pi_1(Maps(S^2,M))$. This element is represented
...
, they show that this function satisfies the same forward
...
in Theorem~ the entire manifold must disappear at
finite time.
...
curve-shrinking flow (see ). Unfortunately, this flow can
...
the Ricci flow does not. Following Perelman, or indeed ,
...
Perelman's arguments . We have referred to those
...
Xiao-Dong Wang, see , wrote notes for the seminars at MIT,
...
First and foremost are Perelman's three preprints, ,
...
Kleiner and John Lott, , which greatly expand and
clarify Perelman's arguments from the first two preprints. There is
also a note on Perelman's second paper by Yu Ding  .
There is the article by Colding-Minicozzi ,
...
. Lastly, after we had submitted a preliminary version of
