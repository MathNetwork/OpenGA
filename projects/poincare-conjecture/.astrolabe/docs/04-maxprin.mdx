...
$n$-dimensional manifold. Denote by $R_min(t)$ the minimum
value of the scalar curvature of $(M,g(t))$. Then:
\begin{itemize} \item $R_min(t)$ is a non-decreasing function
of $t$.
\item If $R_min(0)\ge 0$, then
$$R_min(t)\ge R_min(0)\left(\frac{1}{1-\frac{2t}{n}R_min(0)}\right),$$ in particular,
$$T\le\frac{n}{2R_min(0)}.$$
\item If $R_min(0)<0$, then
$$R_min(t)\ge
-\frac{n\bigl| R_min(0)\bigr|}{2t\bigl|R_min(0)\bigr|+n}.$$
\end{itemize}

**Proof.** According to Equation~(), the evolution equation for $R$
is
$$\frac{\partial}{\partial t}R(x,t)=\Delta
R(x,t)+2|Ric(x,t)|^2.$$ Since $M$ is compact, the function
$R_min(t)$ is continuous but may not be $C^1$ at points where
the minimum of the scalar curvature is achieved at more than one
point.

The first thing to notice is the following:

**Claim.**   If $R(x,t)=R_min(t)$ then $(\partial R/\partial t)(x,t)\ge \frac{2}{n}R^2(x,t)$.

\begin{proof}
This is immediate from  the evolution equation for $R$, the fact
that if $R(x,t)=R_min(t)$, then $\Delta R(x,t)\ge 0$, and the
...
**Claim.**
$$\frac{d}{d t}(R_min(t))\ge \frac{2}{n}R_min^2(t),$$
where, at times $t$ where $R_min(t)$ is not smooth, this
...
If follows immediately from Claim~ and
Lemma~ that $R_min(t)$ is a non-decreasing
function of $t$. This establishes the first item and also the second
item in the case when $R_min(0)=0$.

Suppose that $R_min(0)\not= 0$.
 Consider the function
$$S(t)=\frac{-1}{R_min(t)}-\frac{2t}{n}+\frac{1}{R_min(0)}.$$
 Clearly, $S(0)=0$ and $S'(t)\ge 0$ (in the sense of forward difference quotients),
  so that by  Lemma~ we have $S(t)\ge 0$ for all $t$. This means that

$$
 \frac{1}{R_min(t)}\le
\frac{1}{R_min(0)}-\frac{2t}{n}
$$
 provided that
$R_min$ is not ever zero on the interval $[0,t]$. If $R_min(0)>0$, then by the first item, $R_min(t)>0$ for all $t$
for which the flow is defined, and the inequality in the second item
of the proposition is immediate from Equation~(). The
third inequality in the proposition also follows easily from
Equation~() when $R_min(t)<0$. But if $R_min(t)\ge 0$, then the third item is obvious.
\end{proof}
...
\partial H$ and $Z\subset H$. For $z\in int\, Z$ we define
...
the related Theorem 4.2 and its proof) in  .
...
in the proof of Theorem 4.3 in .
...
$|Rm(x,t)|\le 2$ for all $x\in M$ and all $t\in [0,min(T,2^{-4})]$.
Furthermore, for any $t\in [0,min(T,2^{-4})]$ and any $x\in M$ and any
$r\le 1$ we have $Vol\,B(x,t,r)\ge \kappa_0r^n$.

**Proof.** The bound on the Riemann curvature follows directly from Lemma 6.1
on page 207 of  and the definition of normalized
...
in Equation~(), we see that there is $A<\infty$ such that
...
reference see  Theorem 6.3 on page 208.
...
evaluating the expressions in Equation~() in the
...
respect to the evolving frame as in Proposition~. Then
the evolution equation given in Proposition~ is:
...
parallel translation. The expressions in Lemma~ show that
...
{\mathcal Z}$ for all $x\in M$. Applying Theorem~ proves the
result.
...
**Proof.** The statement that $Ric(x,t)\ge 0$ is equivalent to the statement that
...
Lemma~ the derivative of $s_x$ at ${\mathcal T}$ in the
...
preserves ${\mathcal S}$. Applying Theorem~ gives the result.
...
$(x,t)\in int(\overline U)\times(0,T]$.
...
U$ and all $t\in [0,T]$. Suppose also that there is $x_0\in int(\overline U)$ with $s({\mathcal T}(x_0,0))>0$. Then
$s({\mathcal T}(x,t))> 0$ for all $(x,t)\in int(\overline
U)\times(0,T]$.
...
the local version of the maximum principle (Theorem~),
...
for all $y\in int(V)$. Also, from Equation~() we
have
...
**Proof.** First notice that it follows from Theorem~ that
...
proof of Corollary~ it is invariant under the vector
...
Then by Theorem~ $s(y,T)>0$ for all $y\in int(V)$
...
time translation; cf. Lemma 8.2 in . Fix a point $x\in M$,
...
Equation~(), under Ricci flow on the $3$-manifold, the time
...
only a non-orientable line field.
 ...
 Let $(N,g)$ be a Riemannian manifold. Recall from Definition~
  that the open cone
...
**Proof.** If $(U,g(T))$ is flat, then by Theorem~
...
Proposition~,  for each $x\in U$ the Riemann curvature tensor of
...
computations in Proposition~ the non-zero eigenvalue is not
...
the line field along the cone lines. Corollary~ says that since
...
) that compact $3$-manifolds of non-negative
...
$$lim_{t\rightarrow T}\frac{max_{x\in M}\lambda(x,t)}{min_{x\in M}\nu(x,t)}=1.$$ Furthermore, for any $x\in M$ the
...
Hamilton's proof in  uses the maximum principle
...
follows from Theorem~ that $T<\infty$ and that as
...
## Pinching toward positive
curvature

As the last application of the maximum principle for tensors we give
a theorem due to R. Hamilton (Theorem 4.1 in )
and T. Ivey  which shows that, in dimension three, as the
...
**Proof.** First note that by Proposition~, if $R_min(0)\ge 0$, then the same is true for $R_min(t)$ for every
$t>0$ and thus the first inequality stated in the theorem is clearly
true. If $R_min(0)<0$, the first inequality stated in the
...
V}$. There are two solutions to $x(log(x)+(log(1+t)-3)=-3/(1+t)$.
One is $x=1/(1+t)$; let $\xi(t)>1/(1+t)$ be the other. We set $S({\mathcal
T})=tr({\mathcal T})$, so that $R=2S$, and we set $X({\mathcal T})=max(-\nu({\mathcal T}),0)$. Define
...
where $f_t(x)=x(logx+log(1+t)-3)$. Then we define
...
\begin{proof}
First consider the function $f_t(x)=x(log(x)+log(1+t)-3)$ on the interval $[\xi(t),\infty)$. Direct computation
...
$X'\le X$. (See Fig.~). By definition an element
...
X(logX-3)$. This completes the proof that ${\mathcal
...
The above claim and Equation~() immediately imply that
...
$$
 ...
 Set $W=\frac{S}{X}-log\,X$, then rewriting
 Equation~()
 in terms of $W$ gives
...
Equation~() implies that $S(t)\ge -3/(1+t)$ for all $t$.
...
close to $t_1$. Hence, by Equation~() we have
...
$S(t_1)=f_{t_1}(X(t_1))$, we have $W(t_1)=(log(1+t_1)-3)$. It
follows immediately that $W(t)\ge (log(1+t)-3)$ for all
...
By Theorem~, the previous three claims imply that ${\mathcal
...
$\lambda(x,t)\ge \mu(x,t)\ge \nu(x,t)$ and set $X(x,t)=max(-\nu(x,t),0)$. Assume that for every $x\in M$ we have
$$R(x,a) \ge  \frac{-6}{4a+1}$$
and $$ R(x,a) \geq 2X(x,a)\left(logX(x,a)+log(1+a)-3\right),$$ where the second inequality holds whenever
...
2X(x,t)\left(logX(x,t)+log(1+t)-3\right),
\end{aligned}
$$
...
$X(x,t)>0$, then by Theorem~ it is bounded by $R_0$.
...
\begin{enumerate}
\item $$R(x)\ge \frac{-6}{4**t**(x)+1}$$
\item $$R(x)\geq 2X(x)\left(logX(x)+log(1+**t**(x))-3\right),
$$ whenever
$0<X(x)$
\end{enumerate}
where, as in the statement of Theorem~, $X(x)$ is the
maximum of zero and the negative of the smallest eigenvalue of $Rm(x)$.

The content of Theorem~ is that if $(M,g(t)),\ 0\le a\le
...
we establish a strengthening of Theorem~.
...
[0,T)$ there is a constant $c(t)$ with  $lim_{t\rightarrow
T}c(t)=0$ and with the property that there is an isometry from
$(M,g(t))$ to $(M,c(t)g)$. Then $(M,g)$ is round.

**Proof.** By rescaling we can assume that for all $x\in M$ all the eigenvalues of $Rm(x,0)$ have absolute value $\le 1$.  This implies that $(M,g(0))$ satisfies
the hypothesis of Theorem~. Our first goal is to show that $Rm(x,0)\ge 0$ for all $x\in M$. Suppose that this is not true; then there is a
...
Theorem~. Now, according to Theorem~ either all
...
Theorem~ this means that as the singularity
...
\chi(x,t),\nabla R(x,t)\rangle +2Ric(x,t)(\chi(x,t),\chi(x,t))\ge 0.$$ In particular, we have
...
two points of $M$.  Then $$log\left(\frac{R(x_2,t_2)}{R(x_1,t_1)}\right)\ge
...
result is
$$R^{-1}(\partial R/\partial t)-|\nabla (log R)|^2+\frac{Ric(\nabla(log R),\nabla(log R))}{2R}\ge 0.$$ Since $Ric(A,A)/R\le |A|^2$, it follows
that
$$\frac{\partial}{\partial t}(log R)-\frac{|\nabla(log R)|^2}{2}\ge 0.$$
...
$$
\begin{aligned}
log\left(\frac{R(x_2,t_2)}{R(x_1,t_1)}\right) & = & \int_{t_1}^{t_2}
 \frac{d}{dt}log(R(\mu(t))dt \\
 & = & \int_{t_1}^{t_2}\frac{\frac{\partial R}{\partial t}(\mu(t))}{R(\mu(t))}+\langle\nabla(log R)(\mu(t)),\frac{d\mu}{dt}(\mu(t))dt \\
 & \ge & \int_{t_1}^{t_2}\frac{1}{2}|\nabla (log R)(\mu(t))|^2-|\nabla (log R)(\mu(t))|\cdot \left|\frac{d\gamma}{dt}\right|dt \\
...
$$log\left(\frac{R(x_2,t_2)}{R(x_1,t_1)}\right)\ge
