...
$Rm(x)$ on $\wedge^2T_xM_{**t**(x)}$, as measured with
...
\item[(2)] $$R(x)\geq 2X(x)\left(logX(x)+log(1+**t**(x))-3\right),
$$ whenever
$0<X(x)$.
\end{enumerate}

According to Theorem~ if $(M,g(t)),\ 0\le a\le t<T$, is
...
$2\epsilon$ and $\alpha=10^{-2}$, and Proposition~
holds for $2\epsilon$.
...
suppose that $x\in {\mathcal M}$. Set $t=**t**(x)$. We suppose
...
$R(y)\ge 4R(x)$  and $**t**(y)\le t$ has a strong
...
contradicts Hamilton's result (Theorem~) which says
...
We shift the time parameter of $({\mathcal M}_n,G_n)$ by $-**t**(x_n)$. We change notation and denote these shifted flows by
$({\mathcal M}_n,G_n)$. This allows us to arrange that $**t**(x_n)=0$ for all $n$. Since shifting leaves the curvature
...
Proposition~, for all $n$ sufficiently large
...
Lemma~ that any sequence of minimizing paths from
...
immediately from Lemma~ that there is a minimizing
...
half. Thus, by Lemma~ any minimizing sequence of
...
either $z_n$ or $y_n$. By Corollary~, since $Y$
...
follows from Proposition~ that there is a balanced
...
described in Proposition~ the new necks we add can
...
containing $y'_n$. The union of this chain is $T_n$. (See Fig.~.)
...
Corollary~ $S$ is isotopic to the $2$-spheres of the
...
Proposition~ each is a homotopically non-trivial
...
Proposition~, $S_x$ is isotopic to the
...
Proposition~ and our assumption that
...
suffices to consider points $w_n$ in $Reg_\delta(U_n,g'_n)$
...
Lemma~ implies that the forward difference
...
Lemma~ again both to the distance from $w_n$ and
...
sufficiently large, $Reg_\delta(U_n,g'_n)\subset
B(z_n',A_1-\delta/4)$, and consequently that the curvature on $Reg_\delta(U_n,g'_n)$ is bounded independent of $n$.

By Shi's theorem (Theorem~), the fact that each point of
...
0$ there is a uniform bound for $|\nabla^\ell Rm|$ on $Reg_\delta(U_n,g'_n)$. Clearly, since the base point $z_n'$ has
bounded curvature it lies in $Reg_\delta(U_n,g'_n)$ for
...
Theorem~ we see that after passing to a subsequence
...
$U_\infty$ under the diffeomorphism $\psi$ (see Fig.~).
...
properties stated in Definition~ so as to exhibit
...
neck has closure contained in $Reg_\delta(U_n,g_n)$ for some
...
follows from Proposition~ that for all $m$
...
$n$ sufficiently large $R(y_n')>>R(q_n))$, and hence
 ...
 Corollary~, we see that  the distance from
 ...
 compact closure contained in $Reg_\delta(U_n,g_n)$ for some $\delta$
 ...
 $g_\infty|_{V_m}$ as $n$ tends to infinity, we see, by Proposition~
 that for all $n$
 sufficiently large $\varphi_n^{-1}(N_m)$ contains a $2\epsilon$-neck in $U_\infty$ centered
 at $q$.

It now follows from Proposition~ that $W^+_\infty$
...
The next step in the proof of Proposition~ is to extend
...
Proposition~ by passing to a subsequence we
...
given in Equation~ and the fact that $Q_n\rightarrow
\infty$ as $n\rightarrow \infty$. (See Theorem~.)

This completes the proof that all the properties claimed in
Proposition~ hold for the geometric limit
$(U_\infty,g_\infty,z_\infty)$. This  completes the proof of that
proposition. \end{proof}

## Cone limits near the end ${\mathcal E
$ for rescalings of $U_\infty$}

The next step is to study the nature  of the limit $U_\infty$ given
in Proposition~. We shall show that an appropriate
...
Proposition~. Let $Q_\infty=R_{g_\infty}(z_\infty)$ and
...
that by Lemma~ any minimizing sequence of arcs from $x$
...
distance (see Section~) from $x$ to ${\mathcal E}$.
...
Lemma~ for all $i$ sufficiently large, the extrinsic
...
### The Metric on the space of directions at ${\mathcal
E
$}
...
property (Theorem~) for manifolds with
...
**Proof.** By Corollary~, after replacing $\gamma, \mu,\nu$ by
...
On the other hand by Definition~ and the law of
...
Now we are ready to prove Proposition~. In fact, we
...
hypothesis of Proposition~. Fix a minimizing geodesic
...
According to Corollary~ there is a $\delta$-net
...
Because of the convergence result on angles (Lemma~),
for all $i$ and $j$ we have
$$

$$
\begin{aligned}
lim_{n\rightarrow\infty}d_{\lambda_ng_\infty}\left(\varphi_n(\ell_i,s_i),\varphi_n(\ell_j,s_j)\right)
 & = & s_i^2+s_j^2-2s_is_jcos(\theta(\gamma_i,\gamma_j)) \\ & = & d_{g_{\mathcal
...
## Comparison of the Gromov-Hausdorff limit and the smooth
limit
...
bounded. Hence, by Theorem~ we can pass to a
subsequence and extract a geometric limit. In fact, by
Proposition~ there is even a geometric
...
by Proposition~ we see that the distance
...
Lemma~ with respect to this metric there is a sphere
...
Gromov-Hausdorff limiting cone constructed is Section~
...
contradicts Proposition~,  one of the consequences of
...
was that Theorem~ did not hold for some $A_0<\infty$.
Thus, we have established Theorem~  by contradiction.
