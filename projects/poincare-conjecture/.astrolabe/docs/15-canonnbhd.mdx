...
in the neck structure on $N$ at most once; see Fig.~.

**Proof.** We parameterize $\gamma$ as a map from $[a,b]\to M$. By
Corollary~, provided that $\epsilon>0$ is sufficiently
...
$\epsilon$ is sufficiently small, by Lemma~ we can
...
the two-sphere. In particular, by Corollary~ the length
...
by Corollary~, replacing $\gamma|_{[a',b']}$ with a
...
hence by Corollary~ there is a subinterval $[d,b']$
...
diffeomorphism mapping $S(y)$ to $S^2\times \{1/2\}$, see Fig.~.
\end{enumerate}

**Proof.** Fix $0<\epsilon_5(\alpha)\lemin(\epsilon_1(\alpha_1),\epsilon_2(\alpha/3),\epsilon_3(\alpha),\alpha/3)$
sufficiently small so that Corollary~ holds. The
first two items are then immediate from Lemma~. The
third statement is immediate from Lemma~, and the
fourth statement from Corollary~. Let us consider
...
## Regions covered by $\epsilon$-necks and
$(C,\epsilon)$-caps
...
Proposition~, $U$ is the union of two manifolds
...
Corollary~ there is a positive lower bound, independent
...
to $x$ .This is impossible since $x_i$ converges to $x$.
 ...
 In fact, there is a geometric version of Lemma~.
...
**Proof.** Since according to Lemma~ the two-spheres for $N_j$
...
**Proof.** By Corollary~,
Inequality~() holds for $d(x,x')$. Once we have this
...
satisfies Inequality~() so that Lemma~
...
**Proof.** According to Lemma~ for $\epsilon>0$ sufficiently
...
Lemma~ the frontier of this union is the frontier
...
$\epsilon$-necks containing $X$. By Lemma~ the union
...
## Subsets of the union of cores of $(C,\epsilon)$-caps
and $\epsilon$-necks.
...
$h_i'$ the scale of $N'_i$. By Lemma~ the ratio
...
(5) of Definition~), it follows from
Corollary~ that this width is at least
...
Proposition~ the $s$-directions in $N_{i-1}$ and
...
$(0.94)\epsilon^{-1}<s_{N_{i-1}}(x')<(0.96)\epsilon^{-1}$.
 ...
 through this point. According to Proposition~,
...
$(0.99)\epsilon^{-1}h'_i$,  It follows from Corollary~
...
to its positive end. According to Corollary~ this
...
$C$-capped infinite $\epsilon$-tube. By Lemma~ this
...
Lemma~ the union $N_0\cup N_1\cup\cdots\cup N_k$ is
...
Now we return to the proof of Claim~. We consider
three cases.

**First Subcase: $S'\subset \widetilde C$.** In this case
we apply Claim~ to see that $\widetilde C\cup C'$ is a
component of $M$ containing $X$.
...
Proposition~,  the neck $N'\subset C'$ meets
...
Claim~ we see that $\widetilde C\cup C'$ is a component
of $M$ containing $X$.
...
N'\not=\emptyset$, by Lemma~ the scales of $N_k$ and
...
separate $M$, it follows from Proposition~ that $X$
...
Proposition~ the intersection of $N_a\cap N_b$
...
\item[(2)] $M$ is diffeomorphic to  $\Ar^3$ or $\Ar P^3\setminus \{point\}$,
