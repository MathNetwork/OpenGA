# A comparison geometry approach to the Ricci
flow

In this section we discuss Perelman's notions, introduced in
, of the ${\mathcal L}$-length$-length} in the context of generalized Ricci flows. This is a
...
L}$-geodesics$-geodesic}. Using ${\mathcal
L}$-geodesics we define the ${\mathcal L}$-exponential
mapping$-exponential map}. We derive the
${\mathcal L}$-Jacobi equation$-Jacobi equation}
and relate ${\mathcal L}$-Jacobi fields$-Jacobi
field} to the differential of the ${\mathcal L}$-exponential
...
Section~ we use the theory of ${\mathcal L}$-geodesics and the
...
Section~ to $\kappa$-solutions (ancient, $\kappa$-non-collapsed
...
## ${\mathcal L
$-length and ${\mathcal L}$-geodesics}
...
{\mathcal HT}{\mathcal M}$. We define the $\mathcal{L}$-*length*$-length|ii} of $\gamma$ to be:
$$\mathcal{L}(\gamma) = \int_{\tau_{1}}^{\tau_{2}}
\sqrt{\tau}\left(R(\gamma(\tau)) +
\abs{X_\gamma(\tau)}^{2}\right)d\tau,$|ii}$$
where the norm of $X_\gamma(\tau)$ is measured using the metric
$G_{T-\tau}$ on ${\mathcal HT}{\mathcal M}$. When $\gamma$ is clear
from the context, we write $X$ for $X_\gamma$; see Fig.~ from the Introduction.
...
**Lemma.**   The
Euler-Lagrange equation$-length!Euler Lagrange
equation} for critical paths for the $\mathcal{L}$-length is

$$

 \nabla_{X}X - \frac{1}{2}\nabla R
+ \frac{1}{2\tau}X + 2Ric(X,\cdot)^* = 0.
$$

**Remark.** $Ric(X,\cdot)$ is a horizontal one-form along $\gamma$ and its
...
On the other hand, since $\partial g/\partial \tau =2Ric$ and
...
- \frac{1}{\tau}X - 2\nabla_{X}X - 4Ric(X,\cdot)^*\bigr)\rangle d\tau.
\end{aligned}
$$
...
Equation~() over these sub-intervals shows that the
...
$Y(\tau_1)=Y(\tau_2)=0$. Equation~() holds for all
...
point of the ${\mathcal L}$-length is called an $\mathcal{L}$-*geodesic*$-geodesic|ii}.
Equation~() is the {\em ${\mathcal L}$-geodesic
equation}$-geodesic equation|ii}.
...
**Proof.** Since the ODE in Equation~()  is non-singular even at
...
**Definition.** An ${\mathcal L}$-geodesic is said to be *minimizing*$-geodesic!minimizing} if there is no
...
**Lemma.** $Y(\tau)$ satisfies the {\em ${\mathcal L}$-Jacobi
equation}$-Jacobi equation|ii}:
...
-\frac{1}{2\tau}\widetilde X - 2Ric(\widetilde X,\cdot)^*.$$
...
Equation~() and exchanging orders of differentiation.
The result Equation~() is a second-order differential
...
called an {\em ${\mathcal L}$-Jacobi field}$-Jacobi field|ii} if it satisfies the ${\mathcal L}$-Jacobi
equation, Equation~(), and if it vanishes at $\tau_1$.
...
On the other hand, from Equation~() we have
$\nabla_X(Y)=\frac{1}{2\sqrt{\tau}}\nabla_A(Y)$, so that
$$\sqrt{\tau}\nabla_X(Y)=\frac{1}{2}\nabla_A(Y).$$

### Second order variation of ${\mathcal
L
...
& & -\int_{\tau_1}^{\tau_2}2\sqrt{\tau}\langle Jac(Y_1),Y_2\rangle d\tau.
\end{aligned}
$$
...
geodesics, i.e., the left-hand side of Equation~().
...
L}$-geodesic. It follows from Lemma~ and the remark
...
\tau$, respectively. Applying Lemma~ and using the fact that
...
Now we are in a position to establish Proposition~.

**Proof.** (Of Proposition~)
From the equation in Lemma~, the equality of the second variation
...
Jac(Y_1),(Y_2)d\tau,$$ which is symmetric by Corollary~, is
positive semi-definite. Since $B(Y,Y)=0$, it follows that $B(Y,\cdot)=0$; that
is to say, $Jac(Y)=0$.

## The ${\mathcal L
$-exponential map and its first-order
...
For Section~ we fix $\tau_1\ge 0$ and a point $x\in
{\mathcal M}$ with $**t**(x)=T-\tau_1$. We suppose that $T-\tau_1$
...
**Definition.**
We define *the domain of definition of* ${\mathcal L}exp_x$exp$|ii},  denoted ${\mathcal D}_x$, to be the
...
interval whose closure contains $\tau_1$. Furthermore, ${\mathcal L}exp_x\colon {\mathcal D}_x\to {\mathcal M}$ is a smooth map, and $\widetilde
...
### Positivity of the second variation at a minimizing
${\mathcal L
$-geodesic}
...
**Proof.** According to Proposition~ the second variation  in the
...
diffeomorphism, by Lemma~ there are no non-zero ${\mathcal
...
**Lemma.**
For any $x$ in ${\mathcal M}$ with $**t**(x)=T-\tau_1$ and any $Z\in T_xM_{t-\tau_1}$, there is $\delta>0$
...
suffices to prove that there is $\delta>0$ such that $d_Z{\mathcal L}exp_x^{\tau}$ is an isomorphism for all $\tau_1<\tau<\tau_1+\delta$. By
Lemma~ it is enough to find a $\delta>0$ such that any ${\mathcal
L}$-Jacobi field $Y$ along
 ...
 $A=d\gamma_Z/ds$ we have $\nabla_AY=2lim_{\tau\rightarrow
 0}\sqrt{\tau}\nabla_XY$. In these variables, the argument for $\tau_1=0$
 is the same as the one above for $\tau_1>0$.

**Remark.**
When $\tau_1>0$ it is possible to consider the ${\mathcal L}exp^{\tau}_x$ defined for $0<\tau<\tau_1$. In this case, the curves
are moving backward in $\tau$ and hence are moving forward with
respect to the time parameter $**t**$. Two comments are in order.
...
Lemma~ is true for $\tau<\tau_1$ with $\tau$
sufficiently close to $\tau_1$.

## Minimizing ${\mathcal L
$-geodesics and the injectivity
domain}

Now we discuss the analogue of the interior of the cut locus for the
usual exponential map of a Riemannian manifold. For
Section~ we keep the assumption that $x\in
{\mathcal M}$ with $**t**(x)=T-\tau_1$ for some $\tau_1\ge 0$.

**Definition.**
 The *injectivity set*
$\widetilde{\mathcal U}_x\subset {\mathcal D}_x\subset
\left(T_xM_{T-\tau_1}\times (\tau_1,\infty)\right)$_x$|ii} is the subset of all $(Z,\tau)\in {\mathcal D}_x$ with the following
properties:
\begin{enumerate}
...
U}_x(\tau)\subset T_xM_{T-\tau_1}$_x(\tau)$|ii} equal to the slice of $\widetilde{\mathcal U}_x$ at
...
**Definition.**
We set ${\mathcal U}_x\subset {\mathcal M}$_x$|ii} equal to $ {\mathcal L}exp_x(\widetilde{\mathcal
...
$(Z,\tau)\in \widetilde{\mathcal U}_x$ with ${\mathcal L}exp_x(Z,\tau)=q$, the ${\mathcal L}$-geodesic
...
**Proof.** We consider the differential of ${\mathcal L}exp_x$ at any
...
L}$-minimizing, we see that $L_x\circ {\mathcal L}exp_x|_{\widetilde{\mathcal U}_x}=\widetilde L|_{\widetilde
...
connecting $x$  to $q$. (See Fig.~ in the
Introduction.)
...
### Monotonicity of the $\widetilde{\mathcal U
_x(\tau)$ with respect to
$\tau$}_x(\tau)$!monotonicty}
...
$d_Z{\mathcal L}exp_x^\tau$ is not an isomorphism.
According to Lemma~, $\tau_1<\tau'_2\le \bar\tau$. Since
${\mathcal L}exp_x^{\tau'_2}$ is not a local diffeomorphism at
$(Z,\tau'_2)$, by Lemma~  there is a non-zero
${\mathcal L}$-Jacobi field $Y$ along $\gamma_Z|_{[\tau_1,\tau'_2]}$
vanishing at both ends. Since $\gamma_Z|_{[\tau_1,\tau_2']}$ is
${\mathcal L}$-minimizing, according to Proposition~,
...
vanishes. But according to Proposition~ this means that
...
**Proof.** It suffices to show that the differential of ${\mathcal L}exp_{q_2}^\tau$ is an isomorphism for all $\tau\in
...
Proposition~ it must be the case that $\widehat Y$ is a
...
**Lemma.**
 Suppose that
$q\in  {\mathcal U}_x$ with $**t**(q)=T-\bar\tau$ for some $\bar\tau>\tau_1$.
...
L_x,X(\tau)\rangle.$$ Now using Corollary~, and rearranging
the terms gives the result.

## Second-order differential inequalities for $\widetilde L^{\bar\tau
$
and $L_x^{\bar\tau}$}

Throughout Section~ we fix $x\in {\mathcal M}$ with $x\in
M_{T-\tau_1}$.
...
Lemma~ in the special case when $u_1=u_2$.
...
$$2Ric_{ij}\bigl(X^k\partial_k
Y^i+\Gamma_{kl}^iX^kY^l\bigr)Y^j=2Ric(\nabla_XY,Y\rangle.$$
...
\begin{aligned}
g_{ij}\frac{\partial
\Gamma_{kl}^i}{\partial \tau} & = & -2Ric_{ij}\Gamma_{kl}^i+g_{ij}g^{si}(\partial_kRic_{sl}+\partial_lRic_{sk}
-\partial_sRic_{kl})\\
& = & -2Ric_{ij}\Gamma_{kl}^i+
\partial_kRic_{jl}+\partial_lRic_{jk} -\partial_jRic_{kl}.
\end{aligned}
$$
...
\begin{aligned}
g_{ij}\frac{\partial\Gamma^{i}_{kl}}{\partial \tau}X^{k}Y^{l}Y^{j}
 & = & \bigl(-2Ric_{ij}\Gamma^i_{kl}+\partial_kRic_{jl})\bigr)X^kY^lY^j \\
& = & (\nabla_XRic)(Y,Y)
\end{aligned}
$$
...
Equation~ for $Jac(Y)$ and this makes the
integrand
...
$$
\begin{aligned}
\sqrt{\tau}Hess(R)(Y,Y)+2\sqrt{\tau}\langle{\mathcal R}(Y,X)Y,X\rangle
+2\sqrt{\tau}|\nabla_XY|^2 \\
...
of this section is Proposition~ below. In the end we
...
\\& &-4(\nabla_{X}Ric)(Y,Y) +4(\nabla_{Y}Ric)(Y,X)
\\& & - 2\frac{\partialRic}{\partial\tau}(Y,Y) + 2\abs{Ric(Y,\cdot)}^{2} -
\frac{1}{\tau}Ric(Y,Y),\nonumber.
\end{aligned}
$$
...
$$
 ...
 We
have equality in Equation~() if and only if the adapted vector
...
Now we are ready to begin the proof of Proposition~.
...
Proposition~, and using the fact that
$Y(\tau_1)=0$, we have
...
$$
 ...
 Plugging in Equation~(), and using the fact that
 $|Y(\tau)|^2=
...
$$
 ...
 Using the definition of $H(X,Y)$ given in the statement, Equation~(),
 allows us to write
...
-2\frac{\partialRic}{\partial\tau}(Y,Y) +4|Ric(Y,\cdot)|^2 \bigr) \Bigr.\\
& & \Bigl.+
\frac{|Y(\bar\tau)|^2}{2\sqrt{\tau}(\sqrt{\bar\tau}-\sqrt{\tau_1})^2}
-\left(\frac{2}{(\sqrt{\tau}-
...
$$
 ...
 To simplify
further, we compute, using Equation~()

$$

$$
\begin{aligned}
\frac{d}{d\tau}\bigl(Ric(Y(\tau),Y(\tau))\bigr) &= & \frac{\partialRic}{\partial{\tau}}(Y,Y) +
        2Ric(\nabla_{X}Y,Y) + (\nabla_{X}Ric)(Y,Y)\\
        & = & \frac{\partialRic}{\partial{\tau}}(Y,Y)+ (\nabla_{X}Ric)(Y,Y) \\ & & +
...
= & 2\sqrt{\tau}\left(\frac{\partialRic}{\partial{\tau}}(Y,Y)
...
This proves Inequality~(). Now we examine when equality
...
Y'(\bar\tau,0)=Y(\bar\tau)$. Since $L_x\circ {\mathcal L}exp_x=\widetilde L$, we see that
$$\frac{d^2}{d v^2}{\mathcal
L}(\mu_v)|_{v=0}=\frac{d^2}{du^2}L^{\bar \tau}_x(\mu(u))|_{u=0}.$$
Hence, the assumption that we have equality in~()
implies that
$$\frac{d^2}{d v^2}{\mathcal
...
$\gamma_{0,0}=\gamma_Z$. By Remark~ the second
...
**Claim.** $$\frac{\partial}{\partial v}\frac{\partial}{\partial w}{\mathcal L}(\mu)|_{u=v=0}=
...
If Inequality~() is an equality, then
...
Proposition~, that $\widetilde W(\tau,0,0)=0$ for all
...
This shows that we have equality in~() if and only if
the adapted vector field $Y(\tau)$ is also an ${\mathcal L}$-Jacobi
field.
\end{proof}
...
\sqrt{\tau}(\sqrt{\tau}-\sqrt{\tau_1})^2H(X)d\tau,$$_{\tau_1}^{\bar\tau}$} with
...
Inequality~() is an equality if and only if for every
...
-Ric(Y_{\alpha},\cdot)^*.$$ As in Equation~(),
we have
...
To establish Inequality~() it remains to prove the
following claim.
...
\begin{proof}
To prove the claim we sum Equation~() giving
...
by~(), recalling that $\partial R/\partial\tau=-\partial R/\partial
t$, we have
$$\frac{\partial R}{\partial \tau} = -\triangle R - 2|Ric|^{2}.$$
On the other hand, $$\frac{\partial R}{\partial\tau} =
\partial(g^{ij}R_{ij})/\partial{\tau}= -2|Ric|^{2} + \sum_{\alpha}\frac{\partialRic}{\partial\tau}(W_{\alpha}, W_{\alpha}),$$ and so
$\sum_{\alpha}\frac{\partialRic}{\partial\tau}(W_{\alpha},W_{\alpha}) = -\triangle R$. Putting
all this together gives
$$I^2(\tau)\sum_{\alpha}H(X,Y_{\alpha})= H(X).$$

Clearly, Inequality~() follows immediately from
Inequality~() and the claim. The last statement of
Proposition~ follows directly from the last statement of
Proposition~ and Lemma~. This completes
...
Throughout Section~ we fix $x\in{\mathcal M}$ with
$**t**(x)=T-\tau_1$. We shall always suppose that $T-\tau_1$ is
greater than the initial time of the generalized Ricci flow.

### The reduced length function $l_x$ on space-time

**Definition.** We define
 the $\mathcal{L}$-*reduced length*$-length!reduced} (from $x$)
$$l_x\colon {\mathcal U}_x\to \Ar$$
by setting $$l_x(q)= \frac{L_x(q)}{2\sqrt{\tau}},$$ where $\tau=T-**t**(q)$.
...
\int_{\tau_1}^{\bar\tau}\tau^{3/2}H(X)d\tau.$$^{\bar\tau}$|ii}
...
which is equivalent to Equation~(). In the case when
...
**Proof.** It follows immediately from Equation~() that
$$\frac{\partial l_x}{\partial \tau}=R-\frac{1}{2}(R+|X|^2)-\frac{l_x}{2\tau}.$$
Using Equation~() this gives the first equality stated
in the lemma.
 It follows immediately from Corollary~ that
$\nabla l_x^\tau=X(\tau)$ and hence $|\nabla
l_x^\tau|^2=|X(\tau)|^2$. From this and Equation~() the
...
Equation~() vanishes in this case.
...
Theorem~ in a way that will be useful later.
...
**Proof.** This is immediate from Lemma~ and Lemma~ (after the
...
{\mathcal M}\cap **t**^{-1}(-\infty,T-\tau_1+\epsilon)$. Suppose that there
...
flow and let $x\in{\mathcal M}$ be a point with $**t**(x)=T-\tau_1$. Let $y\in {\mathcal M}$ be a point with $**t**(y)=t=T-\bar\tau$ where $\tau_1+\epsilon\le \bar\tau\le
...
Before proving Proposition~, let us show how it implies
Proposition~.

**Proof.** (that Proposition~ implies
Proposition~) Suppose given $\epsilon>0$, $A$, $\nu(A)$ and $F$ as in
the statement of Proposition~. For each $y\in A$ there is
...
L}$-geodesic in $F$. Thus, Proposition~, with $\epsilon$ replaced
...
G=G+dt^2$ and $G(**t**(z))+dt^2$ are uniformly comparable on all of
...
Now we turn to the proof of Proposition~. We begin with
...
$$max_{\tau}\left(\sqrt{\tau}|X_\gamma(\tau)|\right)\le C_1min_{\tau}\left(\sqrt{\tau}|X_\gamma(\tau)|\right)+\frac{(C_1-1)}{2}\sqrt{\bar\tau},$$
...
following holds. Let $y\in {\mathcal M}$ be a point with $**t**(y)=t_0=T-\bar\tau$ where $\tau_1+\epsilon\le \bar\tau\le
\bar\tau_0$. Suppose that there is a minimizing ${\mathcal
...
Claim~ this implies that the $g(t_0)$-speed of $\mu$ is at most
...
Lemma~ for the following set of input constants:
...
$C_0$. By Lemma~ there is a curve parameterized by
...
Lemma~ applies to show that for very $w=\iota(c,t)\in
...
with $**t**(x)=T-\tau_1$. Let
 $A\subset {\mathcal
M}\cap **t**^{-1}(-\infty,T-\tau_1)$ be a subset whose intersection with each
...
${\mathcal L}$-geodesic to $z$ then the differential of ${\mathcal L}exp_x^\tau$ at any $Z$ is an isomorphism. The second is the intersection of
...
Corollary~ we see that the initial conditions
...
proof of the claim.
 ...
 According to the next claim, the first subset given in Claim~ is contained in the
...
on p. 81 of . The second set is of measure zero by Sard's
...
ending at $z$ with the property that the differential of ${\mathcal L}exp^{\tau}$ is an isomorphism at both $Z_1$ and $Z_2$. Then the function
...
Lemma~ the resulting function $L_{Z_0}$ is smooth and its gradient
...
$({\mathcal M}',G')$. Recall that $\tau=T-**t**$ is the parameter
for ${\mathcal L}$-geodesics in $({\mathcal M},G)$ The corresponding
parameter in the rescaled flow $({\mathcal M}',G')$ is $\tau'=T-**t'**=Q\tau$. We denote by ${\mathcal L}'exp_x$ the ${\mathcal
...
$${\mathcal
L}'exp_x(\sqrt{Q^{-1}}Z,\tau')=\iota({\mathcal L}exp_x(Z,\tau))$$ and
...
where ${\mathcal J}(Z,\tau)$(Z,\tau)$\ii} is the Jacobian
...
**Proof.** It follows from the first equation in Corollary~ that
...
$\sqrt{Q^{-1}}Z$,  by Lemma~ we have
$\gamma(\tau)=\beta(\tau')$. From Corollary~ and the
...
In particular, the Jacobian determinant of ${\mathcal L}exp_{(x,T)}^\tau$ is constant and equal to $2^n\tau^{n/2}$. The
...
{\mathcal M}_k$$ compatible with the time function $**t**_k$ and the vector
...
ODE given in Equation~() is regular even at $0$, this implies that
...
space. Of course $|Z'|^2=|Z|_G^2$. By Claim~, this
means that we have
...
${\mathcal D}_x^\delta$, in the domain of definition of ${\mathcal L}exp_x^\delta$ as given in Definition~, and ${\mathcal L}exp_x$ is a diffeomorphism on this subset. We shall show that if $\delta>0$ is
...
no greater than that of $\gamma$. According to Lemma~ there is a
...
Lemma~ for $C_1$, it follows that, fixing the bound of
...
pushes forward under the differential at $Z$ of ${\mathcal L}exp_x^\tau$  to an orthonormal basis $\{Y_\alpha\}$ for
$M_{T-\tau}$ at $\gamma_Z(\tau)$. Notice that, letting $\tau'$ range
from $0$ to $\tau$ and taking the push-forward of the
$\partial_\alpha$ under the differential at $Z$ of ${\mathcal L}exp_x^{\tau'}$ produces a basis of ${\mathcal L}$-Jacobi fields
...
By Lemma~ and by Proposition~ (recall that
$\tau_1=0$) we have
...
proof of Proposition~ and Claim~ yields
...
Corollary~ says that the right-hand side of the
...
As we have already seen, this proposition implies
 Theorem~, and hence the proof of this theorem is complete.
\end{proof}
...
Theorem~ we see that $\tau^{-n/2}e^{-\tilde
