...
). As was established in Corollary~, since
...
$g(t)=(1-t)g_{0}$. Then $\partial g(t)/\partial t=-2Ric(g(t))$, $-\infty<t\le 0$. This Ricci flow is an ancient
...
**Proof.** Recall that from Definition~ we have
$$H(X)=-\frac{\partial R}{\partial
\tau}-\frac{R}{\tau}-2\langle \nabla R,X\rangle+2Ric(X,X).$$
Using Hamilton's Harnack's inequality (Theorem~) with
$\chi=-X$, we have
...
$$
 Together with
the second equality in Theorem~, this gives
...
extends as an equation of $L^\infty_loc$-functions on all of
$M$.
...
The estimate on $K^\tau$ in Equation~() then gives
$$R(q,\tau)-\frac{(1+c^{-1})l_x(q,\tau)}{\tau}\le
...
valid as inequalities of $L^\infty_loc$-functions on all of
$M\times \{\tau\}$.
...
## The asymptotic gradient shrinking soliton for
$\kappa$-solutions
...
By Theorem~ for every $\tau>0$ there is a point $q(\tau)\in
...
establish below (see Corollary~ and
Corollary~) that in the case $n=2,3$, the gradient
...
We will divide the proof of Theorem~ into steps.
...
compactness theorem (Theorem~), we conclude that a
...
reduced volume (Lemma~).

By Theorem~ the reduced volume function $\widetilde
...
exists. By Corollary~ either this limit as $\tau$ goes
...
**Proof.** By the choice of $q_k$ we have $l_{x_k}(q_k,\tau_k)\le \frac{n}{2}$.
 By the scale invariance of $l$ (Corollary~) we have
...
If $\tau>1$, then
 integrating the second inequality in the second displayed line of Corollary~ gives
...
**Proof.** By Corollary~, for any $q\in M_k$ we have $|\nabla
...
statement follows from this and Proposition~.

It follows immediately from Corollary~ that for each
...
follows from Corollary~ that $R_{g_k}$ are also
...
scales, it follows from Theorem~ that after passing
...
Hamilton's Harnack inequality (Corollary~), we have
...
Corollary~, for any $A<\infty$ and any $0<\tau_0<T$,
...
in $C^{0,\alpha}_loc$ to a function $l_\infty$ defined on
$M_\infty\times(-\infty,0)$. Furthermore, it follows that the
restriction of $l_\infty$ is locally Lipschitz, and hence the
function $l_\infty$ is an element of $W^{1,2}_loc\left(M_\infty\times(-\infty,0)\right)$. Also, by passing to a
further subsequence if necessary, we can assume that the $l_k$
converge weakly in $W^{1,2}_loc$ to $l_\infty$.
...
**Proof.** This is immediate from Corollary~ and Fatou's lemma.
...
Chapter~ for $l_x$. Here is a crucial result.
...
The proof of this result is contained in Sections~
through~

### Preliminary results toward the proof of
Proposition~\protect{}

In this subsection we shall prove  that the left-hand side of
Equation~() is a distribution and is $\ge 0$ in the
...
=|\nabla l_\infty|_{g_\infty}^2 dvol(g_\infty)$$ in the sense
...
$$lim_{k\rightarrow \infty}|\nabla
l_k|^2_{g_\infty}dvol(g_\infty)\ge|\nabla
l_\infty|^2_{g_\infty}dvol (g_\infty)$$ in the sense of
distributions on $M_\infty\times \{t\}$. Thus, we have the following
inequality of distributions:
$$lim_{k\rightarrow\infty}|\nabla^{h_k}l_k|^2_{h_k}dvol(h_k)\ge
|\nabla l_\infty|^2_{g_\infty}dvol (g_\infty).$$
...
Theorem~. We must show that for each $t\le -\tau_0$ and
...
\left(|\nabla^{h_k} l_k|_{h_k}^2dvol(h_k)-|\nabla
l_\infty|_{g_\infty}^2dvol(g_\infty)\right)\le 0.$$ First,
...
\left(|\nabla^{h_k}l_k|_{h_k}^2dvol(h_k)-|\nabla
l_\infty|_{g_\infty}^2dvol(g_\infty) \right)} & & \\
&  = & lim_{k\rightarrow\infty}\int_{M\times\{t\}}\varphi (|
\nabla^{h_k} l_k|_{h_k}^2-|\nabla
l_\infty|_{h_k}^2)dvol(h_k) \nonumber \\
\nonumber& = & lim_{k\rightarrow\infty}\int_{M\times\{t\}}\langle\nabla^{h_k}
l_k-\nabla l_\infty),\varphi\nabla^{h_k} l_k\rangle_{h_k}dvol(h_k)
\\ & & \hskip.55in+\int_{M\times\{t\}}\langle\nabla^{h_k}l_k-\nabla l_\infty),
\varphi\nabla l_\infty\rangle_{h_k}dvol(h_k).
...
l_k-l_\infty),\varphi\nabla l_\infty\rangle_{g_\infty}dvol(
g_\infty).
$$
...
given in~() vanishes and hence that
$$lim_{k\rightarrow\infty}\int_{M\times\{t\}}\langle\nabla^{h_k}(
l_k-l_\infty),\varphi\nabla l_\infty\rangle_{h_k}dvol(h_k)=0.$$

It remains to consider the first term in the last expression in
Equation~(). (This is where we shall need the differential
...
$$
\begin{aligned}
\lefteqn{lim_{k\rightarrow\infty}\int_{M\times\{t\}}\langle\left(\nabla^{h_k}
...
**Claim.** $$lim_{k\rightarrow\infty}\int_{M\times\{t\}}\langle\nabla^{h_k}(l_k-l_\infty-\epsilon_k),
\varphi\nabla^{h_k}  l_k\rangle_{h_k} dvol(h_k)\le 0.$$

\begin{proof}
Since $\varphi$ is a compactly supported, non-negative smooth
function, it follows from Theorem~ that we have the
...
l_k,\nabla^{h_k}(\varphi \cdot f)\rangle_{h_k} dvol(h_k) \le}
\\
& & \int_{M\times\{t\}}\frac{\varphi f}{2}\left(|\nabla^{h_k}
l_k|_{h_k}^2-R_{h_k}-\frac{l_k-n}{\tau}\right)dvol(h_k).
\end{aligned}
$$
...
\begin{aligned}
\lefteqn{\int_{M\times\{t\}}\langle \nabla^{h_k}(\varphi(
l_k-l_\infty-\epsilon_k)),\nabla^{h_k} l_k\rangle_{h_k}dvol(h_k) \le}  \\
...
l_k-l_\infty-\epsilon_k)),\nabla^{h_k} l_k\rangle_{h_k}dvol(h_k)$$ has a limsup $\le 0$ as $k$ tends to $\infty$. Now we
expand
...
hence completed the proof of Lemma~.
\end{proof}
...
\tau$. Hence, by taking limits from Theorem~, using
Lemma~, we see that
...
element of $L^\infty_loc$, it also extends to a continuous
...
l_\infty\rangle_{g_\infty} dvol(g_\infty)d\tau.$$ Since
...
Proposition~ are absolutely convergent so that they extend to
...
By Corollary~, we have $\gamma_1'(\tau)=\nabla
...
From this inequality, Corollary~, and the fact that
$R\ge 0$, we have
...
$-\partial/\partial t$ of Proposition~, we apply
...
Now plugging Equation~() and~() and  the above
inequality into Equation~() we see that
...
where  $c_1$ is the constant from Lemma~.

**Proof.** By Claim~
$$l_{x_k}(q_k,\tau')\le \frac{n}{2(\tau')^2}+\frac{n\tau'}{2}.$$
Now applying Lemma~ we see that for any $0<\tau'$ and any $q'\in
M_k$ we have
$$
...
curvature, by Theorem~ the volume of the ball of
...
**Proof.** By
Corollary~, each of the Lipschitz functions $|\nabla
...
l_\infty\right)f\tau^{-n/2}e^{-l_\infty}dvol_{g_\infty}d\tau\ge 0.$$
...
functions. Hence, we can apply Claims~
and~ to establish that the integral in question is the
limit of an absolutely convergence sequence. By
Corollary~ each term in the sequence is non-positive.
...
l_\infty(q,\tau)}dvol_{g_\infty(\tau)}.$$ By Corollary~, the function $\tau\rightarrow
...
\left(\tau^{-n/2}e^{-l_\infty(q,\tau)}dvol_{g_\infty(\tau)}\right).
\end{aligned}
$$

$$
 According to
Corollary~ this is an absolutely convergent integral,
...
e^{-l_\infty}dvol_{g_\infty}d\tau & =
&\int_{\tau_0}^{\tau_1}\int_{M_\infty\times\{-\tau\}}\left(|\nabla
l_\infty|^2-\triangle l_\infty\right)e^{-l_\infty}dvol_{g_\infty}d\tau \\
& = & 0.
\end{aligned}
$$
...
Corollary~, necessary to show that this integral is
well defined.
...
l_\infty\right)\tau^{-n/2}e^{-l_\infty}dvol_{g_\infty}=0.
$$
...
Equation~() holds in the usual sense.

Now from the last two equations in Corollary~ and the
...
Now we return to the proof of Theorem~. We have shown that the
...
Proposition~. We shall use these to establish the gradient
shrinking soliton equation,
Equation~(), for the limit for $f=l_\infty$.
...
to Proposition~ and the first statement in
Lemma~, the function $u$ satisfies the conjugate heat
...
other hand, the second equality in Proposition~ shows
...
Then, of course,
$$\frac{\partial v}{\partial t}=\frac{\partial H}{\partial t}u+H\frac{\partial u}{\partial t}\ \ \ and$$
...
$$
\begin{aligned}
\frac{\partial }{\partial t}\triangle f & = & 2Ric^{ij}Hess(f)_{ij}+g^{ij}\frac{\partial}{\partial t}\left(Hess(f)_{ij}\right)
\\
& = & 2Ric^{ij}Hess(f)_{ij}+g^{ij}Hess\left({\frac{\partial f}{\partial
...
$$g^{ij}\frac{\partial \Gamma_{ij}^k}{\partial t}=g^{kl}g^{ij}\left(-(\nabla_j Ric)_{li}-(\nabla_i
 Ric)_{lj}+(\nabla_lRic)_{ij}\right).$$
 This expression vanishes by the second Bianchi identity (Claim~).
...
Plugging this and Claim~ into Equation~()
yields
...
using the Equation~() for $\partial R/\partial t$ yields
...
$|Hess(f)|^2$. According to Lemma~ we have
...
& = & 2\tau\bigl|Ric+Hess(f)-\frac{1}{2\tau}g_\infty\bigr|^2
\end{aligned}
$$
...
results claimed in Theorem~ except for the fact that
...
Theorem~\protect{}
 ...
 To complete the
proof of Theorem~ it remains to show that for no $t\in
...
Lemma~ and hence by the above argument, $f$ also satisfies
the equation given in  Proposition~. If the limit is
...
Theorem~ this implies that $\widetilde
V_\infty(\tau)=(4\pi)^{n/2}$. This contradicts Corollary~, and
the contradiction establishes that $(M_\infty,g_\infty(t))$ is not flat for any
$t<0$.  Together with Proposition~, this completes the proof of
Theorem~, namely of the fact that $(M_\infty,g_\infty(t)),\
...
of non-negative curvature. They are both based on Theorem~.
...
continuous function on $B_i$. Applying Corollary~ to
...
Theorem~ there is a geometric limit flow
...
$(M_\infty,g_\infty(-1))$ is the geometric limit of $(M,\lambda_ig(-1),q_i)$.
 Thus, according
to Theorem~, the limit $(M_\infty,g_\infty(-1))$ splits as a
...
from the maximum principle (Corollary~) that the entire flow
...
Now let us begin the proof of Theorem~
...
$$
 We compute
$$\frac{\partial G}{\partial t}=-\Phi_{h(t)}^*g+2\Phi_{h(t)}^*Hess^g(f)=-2\Phi_{h(t)}^*Ric(g)=-2Ric(G(t)),$$ so that
...
### Case 1: $M$ is compact and the curvature is strictly
positive

**Claim.** Suppose that $(M,g)$ and $f\colon M\to \Ar$ satisfies the hypotheses of
Theorem~ and that $M$ is compact and of positive curvature. Then
the Ricci flow $(M,G(t))$ with $G(-1)=g$ given in Equation~() is a
shrinking family of compact round manifolds.

**Proof.** The
manifold $(M,G(t))$ given in Equation~() is equivalent up to
...
(Theorem~), the Ricci flow becomes singular in finite time and
...
. According to Proposition 5.21 on p.. 118 of
, $M$ is a shrinking family of constant positive curvature
...
Theorem~), this implies that the $(M,g(t))$ are all
exactly round.
...
of Theorem~ and that $(M,g)$ does not have strictly
positive curvature. Then $n=3$ and the Ricci flow $(M,G(t))$ with
$G(-1)=g$ given in Equation~() has a one- or two-sheeted
...
(Corollary~), the Ricci flow $(M,G(t))$ has a one- or
...
one-manifold. Then it follows from Equation~() that the
...
### Case 3: $M$ is non-compact and
strictly positively curved
...
Theorem~ with $(M,g)$ non-compact and of positive curvature.

We suppose that we have $(M,g)$ as in Theorem~ with
...
Lemma~ we rewrite this equation as
...
\right. $$ Adding up Equation~() for each $i$ gives
...
\begin{array}{ll} s^2Ric(X,X) & \mbox{if $0\le s\le 1$} \\
Ric(X,X) & \mbox{if $1\le s\le \bar s-1$} \\ (\bar s-s)^2Ric(X,X) & \mbox{if $\bar s-1\le s\le \bar s$.} \end{array} \right.
$$ Since the curvature is bounded and $|X|=1$, we see that
$\int_0^{\bar s}(1-s^2)Ric(X,X)ds+\int_{\bar s-1}^{\bar
s}(\bar s-s)^2Ric(X,X)$ is bounded independent of $\gamma$ and
of $\bar s$. This concludes the proof of the claim.

**Claim.** $|Ric(X,\cdot)|^2\le R \cdot Ric(X,X)$.
...
s\int_0^{\bar s}R\cdot Ric(X,X)ds.
\end{aligned}
$$
...
applying the gradient shrinking soliton equation to the pair
$(X,\widetilde E_i)$, using Equation~() and the fact that
...
Now apply Equation~() to see that $R$ is increasing along the gradient
...
Equation~() has the property that $G(-1)=g$ and that
$(M,G(t))$ is isometric to $(M,|t|g)$. Since the original Riemannian
manifold $(M,g)$ given in the statement of Theorem~ is
...
satisfies all the hypotheses of Proposition~. Hence,
by Corollary~ we see that $n$ cannot be equal to two.
Furthermore, by Proposition~, when $n=3$ there is
...
satisfying the hypotheses of Theorem~. For any
non-compact three-manifold $(M,g)$ of positive curvature satisfying
the hypotheses of Theorem~, there is a sequence of
...
**Proof.** In Claim~ we saw that every two-dimensional $(M,g)$ satisfying
the hypotheses of Theorem~ has strictly positive curvature. The
...
the hypotheses of Proposition~ except possible the
...
Proposition~ except possibly non-compactness. It then follows
from Corollary~ that $M$ is compact. This proves the first item.
...
and
 Theorem~ the Ricci flow $(M,g(t))$ satisfies all the hypotheses
 of Proposition ~. But this contradicts Corollary~.
 We conclude that $(M,g(-1))$ has bounded curvature. According to
 Corollary~ this means that
 $(M,g(t))$ is compact.
Results going back to Hamilton in  imply that
...
5.64 on p. 149 of  the surfaces $(M,g(t))$ are also becoming
round as $t$ approaches $\Omega$ from below. Also, according to Proposition
5.39 on p. 134 of  the entropy of the flow is weakly monotone
...
of Proposition~. Let $(M,g)$ be a non-compact
manifold of positive curvature satisfying the hypotheses of
Theorem~. According to Corollary~ the
limiting Ricci flow $(M_\infty,G_\infty(t))$ referred to in
Corollary~ is the product of a line and a
...
$R_inf$, is positive. Thus, the infimum of the scalar curvature of
$(M,G(t))$ is $R_inf/|t|$ and goes to infinity as $|t|$ approaches $0$.
...
According to Equations~() and~() the angle between $\nabla
f$ and the gradient of the distance function from $p$ goes to zero as we go to
infinity. According to Theorem~ the gradient of the distance
...
$$S=\frac{Hess(f|TN)}{|\nabla f|}.$$
Clearly, we have $R-2Ric(e_3,e_3)=2K_M(e_1,e_2)$, so that
$$R_N=R-2Ric(e_3,e_3)+2det\, S.$$
...
$$
 R_N\le R-2Ric(e_3,e_3)+\frac{(1-R+Ric(e_3,e_3))^2}{2|\nabla f|^2}.
$$
...
Plugging this into Equation~() gives that
...
the proof of Proposition~, that is to say this shows
...
We return now to the second case of Theorem~.  We
extend $(M,g)$ to a Ricci flow $(M,G(t))$ defined for $-\infty<t<0$
as given in Equation~(). By Claim~ $M$
...
itself a $\kappa$-solution and hence by Corollary~ the
...
Proposition~. Then the limit constructed in that proposition
...
Proposition~ is the product of a shrinking family of round
surfaces and the real line.

**Proof.** Let $(M,g(t))$ be a three-dimensional Ricci flow satisfying the
hypotheses of Proposition~ and let $(N^2,h(t))\times
...
Proposition~ except possibly non-compactness. It now
follows from Corollary~ that $(N,h(t))$ is a shrinking
...
Corollary~, $(M,g(t))$ has a covering that splits as a product
of a a two-dimensional Ricci flow and a line. The two-dimensional Ricci flow
satisfies all the hypotheses of Proposition~ except possibly
compactness, and hence by Corollary~ it is a shrinking family of
...
to infinity. By Corollary~ we can replace the points $p_i$ by
...
Since $(M,g(t))$ has positive curvature, by Theorem~, it is
diffeomorphic to $\Ar ^3$, and hence it contains no embedded $\Ar P^2$. It
follows that $(N,h(t))$ is a round two-sphere.

Fix $\epsilon>0$ sufficiently small as in Proposition~. Then the
...
contradicts Proposition~, establishing that for each $t<0$ the
...
this $\kappa$-solution, as constructed in Theorem~, is of one of
the three types listed in Theorem~.

**Proof.** Let $(M_\infty,g_\infty(t)),\ -\infty<t<0$, be an asymptotic
gradient shrinking soliton for $(M,g(t))$. According to
Corollary~, this soliton is a $\kappa$-solution,
...
to Theorem~ there is a function $f(\cdot, -1)$ from
$M_\infty$ to $\Ar$ satisfying the gradient shrinking soliton
equation at the time-slice $-1$.
 Thus,
Theorem~ applies to $(M_\infty,g_\infty(-1))$ to
...
the second type listed in Theorem~. Then
$(M_\infty,g_\infty(-1))$ is a product of a round two-sphere and the
real line. By Corollary~ this implies that the entire
...
**Proof.** By Corollary~ the only two-dimensional $\kappa$-solutions are
...
Corollary~ it must be three-dimensional and it has a
...
$B_{h_n}(x_n,0,\alpha)\times (-\infty,0]$.
 This means
that by Shi's theorem (Theorem~), there are bounds,
...
$$(B_{g_\infty}(x_\infty,0,\alpha/2),g_\infty(t)),\ -\infty<t\le 0,$$
 this contradicts Proposition~. This completes the proof that
...
Theorem~, and the fact that the original flow is $\kappa$
...
$$lim_{n\rightarrow\infty}Vol\left(B_{g}(p,0,\sqrt{\lambda_n}r)\setminus
B_g(p,0,1)\right)\rightarrow \omega_3\lambda_n^{3/2}r^3.$$ By
Theorem~ and the fact that the Ricci curvature is
...
Corollary~ since $(M,g(t))$ is not a family of
round manifolds, the asymptotic soliton for the $\kappa$-solution
cannot be compact. Thus, according to Corollary~ there
...
length under rescalings, see Corollary~), it follows easily
...
bounded independent of $k$. Now by Theorem~ this implies
...
(Theorem~), this is a non-increasing function of
...
in all dimensions. Our proofs use Theorem~ and
Proposition~ which are also valid in all dimensions but which we
...
Corollary~ there are only compact $\kappa$-solutions, which clearly
have zero asymptotic volume. Suppose that we have established the result for
$n-1\ge 2$ and let us prove it for $n$.

According to Proposition~ there is a sequence of points $p_n\in
...
Theorem~ we can then pass to a subsequence that has a
...
inequality (Corollary~) the scalar curvature of
...
curvature operator. Suppose $p\in M$, and $r>0$ are such that $Vol\,B(p,0,r)\ge \nu r^n$. Then $r^2R(q,0)\le C$ for all $q\in
B(p,0,r)$.
...
\end{enumerate}
Using Lemma~ we can find points $q_k'\in B(p_k,0,2r_k)$
...
inequality (Theorem~) that $Vol\,B(q_k',0,s)\ge (\nu/3^n)s^n$ for any $s\le s_k$.

Of course, by Corollary~, we have $R(q,t)<4Q_k'$ for
...
positive constant (see Claim~ for a more precise
...
inequality (Theorem~),  it follows that for every
$k$ sufficiently large there is $r_k<2d_k$ such that

$$

Vol\,B(q_k,0,r_k)=[\omega_3/2]r_k^3.
$$
 Of course,
because of Equation~() we see that $lim_{k\rightarrow\infty}r_k/d_k=0$. Then, according to
Corollary~, we have for all $q\in M_k$

$$(r_k+d_{g_k(0)}(q_k,q))^2R(q,0)\le K(d_{g_k(0)}(q_k,q)/r_k),$$
where $K$ is as given in Corollary~. Form the sequence
...
Furthermore, Equation~() passes to the limit to give

$$

Vol\,B_{g_\infty}(q_\infty,0,1)=\omega_3/2.
$$
...
scalar curvature (Theorem~), this implies that
the limit $(M_\infty,g_\infty(0))$ is flat. But
Equation~() tells us that this limit is not $\Ar^3$. Since
...
Harnack inequality (Corollary~) that  $R(q',t)\le CQ$ for all
...
estimates (Theorem~)applied  with $T=2$ and $r=2$, there is
...
the metric is non-negatively curved, by Corollary~ we
have  $2|Ric(q,t)|^2\le 2Q^2$ for all $t\le 0$.
 From these two facts we conclude from
the flow equation~() that there is a constant $1<C''<\infty$ with
...
universally bounded on $B(q,0,2d)$ by Claim~, and since
$R(q,0)$ is universally bounded by Lemma~, the second
...
$\kappa$-solutions with $R(p_k,0)=1$ for all $k$.
 The immediate
consequence of Lemma~ and Corollary~
...
the sequence are $\kappa$-non-collapsed, by Theorem~
...
\begin{aligned}
 ...
 sup_{(x,t)}\frac{\left|\frac{d}{dt}R(x,t)\right|}{R(x,t)^2}<C.
\end{aligned}
$$
...
Because of Proposition~, and the fact that the previous
...
Notice that, using Equation~(), we can rewrite the second
...
In Chapter~ we defined the notion of an
...
**Proof.** This is immediate from Equation~().
...
\item[(4)] $$C^{-1}sup_{y\in M}\left(R(y)^{-1/2}\right)<diam(M)<Cinf_{y\in M}\left(R(y)^{-1/2}\right).$$
\end{enumerate}
...
{\mathcal M}$ with $**t**(x)=t$ is an embedding $\psi\colon
...
interval (see Definition~ for the notion of two
...
{\mathcal M}$ be a point with $**t**(x)=t$. We say that an open
...
{\mathcal M}$ be a point with $**t**(x)=t$.
...
in Corollary~, there is a covering $\widetilde M$ of
...
a geometric limit. Thus, by Theorem~ and
Corollary~ the limit splits as a product of a
$2$-dimensional $\kappa$-solution and $\Ar$. By
Corollary~ it follows that the limit is the standard
...
This contradicts Lemma~, and completes the proof of
...
Proposition~ either it is positively curved or it is a
...
Proposition~ for any $L<\infty$, for all $k$
...
By Corollary~ the subset of points in $M_k\times
...
Part 3 of Proposition~ each $(q_k,0)$ is the center
...
center of a $2\epsilon$-neck, from Lemma~ we see
...
ends. According to Theorem~, after passing to a
...
Proposition~, it is the center of a $4\epsilon$-neck
in the limit.
...
$R(p_k,0)=1$. By Lemma~, if $\epsilon$ is sufficiently
small, then $p_k$ cannot be the center of an $\epsilon$-neck. It
follows from Proposition~ that for any limit of a
...
Proposition~ and Corollary~. If the
...
$\Ar P^2$ with trivial normal bundle.
 ...
 We assume that $(M,g(0))$ is not round so that by Proposition~
...
According to Theorem~, after passing to a
...
is non-compact. According to Corollary~ the point
...
Proposition~ that for all $k$ sufficiently large,
...
Now it follows from Proposition~ that if the
...
finite, see Theorem 4.1 on p. 154 of . This rules out
...
round, by Corollary~ its asymptotic soliton is not
compact. Thus, by Theorem~ its asymptotic soliton is
...
Corollary~ and Theorem~, respectively.
For each $0<\epsilon\le min(\bar \epsilon_2,\bar\epsilon_3)$
let $C_1=C_1(\epsilon)$ be as in Theorem~. There is
...
**Proof.** The result is immediate from Corollary~ and
Theorem~.
...
sequence of based, generalized Ricci flows with $**t**(x_n)=0$ for
...
Proposition~, any family of metrics on this neck
...
Proposition~ for all $n$ sufficiently large $x_n$ is
