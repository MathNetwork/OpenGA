...
defined on $[0,\delta)$. We denote by $Reg_\delta(U,g)$ the
subset of $\delta$-regular points in $(U,g)$. For any $x\in Reg_\delta(U,g)$ we denote by $Reg_\delta(U,g,x)$ the
connected component of $Reg_\delta(U,g)$ containing $x$.

Intuitively, the $\delta$-regular points of $(U,g)$ are at distance
at least $\delta$ from the boundary on $U$.

**Lemma.** $Reg_\delta(U,g)$  is a closed subset of $U$.

**Proof.** Suppose that $p_n$ converges to $p$ as $n$ tends to $\infty$ and
suppose that $p_n\in Reg_\delta(U,g)$ for all $n$. Fix
...
(U_k,x_k)$ with the properties that: \begin{enumerate} \item $lim_{k\rightarrow \infty}\varphi_k^*g_k=g_\infty$, where the limit
...
$x_k\in Reg_\delta(U_k,g_k)$ and for any $\ell\ge k$ the image
$\varphi_\ell(V_k)$ contains $B(x_\ell,R)\capReg_\delta(U_\ell,g_\ell,x_\ell)$.
\end{enumerate}
\end{enumerate}
...
Reg_\delta(U_\infty,g_\infty,x_\infty)$. Let $x$ be contained
in the closure of $V_k$. Then by the triangle inequality the closed
ball $\overline{B(x,\delta/3)}$ is contained in
$B(x_\infty,R+\delta)\capReg_{\delta/2}(U_\infty,g_\infty,x_\infty)$. Since the union of
...
large $\varphi_\ell(V_k)\subset B(x_\ell,R+2\delta)\cap Reg_{\delta/4}(U_\ell,g_\ell,x_\ell)$. This implies that, for given
...
$n$) such that $\overline V'_n\subset B(x'_\infty,R)\cap Reg_\delta(V_{n+1},g'_\infty,x_\infty')$. Since $V'_{n+1}$ has compact closure
...
$\varphi'_\ell(V_n)\subset B(x_k,R')\cap Reg_{\delta'}(U_\ell,g_\ell,x_\ell)$. This implies that for all $k$
...
\begin{enumerate}
\item[(1)] There is $\delta>0$ such that $x_k\in Reg_\delta(U_k,g_k)$ for all $k$.
\item[(2)] For each $R<\infty$ and $\delta>0$ there is a constant $V(R,\delta)<\infty$ such
that $Vol(B(x_k,R)\cap Reg_\delta(U_k,x_k))\le
...
$B(x_k,R+\delta)\cap Reg_\delta(U_k,g_k,x_k)$ is geodesically
...
$2\delta'$ respectively.
 Notice that each of the balls $\widetilde B_i$  is contained in $B(x_k,R+\delta)\cap Reg_{\delta}(U_k,g_k,x_k)$. Because $\delta'\le r_0$, because $Vol\,B(x_k,R+\delta)$ is bounded independent of $k$, and because the
...
U_\infty(\delta,R)\to B(x_k,R+\delta)\cap Reg_\delta(U_k,g_k,x_k)$ sending $x_\infty(\delta,R)$ to $x_k$ and
converging as $k\rightarrow \infty$, uniformly in the
$C^\infty$-topology on each $B_i$, to the identity. Furthermore, the
images of each of these maps contains $B(x_k,R)\cap Reg_{2\delta}(U_k,g_k,x_k)$; compare . Also, the pull
...
$$B(x_k,2R+\delta_2)\cap Reg_{\delta_2}(U_k,g_k,x_k).$$ Hence,
...
and since
$$Reg_\delta(U_\infty,g_\infty,x_\infty)=U_\infty$$ for
every $\delta>0$, it is easy to see that the extra condition, (2b), in
Definition~ is automatic in this case.

Now as an immediate corollary of Theorem~ we have the
following.
...
(Theorem~) that for each $A<\infty$ there is a uniform bound
...
Theorem~ that there is a geometric limit.
...
Theorem~ hold and suppose also that there are constants $\kappa>0$
...
**Proof.** Let $A=max(\delta^{-2},C(0,\delta))$, where $C(0,\delta)$ is the constant
given in the second condition in Theorem~. Rescale, replacing the
Riemannian metric $g_k$ by $Ag_k$. Of course, the first condition of
Theorem~ still holds as does the second with different constants,
...
Bishop-Gromov inequality (Theorem~), we have $Vol_{Ag_k}B(x_k,1)\ge \kappa/\Omega$ where
...
independent of $k$.
 According to Theorem~ the lower bound on the volume of the ball
...
sequence of manifolds satisfies the third condition in Theorem~.
Invoking this theorem gives the result.

### Geometric convergence of manifolds in the case of Ricci
flow
...
generalized $n$-dimensional Ricci flows with $**t**(x_k)=0$. Let
...
**Proof.** The first condition in Theorem~ holds by our first
assumption. It is immediate from Shi's theorem (Theorem~)
that the second condition of Theorem~ holds. The result
is then immediate from Corollary~.
...
generalized Ricci flows. We suppose that $**t**(x_k)=0$ for all $k$ and we
...
M_\infty$ satisfying (1) of Definition~ with $M_\infty$ in
place of $U_\infty$,
...
Ricci flows. Suppose that $**t**(x_k)=0$ for all $k$, and denote by
...
$\varphi_k\colon V_k\to M_k$ as in Definition~. Suppose that
...
$g_k(K,I)$ are uniformly bounded $C^\infty$-topology. The basic computation
 done by Hamilton in  shows that after passing
...
Suppose that $**t**(x_k)=0$ for all $k$, and denote by $(M_k,g_k)$ the $0$
...
**Proof.** By Theorem~ there is a geometric limit $(M_\infty,g_\infty(0))$
of the $0$ time-slices, and the limit  is a complete Riemannian manifold. Then
by Proposition~ there is a geometric limit flow defined
...
convergence is uniform in the $C^\infty$-topology. Now by Hamilton's result
 it follows that, passing to a further subsequence, the
...
smoothly with the flow on $[0,T]$ by Proposition~.
...
based metric space,
$$f_k\colon (X_k,x_k)\to
(Z_k,z_k) \ \ \ \ and \ \ \ g_k\colon (Y,y)\to (Z_k,z_k)$$ are
...
For a proof see Proposition 3.5 on page 36 of .
...
$|ab|=|bc|=1$ and $|bc|=\ell(\gamma,\mu)$, see Fig.~.  If $\nu$ is a third geodesic ray
...
$\theta_\infty$ descends to a metric on this space. This space is a
 length space . Notice that the distance between
...
$$d^2([x_1,a_1],[x_2,a_2])=a_1^2+a_2^2-2a_1a_2cos(\theta_\infty(x_1,x_2)).$$
...
in $B_{g_n}(p,R)$. From the second item of Theorem~ and the
...
Clearly, from the second item of Theorem~ and the
...
introduced and studied by Hamilton in , where,
...
generalized Ricci flows. We suppose that $**t**(x_k)=0$ for all
...
originally established in , the latter
...
that $Q_k=R(x_k)$ tends to infinity as $k$ tends to infinity. Let $t_k=**t**(x_k)$ and let $({\mathcal M}_k',G_k',x_k)$ be the result of shifting time by
$-t_k$ so that $**t**'(x_k)=0$. Then any blow-up limit of the sequence
...
Since by Equation~() we have $R(y_k)\ge -6$ for all $k$ and since
...
to Equation~() the ratio of $X(y_k)/R(y_k)$ goes to zero. Since
...
**Proof.** According to Theorem~ the hypotheses imply that for every $k$ the
...
automatically splits off a line; see Fig.~.
...
(first part of Theorem~) and the usual law of
...
Toponogov comparison result (first part of Theorem~), we have
$$d(x,y)\ge \sqrt{\alpha^2+\beta^2-2\alpha\betacos(\theta_n)}.$$
The angle $\theta_n$ satisfies:
$$d_n^2+d^2-2d_ndcos(\theta_n)=d_{m(n)}^2.$$
Thus,
$$
...
implies by Lemma~ that the limit manifold is a Riemannian
