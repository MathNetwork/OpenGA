...
Theorem~. This Ricci flow with surgery becomes extinct after a finite
...
**Proof.** (of Theorem~ assuming
Theorem~). Fix a normalized metric $g(0)$ on $M$, and let
$({\mathcal M},G)$ be the Ricci flow with surgery defined for all $t\in
[0,\infty)$ produced by Theorem~ with initial conditions $(M,g(0))$.
According to Theorem~ there is $T>0$ for which the time-slice
$M_T$ is empty. By Corollary~, if there is $T$ for which $M_T$
...
Theorem~. In particular, if $M$ is simply connected, then $M$ is
...
Theorem~ which will then complete the proof of
Theorem~.
...
distance-decreasing property for surgery in Proposition~ we
...
satisfying the conclusion of Theorem~.
...
(Proposition~) says that the inequality remains
...
The other approach, by Colding-Minicozzi , is
to represent a non-trivial element in $\pi_3(M_T)$ as a non-trivial
element in $\pi_1(Maps(S^2,M))$, and associate to such an
...
(Proposition~) implies that this inequality is
...
that occupy most of Perelman's analysis in .
 ...
 represents a point in the space $Maps(S^2,M)$ that is
...
. He was considering a situation where a
...
that given a Ricci flow with surgery as in Theorem~ there
...
hypothesis of Theorem~, there is a $T_2<\infty$ such
...
Let $(M,g(0))$ be as in the statement of Theorem~, so that $M$ is
...
) the first homology of the factor in question maps onto $\Zee$
...
Thus, by Theorem~,  for any compact $3$-manifold $M$ whose
...
conclusion of Theorem~ with $(M,g(0))$ as the initial conditions.
...
X}\subset **t**^{-1}(I)$ with the property that for every $t\in I$
...
point is a surgery time.  Then the intersection of ${\mathcal X}$ with $**t**^{-1}(I')$ is  the Ricci flow on the time interval $I'$ applied to ${\mathcal
X}(t)$ for any $t\in I'$.  Thus, for such intervals $I'$ the intersection,
${\mathcal X}(I')$, of ${\mathcal X}$ with $**t**^{-1}(I')$ is determined by
...
conclusions of Theorem~. We make no assumptions about the
...
show that if $t$ is a surgery time, then $W_2(t)\le liminf_{t'\rightarrow t^-}W_2(t)$. Lastly, we show that at any
point $t\ge T_0$ we have
$$\frac{dW_2}{dt}(t)\le -4\pi-\frac{1}{2}R_min(t)W_2(t),$$
in the sense of forward difference quotients. It follows easily from
the bound $R_min(t)\ge -6/(4t+1)$ that there is $T_1({\mathcal
...
**Proof.** This is a consequence of Grushko's theorem , which
...
in $X$, then by van Kampen's theorem, see p. 40 of ,
...
$H_1(X)$,
 applying
Lemma~ and using the fact that the fundamental group of a
compact manifold is finitely presented establishes the result.

### Two-sphere surgeries are trivial after finite
time
...
Assumptions (1) -- (7) in Chapter~. Then there can
...
According to Proposition~, if $t$ is a surgery
...
Proposition~ is immediate from this claim and the
previous lemma.
\end{proof}
...
M},G)$ satisfying the conclusion of Theorem~ there is $T_0<\infty$,
...
of Theorem~. Then there is some time $T_1<\infty$ such that every
...
**Proof.** The first statement is Theorem 3.3 in . As for
...
$$E_\alpha(s)=\int_{S^2}\left(1+|ds|^2\right)^\alpha da.$$
 According to
 this energy functional is Palais-Smale on the
...
maps. According to , after passing to a
...
point where the limit $s$  is not a strong limit. Then according to
 pre-composing with a sequence of conformal
...
**Lemma.**
$$\frac{d}{dt}W_2(t)\le -4\pi-\frac{1}{2}R_min(t)W_2(t)$$
...
Let us show how this lemma implies Proposition~. Because
...
and $w_2(T_0)=W_2(T_0)$. Then by Lemma~ and
Lemma~ we have $W_2(t)\le w_2(t)$ for all $t\ge T_0$. On
...
This shows that to complete the proof of Proposition~ we
need only establish Lemma~.

**Proof.** (of Lemma~)
Let $f\colon S^2\to (X(t_0),g(t_0))$ be a minimal $2$-sphere.

**Claim.**
$$\frac{dArea_{g(t)}(f(S^2))}{dt}(t_0)\le -4\pi -\frac{1}{2}
R_min(g(t_0))Area_{g(t_0)}f(S^2).$$
...
\begin{aligned}
 ...
 \f1{d}{dt}Area_{g(t)}(f(S^2))\bigl|_{t=t_0}\bigr. & = &
 \int_{S^2}\frac{1}{2}Tr|_{S^2}\Bigl(\frac{\partial g}{\partial t}\Bigr)\Bigl|_{t=t_0}\Bigr.da \\
 & = &
 -\int_{S^2}(R-Ric(**n**,**n**))da \nonumber
\end{aligned}
$$
...
Ricci curvature of $M$, and $**n**$ is the unit normal vector field of $\g1$
...
$$
\begin{aligned}
\f1{d}{dt}Area_{g(t)}(f(S^2))\big |_{t=0}
 &= &
 -\int_{S^2}K_{S^2}da-\f1{1}{2}\int_{S^2}(|A|^2+R)da,
\end{aligned}
$$
...
() still holds when  the integral on the right is replaced
...
$$
\begin{aligned}
\f1{d}{dt}Area_{g(t)}(f(S^2))\bigl|_{t=t_0}\bigr.  & \leq &
-4\pi-\f1{1}{2}Area_{g(t_0)}(S^2)\min_{x\in M}\{R_{g}(x,t_0)\}.
\end{aligned}
$$
...
$t$, the forward difference quotient statement in Lemma~ follows
immediately from Claim~.
...
$$\int_{f(S^2)}\frac{\partial g}{\partial t}(t)da=-2\int_{f(S^2)}Tr|_{TS^2}(Ric_{g(t)})da\le
4DArea_{g(t)}f(S^2) .$$ Integrating from $t_0$ to $t_1$ gives
the result.
...
sufficiently large, and hence $W_2(t')\le liminf_{n\rightarrow\infty}W_2(t_n)$. This is the lower
semi-continuity.
...
Section~, we write ${\mathcal X}(t)$ as the union
...
Proposition~ that for any $\eta>0$ for all $t'<t$
...
This establishes all three statements in Proposition~ and
completes the proof of the proposition.

As an immediate corollary of Proposition~, we obtain the
...
conditions. According to Proposition~ there is $T<\infty$
...
conclusion of Theorem~ and also has initial condition $M$ that is a
connected $3$-manifold whose fundamental group satisfies the hypothesis of
Theorem~. The argument showing that  components with non-trivial
...
$S^2\times S^1\to M$ that sends $\{pt\}\times S^1$ to $x_0$.
Hence, this map factors through the quotient of $S^2\times S^1$
obtained by collapsing $\{pt\}\times S^1$ to a point. The
...
Now let us formulate the analogue of Proposition~ for
...
X}(t)$ given by Proposition~ is either a homotopy
...
Proposition~ sends $\xi(t')$ to $\xi(t)$ for all
...
Here is the result that is analogous to Lemma~.

**Proposition.**
Suppose that $({\mathcal M},G)$ is a Ricci flow with surgery as in
Theorem~. Let ${\mathcal X}$ be a path of components of ${\mathcal
...
$$\frac{d W_\xi(t)}{dt}\le -2\pi-\frac{1}{2}R_min(t)W_\xi(t).$$ Also, for every $t\in [t_0,t_1]$ that is not a
...
### Proof of Theorem~\protect{ assuming
Proposition~\protect{}}

 According to Proposition~ there is $T_1$ such that
every component of $M_T$ has trivial $\pi_2$ for every $T\ge T_1$.
Suppose that Theorem~ does not hold for this Ricci flow
...
\begin{proof}
See , Theorem 5.2 on page 56 (for the case of a copy of $\Zee$)
and  Theorem 7.1 on page 66 (for the case of a free product
decomposition).
...
Now we can apply Proposition~ to our path of components
${\mathcal X}$ defined for all $t\in [T_1,T_2]$. First recall by
Theorem~ that the curvature of $({\mathcal M},G)$ is pinched toward
positive which implies that $R_min(t)\ge (-6)/(1+4t)$. Let $w(t)$ be the
function satisfying the differential equation
$$w'(t)=-2\pi+\frac{3}{1+4t}w(t)$$
with initial condition $w(T_1)=W_\xi(T_1)$. According to
Proposition~ and Proposition~ we
...
Theorem~ modulo Proposition~.
\end{proof}

Thus, to complete the argument for Theorem~ it remains
only to prove Proposition~.

### Continuity of $W_\xi(t)$

In this subsection we establish the two continuity conditions for
$W_\xi(t)$ stated in Proposition~.
...
**Proof.** This is immediate from the fact from Proposition~ that for any
$\eta>0$ for every $t'<t$ sufficiently close to $t$ there is a homotopy
equivalence ${\mathcal X}(t')\to {\mathcal X}(t)$ which is a
$(1+\eta)$-Lipschitz map.

To prove Proposition~ and hence
Theorem~, it remains to prove the forward difference
...
$\Gamma$ in order to establish Proposition~. Now we
...
Proposition~. Here is the result that shows a
...
\frac{dw_{a,t'}}{dt}=-2\pi-\frac{1}{2}R_min(t)w_{a,t'}(t)
$$
...
quotient result in Proposition~. Let ${\mathcal X}$ be a path
...
all the ${\mathcal X}(t)$ are identified under the Ricci flow.
 Let
$w(t)$ be the solution to Equation~() with value
...
**Definition.** Let $A(t)=\int_{t'}^t\frac{1}{2}R_min(s)ds$.

Direct integration shows the following:

**Claim.**  We have
$$w_{a,t'}(t'')=exp(-A(t''))\left(a-2\pi\int_{t'}^{t''}exp(A(t))dt\right).$$ If $a'>a$, then for $t_0\le t'<t''\le t_1$,
we have
...
Proposition~. We consider the restriction of the
...
identified with each other under the flow.
 Let $w(t)$ be the solution to Equation~()
with initial condition $w(t_0)=W_\xi(t_0)$. There are two cases to
consider: (i) $w(t_1)\ge 0$ and $w(t_1)< 0$.

Suppose that $w(t_1)\ge 0$. Let $\eta>0$ be given. Then by Claim~
and  Corollary~, there is $0<\zeta<\eta/2$ such that the
...
Proposition~ there is a one-parameter family
...
such that $w(t_1)+\eta<0$. Then using Lemma~ and
Claim~, we fix $\zeta$ with $0<\zeta<\eta/2$ such that:
\begin{enumerate}
...
to Proposition~ there is a family of maps
...
length less than $\zeta$. By Lemma~ this implies that
...
proof of Theorem~ is to establish Proposition~.
...
$\widetilde \Gamma(t)$ required by Proposition~ is to evolve
...
For a proof of this result, see Theorem 1.13 in .

### The proof of Proposition~\protect{ in a simple case}
...
$$\frac{dA}{dt}(t)\le -2\pi-\frac{1}{2}R_min(t)A(t)$$
in the sense of forward difference quotients.

**Proof.** According to results of Hildebrandt and Morrey,
and , for each $t\in [t_0,t_1]$, there is a smooth
minimal disk spanning $c(\cdot,t)$. Fix $t'\in [t_0,t_1)$ and
consider a smooth minimal disk $D\to (M,g(t'))$ spanning
$c(\cdot,t)$. It is immersed, see  or .
...
=\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{det(h_{\alpha\beta})(t)}da.$$ Of course,

$$

$$
\begin{aligned}
\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{det(h_{\alpha\beta}(t))}da & = & -\int_{\varphi_{t'}(D)}\Bigl(Tr\,Ric^T\Bigr)\sqrt{det(h_{\alpha\beta}(t))}da \\
& & +\int_{\varphi_{t'}(D)}div\left(\frac{\partial
...
$$
\begin{aligned}
\lefteqn{-\int_{\varphi_{t'}(D)}\Bigl(Tr\,Ric^T\Bigr)\sqrt{det(h_{\alpha\beta}(t'))}da } \\
 & = &
-\int_{\varphi_{t'}(D)}K_{\varphi_{t'}(D)}da
-\f1{1}{2}\int_{\varphi_{t'}(D)}(|\hat A|^2+R)da \\
& \le & -\int_{\varphi_{t'}(D)}K_{\varphi_{t'}(D)}\sqrt{det(h_{\alpha\beta}(t'))}da-\f1{1}{2}Area{\varphi_{t'}(D)}\min_{x\in M}\{R(x,t')\}.
\end{aligned}
$$
...
$$\int_{\varphi_{t'}(D)}div\left(\frac{\partial
\varphi_{t'}}{dt}\right)^T\sqrt{det(h_{\alpha\beta}(t'))}da=-\int_{ \varphi_{t'}(\partial
...
$$\left(\frac{d\varphi_t}{dt}\bigl|_{t=t'}\bigr.\right)\cdot n=k_geod.$$
...
$$
\begin{aligned}
\lefteqn{\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{det(h_{\alpha\beta}(t))}da } \\\ & \le &
-\int_{\varphi_{t'}(D)}K_{\varphi_{t'}(D)}da-\int_{\varphi_{t'}(\partial
D)}k_geodds-\frac{1}{2}R_min(t')Area(\varphi_{t'}(D)).
\end{aligned}
$$
...
$$\frac{d}{dt}\bigl|_{t=t'}\bigr.\int_{\varphi_t(D)}\sqrt{det(h_{\alpha\beta}(t))}da \le -2\pi-\frac{1}{2}R_min(t')Area(\varphi_{t'}(D)).$$

Let $\psi(t)$ be the solution to the ODE
$$\psi'(t)=-2\pi-\frac{1}{2}R_min(t)\psi(t)$$
with $\psi(t^-)=A(t^-)$. The following is immediate from the
previous lemma and Lemma~.
...
$$\psi_{\delta,\hat c}'(t)=-2\pi+2\delta L_{\hat c}(t)-\frac{1}{2}R_min(t)\psi_{\delta,\hat c}(t)$$ with value $A(\hat c(t^-))$ at
...
given the proof of Lemma~. According to this argument
we have
$$\frac{d}{dt}A(\hat c(t))|_{t=t'}\le -2\pi -\frac{1}{2}R_min(t')A(c(t'))+\int_{c(x,t')}\left[k_geod(\hat
...
$$\left|k_geod(\hat c)-\left(\frac{d\varphi_t}{dt}|_{t=t'}\right)\cdot
n\right|<2\delta.$$ Integrating over the circle implies that
$$\frac{d}{dt}A(\hat c(t))|_{t=t'}\le -2\pi -\frac{1}{2}R_min(t')A(c(t'))+2\delta L_{\hat c}(t).$$
 The result is then
...
$$\frac{\partial}{\partial t}(|X|_{c^*g}^2)(x,t)=-2Ric_{g(t)}(X(x,t),X(x,t))-2k^2|X(x,t)|_{g(t)}^2,$$
and
...
with $t$. Thus, we have
$$H(|X|_{c^*g}^2)(x,t)=-2Ric_{c^*g(t)}(X(x,t),X(x,t))
...
& & -2Ric(H,H) +4k^2Ric(S,S)+2Rm(H,S,H,S),
\end{aligned}
$$
...
t}(|H|^2_{c^*g})=-2Ric(H,H)+2\langle\nabla_HH,H\rangle_{c^*g}.
$$

 Now we compute
(using the second equation from Lemma~)
...
2\langle\nabla_HH,H\rangle_{c^*g}=2g(\nabla_S\nabla_SH,H)+4k^4+4k^2Ric(S,S))+2Rm(H,S,H,S).
$$
...
Plugging this into Equation~()  and using
Equation~() yields
...
$$
\begin{aligned}
\frac{\partial}{\partial t}k^2 & = & -2Ric(H,H)+S(S\langle
H,H\rangle_{c^*g})-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle_{c^*g}
\\
& & +2k^4+4k^2Ric(S,S)+2Rm(H,S,H,S).
\end{aligned}
$$
...
Inequality~. It follows from Inequality~() that
...
**Proof.** $$\frac{d}{dt}L=\int\frac{\partial}{\partial t}\sqrt{|X|_{c^*g}^2}dx.$$
By Lemma~ we have $$\frac{d}{dt}L=\int
\frac{1}{2|X|_{c^*g}}\left(-2Ric(X,X)-2k^2|X|_{c^*g}^2\right)dx.$$ Thus,

$$
\frac{d}{dt}L=\int (-Ric(S,S)-k^2)|X|_{c^*g}dx=\int(-Ric(S,S)-k^2)ds.
$$
...
Claim~ and the first equation in Lemma~ we have
...
\frac{k}{2|X|_{c^*g}}(-2Ric(X,X)-2k^2|X|_{c^*g}^2)dx \\
...
As we pointed out in the beginning of Section~ the
...
goes back to , is to replace the original situation of
...
since $Ric(V,U)=0$ for all tangent vectors $V$, by Lemma~
we have
...
$$
\begin{aligned}
\frac{\partial}{\partial t}\langle S,U\rangle_{g(t)} & = & -2Ric(S,U)
...
**Proof.** From  the equation in Claim~, we see that for  $C'$ an
...
Claim~ there is a $t'_1\le t_1$ such that the
...
Using Claim~ and Claim~ we compute
...
manifold immediately become real analytic (see ) and
furthermore, under the curve-shrinking flow the curves $c_1$ and
$c_2$ immediately become analytic (see ).
...
$$\frac{d Area\,A}{d t}(t')=\int_{A}(-Tr(Ric^T(g(t'))))da
+\int_{\partial A}-k_geodds,$$ where $Ric^T$ is the
...
$$\int_{\partial A}-k_geodds =\int_{A}Kda$$
...
$$\int_{\partial A}-k_geodds=\int_{A}Kda-\sum_{i=1}^k2\pi(n_i-1).$$ Thus, we see
$$\frac{d Area\, A}{d t}(t')\le\int_{A}(-Tr(Ric^T(g(t')))+K)da.$$
...
M}Rm(x,t')|$. Hence,
$$\frac{dArea\, A}{dt}(t')\le (2n-1)|max_{x\in M}Rm(x,t')|\mu(t').$$
...
## Proof of Proposition~\protect{}

Now we are ready to use the curve-shrinking flow for ramps in
$M\times S_\lambda^1$ to establish Proposition~ for $M$. As
...
The first step in the proof of Proposition~ is to identify
...
$c_\infty\in X$. Applying Claim~ we see that there is $N$
such that the conclusion of Claim~ holds with $\zeta$
replaced by $\zeta/2$ for $c_\infty$ and all $n\ge N$. Clearly, then
by continuity for all $n\ge N$ the conclusion of Claim~
...
we fix $n$ sufficiently large so that Corollary~
...
Corollary~ that $\widetilde \Gamma$ satisfies the
conclusions of Lemma~ except possibly the last one.
...
first-half subinterval $[\xi_n^k,\xi_n^k\cdotexp(\pi i/n)]$.
The tangent vector $X(x)$ is $(\lambda/2\pi) v$ at the initial point
of this subinterval and is $L\psi_n(\xi_n^k\cdotexp(\pi
...
on the second-half subinterval $[\xi_n^k\cdotexp(\pi
...
large so that $\widetilde \Gamma$ satisfies Lemma~.
...
Lemma~ with initial data the ramp
...
stronger result, see Lemma~.) The next step in the
...
Proposition~ for individual curves (or rather the slightly
stronger version of it) holds for every $\hat c\in {\mathcal S}$.
Then we complete the proof of Proposition~ using the fact
...
$\int_{c(x,t'')}|H|ds$. According to Lemma~ there is a
...
Equation~() that there is a constant $C_5$ (we take
...
Section 4 of . Once $k^2$ is bounded by $\widetilde
C_0/(t-t')$ the higher derivative statements are standard, see
. For completeness we have included the proof of
...
these intervals. According to the construction and Lemma~
...
Proposition~ holds for $p_1\widetilde\Gamma_c^\lambda(t)$.
...
solution to Equation~() with initial condition
...
solutions to Equation~(). In the first claim the
...
Equation~() on these sub-intervals. In both claims the
...
**Claim.**
Fix $C_4$ as in Claim~ and fix a constant
...
such that $R_min(t)\ge -2C_6$ for all $t\in [t_0,t_1]$. Let
...
$\beta_i=f(b_i)-w_{f(t_0),t_0}(b_i)$. Then by Claim~ we
have
...
Now let us return to the proof of Lemma~. Recall
...
Lemma~. Now it is time to fix $B$. First, we fix
$\widetilde A=W(\Gamma)+\zeta$,  we let $C_2$ be as in
Corollary~, $C_4$ be as in Claim~,
$C_5$ be as in Equation~(), and $C_6$ be as in the proof
of Claim~. Then we have $\delta'$ depending on
$C_6,C_4,\widetilde A$ as in Claim~. We fix $B$ so
that:
\begin{enumerate}
\item[(1)]  $B\ge 3C_5(\delta')^{-1}$,
\item[(2)] $B\ge 3e^{C_2(t_1-t_0)}\zeta^{-1}$, and
\item[(3)] $B>C_2/(log4-log3)$.
\end{enumerate}
The first step in the proof of Lemma~ is the
following:
...
According to Lemma~ the maximum of the norms of
...
trivial. It now follows from Lemma~,
Claim~ and Remark~ that one of the
...
$$\frac{dA(t)}{dt}\le -2\pi-\frac{1}{2}R_min(t)A(t)$$
in the sense of forward difference quotients.
\end{enumerate}
...
$$\frac{dA(p_1\widetilde\Gamma_c^{\lambda_n}(t))}{dt}\le -2\pi-\frac{1}{2}R_min(t)A(p_1\widetilde\Gamma_c^{\lambda_n}(t))+\delta''$$ in the
...
Claim~ that for every $t'<t''$ in $[t_0,t_1]$ the
areas satisfy
...
Claim~. Invoking Claim~ and the fact
...
first inequality in Corollary~ to see that the
...
Now we wish to pass from  Lemma~ which deals with an
individual $c\in S^2$ to a proof of Proposition~ which deals
...
Equation~() with initial condition $v_{\hat
...
Lemma~ the fact that the area of the minimal annulus
...
the proof of Proposition~ assuming it.
...
Corollary~ we see that there is a constant $C_8$
...
Lemma~. Then we set $\bar \mu$ equal to the constant
...
Lemma~ implies that for every $t\in [t_0,t_1]$ the
...
c}^\lambda(t')$. Now we have all the hypotheses of Lemma~
...
Fix $\mu>0$ sufficiently small so that Lemma~ and
Claim~ hold. Then we choose a $\mu/2$-net $X$ for
$\widetilde\Gamma$. We take $\lambda$ sufficiently small so that
Lemma~ holds for every $\hat c\in {\mathcal S}$. We
...
Lemma~ either $A(p_1\widetilde\Gamma_{\hat
...
Proposition~ below gives the precise result along
these lines. Before stating that proposition, we show that its
hypotheses hold in the situation that arises in Lemma~.
Let us recall  the situation of Lemma~. We have ramps
...
$\gamma\coprod \hat \gamma$. According to , $\psi$
...
\gamma$. Thus, the area of the image is at most $\mu$. According to
 the only branch points on the boundary are false branch
...
at each interior branch point.
  Since the geodesic
curvature $k_geod$ of the boundary of the annulus is given by $k\cdot n$
where $n$ is the unit normal vector along the boundary pointing into $A$, we
see that the restriction of the geodesic curvature to $\gamma$, $k_geod\colon \gamma\to \Ar$ has the property that for any sub-arc $I$ of
$\gamma$ of length $r$ we have
$$\int_I|k_geod|ds<\delta.$$ Lastly, because the map of $A$ into $M\times
...
Lemma~, then replacing $\hat \gamma$ by a close $C^2$
...
that  establishing the conclusion of Lemma~ for a
...
With all these preliminary remarks, we see that Lemma~
follows from:
...
it holds for any $0<r'<r$. This allows us to assume that $r<min((C'')^{-1/2},1)$. Now let us scale the metric by $4r^{-2}$. The
...
The function $k_geod\colon c_0\to \Ar$ is smooth. We choose a
regular value $\alpha$ for $k_geod$ with $1<\alpha<1.1$. In
this way we divide $c_0$ into two disjoint subsets, $Y$ where
$k_geod>\alpha$, and $X$ where $k_geod\le \alpha$. The
subset $Y$ is a union of finitely many disjoint open intervals and
$X$ is a disjoint union of finitely many closed intervals.

**Remark.**
The condition on $k_geod$ implies that for any arc $J$ in
...
Now we fix $0<\delta'<1/10$ so that Claim~ holds, and
...
and $D'_y$ is called its *right-hand side*. See Fig.~.
...
**Lemma.**
 ...
 Then
$$l(\xi)\le \left(\int_\xi k_geodds+Area(B)\right).$$

**Proof.** We begin with a preliminary computation. We denote by $a(B)$ the
area of $B$. We define
$$t_{\xi}=\int_{\xi}k_geodds \ \ \ and \ \ \ T_\xi=\int_{\xi}|k_geod|ds.$$
...
**Claim.**
Let $a\in int\,\xi$. There is a shortest path in $B$ from $a$
...
$B$ from $a$ to $v$. Fix $a\in int(\partial A\cap B)$. It is
...
**Claim.** $$\frac{dl}{dt}(t)\le max_{\psi\in E}h(\psi)\int_{\lambda(t)}k_geodds.$$
...
$$l'(t)=\int_{\lambda(t)}k_geod(\psi)h(\psi)|cos(\theta(\psi,t))|ds$$ where $\theta(\psi,t)$ is the angle at
...
$$\int_{W(t)}Kda+\int_{\lambda(t)}k_geodds=\theta_B.$$
From Claim~ we conclude that
$$\int_{\lambda(t)}k_geodds\le t_\xi+a(B).$$

Of course, by Claim~ we have $max_{\psi\in
...
$$3\pi/2+\int_{c_0'}k_geodds<3\pi/2+\delta,$$
...
geodesic contained in the ball $B$. Lemma~ then shows
...
**Proof.** The first statement is immediate from Claim~.
...
Using Claim~ we see that if there is  a  focusing
...
Claim~, that there is a focusing triangle ${\mathcal
...
to Corollary~, we have $\sum_il(\xi_i)<\delta+\mu$. We
also know by Remark~ that the length of $X_J$ is at
...
containing $x$ that is not an embedded arc. Let $int\,B$ be
the complementary component of $D'_x$ in $A$ that contains
$c_0\setminus\{x\}$. There is a natural compactification of $int\, B$ as a disk and an immersion of this disk into $A$, an
...
puts us back in the case ruled out in Lemma~.
...
in the case ruled out by Lemma~.
...
$\xi$ by its complement, $c_0\setminusint\,\xi$, gives us
exactly the situation of the previous lemma. (The length of
$c_0\setminusint\,\xi$ is at least one since the length of
$c_0$ is at least $2$.)
...
Corollary~ we can  construct the maximal focusing
...
content of Corollary~. The same reasoning works when we
...
has both endpoints in $\partial A$. By Lemma~, the
...
with $D_y\cap D_{y'}\not=0$. Then, using
Corollary~ we construct the maximal focusing
...
**Proof.** Since, by Corollary~, every base of a focusing triangle
...
embedding, it follows from Claim~ and the choice of
...
## Proof of the first inequality in Lemma~\protect{}
...
The rest of this section is devoted to the proof of this lemma. In
 such a local estimate was established when the ambient
...
. The passage from Euclidean space to a general
...
flow by an equivalent flow, introduced in , called the
...
**Proof.** This is immediate from Corollary~ applied to all
of $c(\cdot,t)$.
...
<\sqrt{\frac{2}{t-t'}}\cdot D_0.$$ Using Equation~(),
...
(kds)}{\partial t}\right|.$$ Using Claim~ for the first
term and Claim~ and arguing as in the proof of
Lemma~ for the second term, we have
$$\left|\frac{d}{dt}\int_{\gamma_t}\varphi
kds\right|\le
...
curvature bound from Claim~. Since the ends of
...
$r/2$, according to Lemma~ the length of
...
not constant. But the graph flow defined above, and studied in
 (in the case of Euclidean background metric), is defined
...
proof of the first equation in of Lemma~ we have we
have
...
\nabla_Z(\psi(Z)Z),Z\rangle-2Ric(Z,Z).$$ By Item (1) in
Claim~ and Item (4) in Claim~ we have
...
Claim~, the first term is at most
...
$z$-axis
 ...
 We follow closely the discussion in Section 4 of  (pages 293 -294).
...
$$
 ...
 Thus, it
follows from Equation~() that we have
$$\frac{\partial}{\partial
...
\frac{(|Y|^2)_{zz}}{|Z|^2}-\frac{2|\nabla_ZY|^2}{|Z|^2}
 ...
 \nabla_ZY,Z\rangle \\
 &  & -2Ric(Y,Y)+2\frac{Rm(Y,Z,Y,Z)}{|Z|^2}-2\langle \nabla_Y(\psi(Z)Z),Y\rangle.
\end{aligned}
$$
...
Z,Y\rangle.$$ Hence, putting all this together and using Claim~
we have
$$\frac{\partial}{\partial
 t}|Y|^2  =
\frac{(|Y|^2)_{zz}}{|Z|^2}-\frac{2|\nabla_ZY|^2}{|Z|^2}
...
the computations on p. 294 of  and adding in the error terms.
...
Using Item (4) of Claim~ this leads to
