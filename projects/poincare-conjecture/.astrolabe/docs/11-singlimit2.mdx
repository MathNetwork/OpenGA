...
and $\alpha=10^{-2}$, and Proposition~ holds for
$2\epsilon$.
...
Ricci flows. We set $t_n=**t**(x_n)$ and $Q_n=R(x_n)$. We denote
...
\item[(2)] Every point $y_n\in ({\mathcal M}_n,G_n)$ with $**t**(y_n)\le
...
through $y_n$ extends backwards for a time at least $\mu\left(max(Q_n,R(y_n))\right)^{-1}$.
\end{enumerate}
...
established Inequality~(). If $J=[0,s_0]$, then we have
...
establishing Inequality~(). The last possibility is that
...
\begin{proof}
This is immediate from  Lemma~ and Assumption (5)
...
Now we are in a position to apply Corollary~. This
...
Theorem~, the limit Riemannian manifold
...
In fact, by Proposition~ for each $A<\infty$,
...
Proposition~ for all $n$ sufficiently large the
...
Corollary~ to the limit flow on
...
small scale. But this contradicts Proposition~. It
follows from this contradiction that the curvature of
$(M_\infty,g_\infty)$ is bounded.

To complete the proof of Theorem~ it remains to
...
Corollary~ to be independent of $A$. According to that
...
Proposition~, we see that, after passing to a
...
the hypothesis of Theorem~. Suppose in addition that
...
Theorem~. The hypotheses of this theorem include all
the hypotheses of Theorem~. The main difference
between the conclusions is that in Theorem~ the
...
**Proof.** In Theorem~ we proved that, after passing to a
...
Theorem~, suppose that there is a geometric limit flow
...
the proof of Theorem~ we see that any point $y\in
M_\infty$ with $R(y,-T')> 4$ has a $(2C,2\epsilon)$-canonical
neighborhood. Hence, applying Lemma~ as in the
argument in the proof of
 Corollary~ shows that for all $n$ sufficiently large,
...
result (Proposition~) this implies that, by
...
Hence by Corollary~, for all $-t_0\in (-T,0]$ we have
that
...
M}_n\}_{n=1}^\infty$ with $**t**(y_{m,n})=**t**(z_{m,n})=t_m$
...
\item[(3)] $d_{Q_nG_n}((y_{m,n}),(z_{m,n}))\le 2\,diam_{g_\infty(t_m)}(X).$
\end{enumerate}
...
point $z\in {\mathcal M}_n$ with $**t**(z_m)\le t_{m}$ and with
$R(z)\ge 4R(y_{m,n})$ has a strong $(C,\epsilon)$-canonical
neighborhood. This then
contradicts Theorem~ and completes the proof of the lemma.
\end{proof}
...
non-negative curvature, by Corollary~, $lim_{n\rightarrow\infty}\alpha_n/d_n=0$. In particular, for all $n$
...
By Claim~  we have $d_{t}(x_\infty,y)\le
...
$y$ to $x_\infty$ and $z$ exit out of opposite ends of $N$ (see Fig.~).
...
Suppose that $M_\infty$ is compact. By Proposition~
we know that
...
can apply Lemma~ to see that the scalar curvature of
$g_\infty$ is bounded on all of $M_\infty\times (-T,0]$.

If $M_\infty$ is non-compact, choose $D$ as in Lemma~.
...
$t$. Now apply Lemma~ to the closure of
...
Now for any $A<\infty$ we apply Lemma~ to the compact
...
Now let us return to the proof of Proposition~.
...
$(M_\infty,g_\infty(-T))$.
 This limit manifold
satisfies the hypothesis of Proposition~ and hence, by
...
(Theorem~), we see that the curvature is bounded on
$[-T,0]$. Since we already know it is bounded in $(-T+\delta/2,-T]$,
this completes the proof of the proposition.
\end{proof}

 It follows immediately from Proposition~ that there is a
...
Theorem~ that all time-slices of the limit flow
...
neighborhoods, it follows from Proposition~ that the
curvature is bounded on each time-slice, and hence universally
bounded by the Harnack inequality (Theorem~). Since for
...
in~. Let $T^-<T$ be such that there is a
diffeomorphism $\rho\colon M_{T^-}\times [T^-,T)\to **t**^{-1}([T^-,T))$ compatible with time and with the vector field.
...
Suppose that $({\mathcal M},G)$ is a generalized Ricci flow defined
for $0\le t<T<\infty$ satisfying the three assumptions given
in~. Let $T'<T$ be as in the previous theorem, set
...
the Lemma~, there is $r>0$ such that $R(y,t)$ is
...
$B(x,t_n,r)\times[t_n,T)$. Now applying Theorem~ we see
...
bounded independent of $n$. It follows from Lemma~
...
conclusions stated in Theorem~. Let us turn to the last
two.

### Canonical neighborhoods for $(\widehat{\mathcal M
,\widehat G)$}

We continue with the notation and assumptions of the previous
subsection. Here we establish the fifth conclusion in
Theorem~, namely the existence of strong canonical
neighborhoods for
...
core contains $(y,t_n)$, then by Proposition~ since
...
the fifth conclusion of Theorem~. It remains to establish
...
$R(x,T)>r_0^{-2}$.   Therefore, by Lemma~ $(x,T)$
...
in a strong $2\epsilon$-tube. To complete the proof of Theorem~, it
...
Proposition~, the latter implies that $X_0$ is
...
## Existence of strong $\delta$-necks sufficiently deep in
a $2\epsilon$-horn
...
$0\le t<T<\infty$ satisfying Assumptions~ and for
...
an end of the ${\mathcal H}$ disjoint from $\Omega_\rho$. See Fig.~.

**Proof.** The proof of the first statement is by contradiction. Fix $\rho>0$
and $\delta>0$ and suppose that there is no $0<h\le min(\rho\cdot\delta,\rho/2C)$ as required. Then there is a sequence
of generalized Ricci flows $({\mathcal M}_n,G_n)$ defined for $0\le
t<T_n<\infty$ satisfying Assumptions~ and points
$x_n\in {\mathcal M}_n$ with $**t**(x_n)=T_n$ contained in
...
the five hypothesis of Theorem~.

\begin{proof}
By our assumptions, Hypotheses (1) and (3) of
Theorem~ hold for this sequence.
...
each $n$, this sequence would violate Theorem~.
...
positive assumption, by Theorem~, the limit Ricci
...
non-negative curvature, according to Theorem~, this
...
According to Theorem~, after passing to a subsequence
...
of $K$. Hence, by Lemma~ the central $2$-sphere of any
...
follows from Definition~ that for all $t\in
...
from Definition~ that the flow near
...
$t'<T$. Applying Theorem~ to the flows
...
$\Ar$. Now, invoking Hamilton's result (Corollary~), we
...
Now let us establish the last statement in Theorem~. The
...
**Proof.** If $h$ satisfies the conclusion of Theorem~ for $\rho$
and $\delta$ and if $\rho'\ge \rho$ and $\delta'\ge \delta$ then $h$
also satisfies the conclusion of Theorem~ for $\rho'$
and $\delta'$. Also, any $h'\le h$ also satisfies the conclusion of
Theorem~ for $\delta$ and $\rho$. Take a sequence
...
statement of Theorem~. We of course can assume that
